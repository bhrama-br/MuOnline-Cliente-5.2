...
{	
	...
	else if ( Type==MODEL_POTION+30 )    // Â¡Ç¥
	{
		Vector(270.f,0.f,0.f,ObjectSelect.Angle);
	}
...
#endif// PET_SYSTEM
	{
		...
		if ( GetBaseClass(Hero->Class) == CLASS_KNIGHT )
		{
			...
			if ( Hero->byExtensionSkill == 1 && CharacterAttribute->Level >= 220 )
			{
				...
				{
					// 99 "\xC4\xDE\xBA\xB8 \xB0\xA1\xB4\xC9"
				...
				{
					// 2115 "\xC4\xDE\xBA\xB8 \xB0\xA1\xB4\xC9(2\xB4\xDC\xB0\xE8\xB8\xB8)"
		...
		{
			// 1480 "%s\xC0\xC7 \xBD\xBA\xC5\xB3\xC0\xBB \xB9\xDE\xC0\xBA\xC8\xC4"
			char temp[256];
			ZeroMemory(temp, sizeof(temp));
			sprintf ( temp, GlobalText[1480], SkillAttribute[BrandType].Name );
			_caution += temp;

			// 1481 "%d\xC3\xCA\xB5\xBF\xBE\xC8 \xBB\xE7\xBF\xEB\xB0\xA1\xB4\xC9\xC7\xD5\xB4\xCF\xB4\xD9"
			ZeroMemory(temp, sizeof(temp));
...
void CMainUIFSCHandler::Callback(GFxMovieView* pmovie, const char* pcommand, const char* parg)
{
	...
	else if(strcmp(pcommand, "onClickCommunityBtn") == 0)
	{
		...
		if(iLevel < 6)
		{
			...
			{
				g_pChatListBox->AddText("",GlobalText[1067],SEASON3B::TYPE_SYSTEM_MESSAGE);	// "\xB7\xB9\xBA\xA7 6\xBA\xCE\xC5\xCD \xB3\xBB\xC4\xA3\xB1\xB8 \xB1\xE2\xB4\xC9 \xBB\xE7\xBF\xEB\xC0\xCC \xB0\xA1\xB4\xC9\xC7\xD5\xB4\xCF\xB4\xD9."
			}
		}
		else
		{
