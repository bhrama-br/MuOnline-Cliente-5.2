...
class CBTree
{
	protected:
		...
		void CycleFrom( CBNode<T, S>* pNode, DWORD_PTR dwParam);
		void ( *m_Process)( T Data, S CompValue, DWORD_PTR dwParam);
	...
	public:
		...
		void Cycle( void ( *Process)( T Data, S CompValue, DWORD_PTR dwParam), DWORD_PTR dwParam);
...
template <class T, class S>
void CBTree<T, S>::Cycle( void ( *Process)( T Data, S CompValue, DWORD_PTR dwParam), DWORD_PTR dwParam)
{
	m_Process = Process;
...
template <class T, class S>
void CBTree<T, S>::CycleFrom( CBNode<T, S>* pNode, DWORD_PTR dwParam)
{
	if ( pNode->GetLeft() != NULL)
	{
...
template <class T, class S>
void ProcessGetOptimizeList( T Data, S CompValue, DWORD_PTR dwParam)
{
	DWORD_PTR *pdwParam = ( DWORD_PTR *)dwParam;
...
BOOL CBTree<T, S>::GetOptimizeList( T *pData, S *pCompValue)
{
	...
	// DWORD_PTR, nao DWORD: estes tres slots carregam ponteiros, e em 64 bits
	// (arm64 do Android) DWORD tem 32 bits e truncaria os enderecos.
...
void CDimension<T>::CheckDimensionSize( int nIndex)
{
	if ( nIndex >= m_nSize)
	{
		// nNewSize precisa sobreviver ao laco: era declarado dentro do for e usado
		// depois, o que so compila pela extensao de escopo nao-conforme do MSVC.
