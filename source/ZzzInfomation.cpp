...
void OpenFilterFile(char *FileName)
{
	...
	delete [] Buffer;
	// O fclose(fp) que existia aqui era o SEGUNDO sobre o mesmo descritor: ele ja
	// tinha sido fechado logo apos as leituras, acima. No CRT do Windows a
	// segunda chamada apenas devolve EOF, mas no musl (Emscripten/Android) o
	// FILE ja foi liberado e fclose salta por um ponteiro de funcao nulo --
	// abortava a carga com "RuntimeError: null function" bem no meio de
	// OpenBasicData. Mesmo bug que ja havia em CGlobalBitmap::OpenJpeg.
}
...
{
	if((SMDFile=Platform::LegacyFileOpen(FileName,"rb")) == NULL)
	{
		char Text[256];
    	sprintf(Text,"%s - File not exist.",FileName);
		g_ErrorReport.Write( Text);
...
void PrintItem(char *FileName)
{
    ...
    fprintf(fp,"                \x3F""\x3F""\x3F""  \x3F""\x3F""\x3F""\x3F""\x3F""\x3F""\x3F"" \x3F""\x3F""\x3F""\x3F""\x3F""\x3F""\x3F"" \x3F""\x3F""\x3F""\x3F"" \x3F""\x3F""\x3F""\x3F""\x3F"" \x3F""\x3F""\x3F""\x3F""\x3F"" \x3F""\x3F""\x3F""\x3F""\xF8 \x3F""\x3F""\xE3\xE1\x3F""\x3F""\x3F""\x3F""\n");
	//fprintf(fp,"                \x3F""\x3F""\x3F""    \x69""\x3F""\x3F""\x3F""\x3F""\x3F""\x3F""\x3F""\x3F""\x3F""\x3F""\x3F""\n");
	bool Excellent = true;
...
{
	if((SMDFile=Platform::LegacyFileOpen(FileName,"rb")) == NULL)	return;
	SMDToken Token;
	while(true)
	{
		Token = (*GetToken)();//��ȣ
		if(Token == END) break;
		if(Token == NAME && strcmp("end",TokenString)==NULL) break;
		// MonsterScript e um vetor fixo e EditMonsterNumber so crescia. O arquivo
		// atual tem 656 entradas, longe de MAX_MONSTER, mas um ++ sem limite
		// escrevendo em vetor fixo e o tipo de coisa que quebra quando alguem
		// adiciona linhas -- e o custo de conferir e uma comparacao.
		if(EditMonsterNumber >= MAX_MONSTER) break;
        MONSTER_SCRIPT *m = &MonsterScript[EditMonsterNumber++];
		m->Type = (int)TokenNumber;
		Token = (*GetToken)();
		// Copia limitada ao campo: TokenString tem 256 bytes e Name tem 64.
		Token = (*GetToken)();
		strncpy(m->Name,TokenString,sizeof(m->Name)-1);
...
void CHARACTER_MACHINE::CalculateAttackSpeed()
{
	...
	if( (false == Amulet->bPeriodItem || false == Amulet->bExpiredPeriod) && Amulet->Type == ITEM_HELPER + 114 )
	{
		...
		//Character.AttackSpeed += Amulet->WeaponSpeed;	// �������� �Ѿ���� ��? ���� Ȯ�¥ë���.
