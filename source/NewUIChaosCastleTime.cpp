...
{
	g_ErrorReport.Write("[ChaosTrace] Create: window=%p manager=%p pos=%d,%d.\r\n", this, pNewUIMng, x, y);
	if( NULL == pNewUIMng )
	{
		g_ErrorReport.Write("[ChaosTrace] Create rejected: null manager.\r\n");
		return false;
	}
	
	m_pNewUIMng = pNewUIMng;
	m_pNewUIMng->AddUIObj( SEASON3B::INTERFACE_CHAOSCASTLE_TIME, this );
	g_ErrorReport.Write("[ChaosTrace] Registered in UI manager.\r\n");
	
	SetPos(x, y);
	g_ErrorReport.Write("[ChaosTrace] Position stored.\r\n");
