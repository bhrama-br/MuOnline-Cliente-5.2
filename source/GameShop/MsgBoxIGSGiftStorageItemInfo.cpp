											   ...
											   unicode::t_char* pszNum, unicode::t_char* pszPeriod)
{
	...
	// Num
	sprintf(m_szNum, GlobalText[3040], pszNum);		// "\xBC\xF6\xB7\xAE : %s"

	// Period
	sprintf(m_szPeriod, GlobalText[3039], pszPeriod);	// "\xB1\xE2\xB0\xA3 : %s"

	// ID Info
	// "\'%s\' \xB4\xD4\xC0\xCC \xBA\xB8\xB3\xBD \xBC\xB1\xB9\xB0\xC0\xD4\xB4\xCF\xB4\xD9."
...
void CMsgBoxIGSGiftStorageItemInfo::RenderTexts()
{
	...
	// Title "\xBC\xB1\xB9\xB0 \xC1\xA4\xBA\xB8\xC3\xA2" 
	...
	if( m_wItemCode == 65535 )
	{
		sprintf(szText, "\xBE\xC6\xC0\xCC\xC5\xDB\xC4\xDA\xB5\xE5\xB0\xA1 \xBE\xF8\xBD\xC0\xB4\xCF\xB4\xD9.");
	}
	else
	{
