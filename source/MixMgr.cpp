...
BOOL CMixRecipes::GetRecipeName(MIX_RECIPE * pRecipe, unicode::t_char * pszNameOut, int iNameLine, BOOL bSimilarRecipe)
{
	...
	if (pRecipe->m_bMixOption == 'C')
	{
		...
		assert(optionTextlist.size() == 2 && "\xBF\xC9\xBC\xC7\xC0\xBA 2\xB0\xB3\xBF\xA9\xBE\xDF \xC7\xD4");
		if (iNameLine == 1)
		{
...
int CMixRecipes::GetSourceName(int iItemNum, unicode::t_char * pszNameOut, int iNumMixItems, CMixItem * pMixItems)
{
	...
	else
	{
			...
			unicode::_sprintf(szTempName, "%s", GlobalText[2335]);
		else if (pMixRecipeItem->m_sTypeMin == 0 && pMixRecipeItem->m_sTypeMax == ITEM_BOOTS+MAX_ITEM_INDEX-1)
			unicode::_sprintf(szTempName, "%s", GlobalText[2336]);
...
{
	assert(m_iMixType >= MIXTYPE_GOBLIN_NORMAL && m_iMixType < MAX_MIX_TYPES && "\xC1\xA4\xC0\xC7\xB5\xC7\xC1\xF6 \xBE\xCA\xC0\xBA \xC1\xB6\xC7\xD5\xC3\xA2");
	return m_iMixType;
}
