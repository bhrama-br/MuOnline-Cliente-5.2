...
void SEASON3B::CNewUIMessageBoxButton::Render()
{
	if(m_bEnable == false)
	{
		SetLegacyColor4f(0.8f, 0.8f, 0.8f, 0.9f);
	}
	else
	{
	...
	if(m_bEnable == false)
	{
		SetLegacyColor4f(0.6f, 0.6f, 0.6f, 0.6f);
	}
...
bool SEASON3B::CServerLostMsgBoxLayout::SetLayout()
{
	...
	if(0 == pMsgBox)
		return false;
	if(NULL == pMsgBox->Create(MSGBOX_COMMON_TYPE_OK, 10.f))
		return false;
...
bool SEASON3B::CGuildRequestMsgBoxLayout::SetLayout()
{
	...
	if(NULL == pMsgBox->Create(MSGBOX_COMMON_TYPE_OKCANCEL))
		return false;
...
bool SEASON3B::CTradeAlertMsgBoxLayout::SetLayout()
{
	...
	DWORD adwColor[4] = { (DWORD)RGBA(255, 178, 0, 255), (DWORD)RGBA(255, 178, 0, 255), (DWORD)RGBA(255, 178, 0, 255), (DWORD)RGBA(255, 32, 32, 255) };
...
bool SEASON3B::CGuildWarMsgBoxLayout::SetLayout()
{
	...
	if(0 == pMsgBox)
		return false;
	if(NULL == pMsgBox->Create(MSGBOX_COMMON_TYPE_OKCANCEL, 15.f))
		return false;
...
bool SEASON3B::CServerImmigrationErrorMsgBoxLayout::SetLayout()
{
	...
	if(0 == pMsgBox)
		return false;
	if(NULL == pMsgBox->Create(MSGBOX_COMMON_TYPE_OK))
		return false;
...
bool SEASON3B::CCry_Wolf_Result_Set_Temple::SetLayout()
{
	...
	{
		wsprintf(Text, "%s", GlobalText[2000]);
		pMsgBox->AddMsg(Text, TextColor);
		wsprintf(Text, "%s", GlobalText[2001]);
		pMsgBox->AddMsg(Text, TextColor);
	}
	else
	{
