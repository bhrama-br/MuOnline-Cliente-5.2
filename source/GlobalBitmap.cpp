...
bool CGlobalBitmap::OpenJpeg(GLuint uiBitmapIndex, const std::string& filename, GLuint uiFilter, GLuint uiWrapMode)
{
	...
	FILE* infile = Platform::LegacyFileOpen(filename_ozj.c_str(), "rb");
	if(infile == NULL) 
	{
		return false;
	}	
	...
	fclose(infile);
	// O conteudo ja esta em CData e a libjpeg le da MEMORIA (jpeg_mem_src),
	// entao o descritor nao serve mais. Anular evita que os fclose adiante,
	// herdados de quando a libjpeg lia do FILE*, fechem o mesmo arquivo de
	// novo: o MSVC tolera, a musl libera a estrutura e a segunda chamada
	// salta para um ponteiro invalido ("function signature mismatch" no wasm).
	infile = NULL;
	...
	if(cinfo.output_width<=MAX_WIDTH && cinfo.output_height<=MAX_HEIGHT)
	{
		...
		strncpy(pNewBitmap->FileName, filename.c_str(), MAX_BITMAP_FILE_NAME); pNewBitmap->FileName[MAX_BITMAP_FILE_NAME - 1] = '\0';
		...
		#if !defined(_WIN32)
	// GL_CLAMP nao existe no GLES3; ver TraduzirWrapLegado.
	uiWrapMode = TraduzirWrapLegado(uiWrapMode);
#endif
...
bool CGlobalBitmap::OpenTga(GLuint uiBitmapIndex, const std::string& filename, GLuint uiFilter, GLuint uiWrapMode)
{
    ...
    FILE *fp = Platform::LegacyFileOpen(filename_ozt.c_str(), "rb");
    if(fp == NULL)
	{
		return false;
	}
...
{
	FILE* fp = Platform::LegacyFileOpen(src.c_str(), "rb");
	if(fp == NULL) 
	{
		return false;
	}
	...
	fp = Platform::LegacyFileOpen(dest.c_str(), "wb");
	if(fp == NULL) 
		return false;
