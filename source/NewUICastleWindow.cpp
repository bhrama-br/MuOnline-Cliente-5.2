...
void CNewUICastleWindow::RenderOutlineUpper(float fPos_x, float fPos_y, float fWidth, float fHeight)
{
	...
	SetLegacyColor4f(0.0f, 0.0f, 0.0f, 0.3f);
...
void CNewUICastleWindow::RenderTaxManagingTab()
{
	...
	EnableAlphaTest();
	SetLegacyColor4f( 0.4f, 0.4f, 0.4f, 0.5f );
	...
	ptOrigin.y += 20;
	unicode::_sprintf(szTemp, "%s", GlobalText[1578]);
	g_pRenderText->RenderText( ptOrigin.x+15, ptOrigin.y, szTemp, 160, 0, RT3_SORT_CENTER );

	ptOrigin.y += 12;
	unicode::_sprintf(szTemp, "%s", GlobalText[1579]);
	g_pRenderText->RenderText( ptOrigin.x+15, ptOrigin.y, szTemp, 160, 0, RT3_SORT_CENTER );
	...
	unicode::_sprintf( szTemp, "%s", GlobalText[1246] );
...
void CNewUICastleWindow::RenderCastleItem(int nPosX, int nPosY, LPPMSG_NPCDBLIST pInfo)
{
	...
	if( g_SenatusInfo.IsGate(pInfo) )	// ¼º¹®
	{
		...
		if( pInfo->btNpcLive )
		{
			...
			SetLegacyColor4f( 1.0f, 0.0f, 0.0f, 1.0f );
			...
			SetLegacyColor4f( 0.0f, 1.0f, 0.0f, 1.0f );
	...
	if( g_SenatusInfo.IsStatue(pInfo) )
	{
		...
		if( pInfo->btNpcLive )
		{
			...
			SetLegacyColor4f( 1.0f, 1.0f, 0.0f, 1.0f );
