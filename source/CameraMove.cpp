...
bool CCameraMove::LoadCameraWalkScript(const std::string& filename)
{
	...
	FILE* fp = Platform::LegacyFileOpen(filename.c_str(), "rb");
	if(fp == NULL)
		return false;
	...
	// A contagem no arquivo tem 4 bytes, nao sizeof(size_t).
	//
	// Quem gravou foi o cliente de PC, 32 bits, onde size_t mede 4. Em Android
	// (LP64) sizeof(size_t) e 8, entao o fread consumia 4 bytes a mais: `size`
	// recebia os bytes do primeiro waypoint como parte alta e virava um numero
	// astronomico, e o laco abaixo alocava WAYPOINT sem limite ate o sistema matar
	// o processo. E o mesmo defeito que o `long` em ZzzBMD.cpp e o sizeof(ponteiro)
	// em w_PetProcess.cpp: largura de campo que casa por acaso em 32 bits.
	//
	// Em 32 bits a leitura continua de 4 bytes: o build de PC nao muda.
	uint32_t size = 0;
	...
	for(uint32_t i=0; i<size; i++)
	{
		...
		// Conferir o retorno do fread importa aqui: com o arquivo truncado, o
		// original enfileirava waypoints nao inicializados em vez de parar.
		if(fread(pWayPoint, sizeof(WAYPOINT), 1, fp) != 1)
		{
			delete pWayPoint;
			break;
		}
...
bool CCameraMove::SaveCameraWalkScript(const std::string& filename)
{
	if(!m_listWayPoint.empty()) {
		FILE* fp = Platform::LegacyFileOpen(filename.c_str(), "wb");
		if(fp == NULL)
			return false;

		DWORD dwSign = 0x00535743;
		fwrite(&dwSign, 4, 1, fp);
		// 4 bytes, para o arquivo continuar identico ao que o PC 32 bits grava --
		// ver o comentario na leitura.
		const uint32_t size = (uint32_t)m_listWayPoint.size();
...
void CCameraMove::RenderWayPoint()
{
	...
	renderer.Color4f(1.0f,0.0f,0.0f,0.8f);
    auto iter = m_listWayPoint.begin();
	for(; iter != m_listWayPoint.end(); iter++) {
		WAYPOINT* pWayPoint = (*iter);
		renderer.Normal3f(0.0f, 0.0f, 1.0f);
		renderer.Vertex3f(pWayPoint->fCameraX+50-10, pWayPoint->fCameraY+50-10, pWayPoint->fCameraZ);
		renderer.Vertex3f(pWayPoint->fCameraX+50+10, pWayPoint->fCameraY+50-10, pWayPoint->fCameraZ);
		renderer.Vertex3f(pWayPoint->fCameraX+50+10, pWayPoint->fCameraY+50+10, pWayPoint->fCameraZ);
		renderer.Vertex3f(pWayPoint->fCameraX+50-10, pWayPoint->fCameraY+50+10, pWayPoint->fCameraZ);
	}
	...
	renderer.Color4f(1.0f,1.0f,1.0f,0.5f);
	iter = m_listWayPoint.begin();
	for(; iter != m_listWayPoint.end(); iter++) {
		WAYPOINT* pWayPoint = (*iter);
		renderer.Vertex3f(pWayPoint->fCameraX+50, pWayPoint->fCameraY+50, pWayPoint->fCameraZ);
	}
