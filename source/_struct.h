...
{
	WORD				Type;
	// 64, nao 32: dois nomes em NpcName_Eng.txt nao cabiam em 32 --
	// "Natasha the Firecracker Merchant" (32 caracteres) e "Illusion Castle Sacred
	// Item Storage" (35). O strcpy em OpenMonsterScript escrevia por cima de Level
	// e de Attribute.Life, e no MSVC isso passava calado; o FORTIFY do bionic
	// aborta o processo ("prevented 33-byte write into 32-byte buffer").
	//
	// Aumentar o campo, em vez de truncar a copia: no PC o nome aparece INTEIRO
	// hoje, porque a leitura segue ate o terminador que caiu no campo vizinho.
	// Truncar em 31 encurtaria esses dois nomes na tela do PC -- corrigir um
	// estouro nao deveria mudar o que o jogador ve.
	//
	// Seguro porque MONSTER_SCRIPT nunca e lida nem gravada como bloco binario:
	// so o script de texto a preenche. Custo: 32 KB nos 1024 registros.
	char				Name[64];
	WORD				Level;
