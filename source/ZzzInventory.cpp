...
int RenderTextList(int sx,int sy,int TextNum,int Tab, int iSort = RT3_SORT_CENTER)
{
	...
	for(int i=0;i<TextNum;i++)
	{
		...
		case TEXT_COLOR_BLUE:
            SetLegacyColor3f(0.5f,0.7f,1.f);
			break;
        case TEXT_COLOR_GRAY:
			SetLegacyColor3f(0.4f,0.4f,0.4f);
            break;
		case TEXT_COLOR_GREEN_BLUE:
			SetLegacyColor3f(1.f,1.f,1.f);
			break;
		case TEXT_COLOR_RED:
            SetLegacyColor3f(1.f,0.2f,0.1f);
			break;
		case TEXT_COLOR_YELLOW:
            SetLegacyColor3f(1.f,0.8f,0.1f);
			break;
		case TEXT_COLOR_GREEN:
            SetLegacyColor3f(0.1f,1.f,0.5f);
			break;
        case TEXT_COLOR_PURPLE:
            SetLegacyColor3f(1.f,0.1f,1.f);
            break;
		}
	...
	for(int i=0;i<TextNum;i++)
	{
		...
		g_pRenderText->SetTextColor(0xffffffff);
			...
			case TEXT_COLOR_REDPURPLE:
				SetLegacyColor3f(0.8f,0.5f,0.8f);
				break;
			case TEXT_COLOR_VIOLET:
				SetLegacyColor3f(0.7f,0.4f,1.0f);
				break;
			case TEXT_COLOR_ORANGE:
				SetLegacyColor3f(0.9f, 0.42f, 0.04f );
				break;
			}
			if ( TEXT_COLOR_DARKRED == TextListColor[i])
			{
...
void RenderHelpCategory(int iColumnType, int Pos_x, int Pos_y)
{
	...
	sprintf(TextList[TextNum], "%s", pText);
...
char ChaosEventName[][100] = {
	"\xC8\xF7\xB5\xC5\xA9 \xB0\xED\xC7\xE2 \xBF\xA9\xC7\xE0\xB1\xC7",
	"\xC6\xE6\xC6\xBC\xBE\xF6""4 \xC4\xC4\xC7\xBB\xC5\xCD",
	"\xB5\xF0\xC1\xF6\xC5\xBB\xC4\xAB\xB8\xDE\xB6\xF3",
	"\xB7\xCE\xC1\xF6\xC5\xD8 \xB9\xAB\xBC\xB1 \xB8\xB6\xBF\xEC\xBD\xBA+\xC5\xB0\xBA\xB8\xB5\xE5 \xBC\xBC\xC6\xAE",
	"256M \xB7\xA5",
    "6\xB0\xB3\xBF \xC0\xE2\xC1\xF6 \xB1\xB8\xB5\xB6\xB1\xC7",
	"\xB9\xAE\xC8\xAD\xBB\xF3\xC7\xB0\xB1\xC7(\xB8\xB8\xBF\xF8)",
	"\xB9\xC2 \xB8\xD3\xB1\xD7\xC4\xC5",
	"\xB9\xC2 T\xBC\xC5\xC3\xF7",
	"\xB9\xC2 10\xBD\xC3\xB0\xA3 \xB9\xAB\xB7\xE1\xC0\xCC\xBF\xEB\xB1\xC7"
};
...
void GetItemName ( int iType, int iLevel, char* Text )
{
	...
	else if(iType == ITEM_POTION+11)
	{
		...
		case 12:
			...
			break;
		case 13:
			sprintf(Text, "%s", GlobalText[117]); break;
        case 14:
			sprintf(Text, "%s", GlobalText[1650]); break;
            break;

        case 15:
			sprintf(Text, "%s", GlobalText[1651]); break;
            break;
		}
	}
...
#endif //PBG_ADD_NEWCHAR_MONK_SKILL
{
    ...
    case AT_IMPROVE_GAIN_LIFE:
        sprintf(Text,"%s", GlobalText[634]);
        break;
    case AT_IMPROVE_GAIN_MANA:
        sprintf(Text,"%s", GlobalText[635]);
        break;
    case AT_IMPROVE_EVADE :
        sprintf(Text, "%s", GlobalText[746] );
        break;
    case AT_SKILL_RIDER :
       gSkillManager.GetSkillInformation( Option, 1, NULL, &iMana, NULL);
...
//			TextBold[TextNum] = true;
	...
	{
		{
			...
			if(GetPersonalItemPrice(indexInv, price, g_IsPurchaseShop)) 
			{
				...
				if((int)gold < price && g_IsPurchaseShop == PSHOPWNDTYPE_PURCHASE) 
				{
					TextListColor[TextNum] = TEXT_COLOR_RED;
					TextBold[TextNum] = true;
					sprintf(TextList[TextNum], "%s", GlobalText[423]);
					TextNum++;
					sprintf(TextList[TextNum],"\n");TextNum++;SkipNum++;
				}
			}
			else if(g_IsPurchaseShop == PSHOPWNDTYPE_SALE)
			{
				TextListColor[TextNum] = TEXT_COLOR_RED;
				TextBold[TextNum] = true;
				sprintf(TextList[TextNum], "%s", GlobalText[1101]);
				TextNum++;
				sprintf(TextList[TextNum],"\n");TextNum++;SkipNum++;
			}
		}
    ...
    if (ip->Type>=ITEM_POTION+23 && ip->Type<=ITEM_POTION+26 )
    {
		if(ip->Type == ITEM_POTION+23)
		{
			...
			case 1: sprintf ( TextList[TextNum], "%s", GlobalText[906] ); break;
			}
		}
		else if(ip->Type == ITEM_POTION+24)
		{
			...
			{
			case 0: sprintf ( TextList[TextNum], "%s", p->Name ); break;
			case 1: sprintf ( TextList[TextNum], "%s", GlobalText[907] ); break;
			}
		}
		else{
	...
	else if(ip->Type == ITEM_POTION+12)
	{
		switch(Level)
		{
		case 0:sprintf(TextList[TextNum],"%s", GlobalText[100]);break;
		case 1:sprintf(TextList[TextNum],"%s", GlobalText[101]);break;
	...
	else if(ip->Type == ITEM_POTION+11)
	{
		...
		{
		case 0:sprintf(TextList[TextNum],"%s",p->Name);break;
		case 1:sprintf(TextList[TextNum],"%s", GlobalText[105]);break;
		case 2:sprintf(TextList[TextNum],"%s", GlobalText[106]);break;
		case 3:sprintf(TextList[TextNum],"%s", GlobalText[107]);break;
		case 5:sprintf(TextList[TextNum],"%s", GlobalText[109]);break;
		case 6:sprintf(TextList[TextNum],"%s", GlobalText[110]);break;
        case 7:sprintf(TextList[TextNum],"%s", GlobalText[111]);break;
		case 8:
		case 9:
		case 10:
		case 11:
		case 12:
			...
			break;
		case 13:
			sprintf(TextList[TextNum],"%s", GlobalText[117]);
			break;
        case 14:
            sprintf(TextList[TextNum],"%s", GlobalText[1650]);
            break;
        case 15:
            sprintf(TextList[TextNum],"%s", GlobalText[1651]);
            break;
		}
	}
	else if(ip->Type == ITEM_POTION+12)
    ...
    else if ( ip->Type==ITEM_POTION+21 )
    {
        ...
        {
        case 0: sprintf ( TextList[TextNum], "%s", p->Name ); break;
        case 1: sprintf ( TextList[TextNum], "%s", GlobalText[810] ); break;
        case 2: sprintf ( TextList[TextNum], "%s", GlobalText[1098] ); break;
        case 3: sprintf ( TextList[TextNum], "%s", GlobalText[1290] ); break;
        }
    }
    else if ( ip->Type==ITEM_HELPER+19 )
    {
        ...
        switch ( Level )
        {
		case 0: sprintf(TextList[TextNum], "%s", GlobalText[811]); break;
		case 1: sprintf(TextList[TextNum], "%s", GlobalText[812]); break;
		case 2: sprintf(TextList[TextNum], "%s", GlobalText[817]); break;
        }
    }
	else if(ip->Type == ITEM_HELPER+20)
	{
		...
		{
		case 0: sprintf( TextList[TextNum], "%s", p->Name ); break;
		case 1: sprintf( TextList[TextNum], "%s", GlobalText[922] ); break;
        case 2: sprintf( TextList[TextNum], "%s", GlobalText[928] ); break;
        case 3: sprintf ( TextList[TextNum], "%s", GlobalText[929] ); break;
		}
	}
    ...
    else if ( ip->Type==ITEM_POTION+7 )
    {
        switch ( Level )
        {
        case 0: sprintf( TextList[TextNum], "%s", GlobalText[1413] ); break;
        case 1: sprintf( TextList[TextNum], "%s", GlobalText[1414] ); break;
        }
    }
    else if ( ip->Type==ITEM_HELPER+7 )
    {
        switch ( Level )
        {
        case 0: sprintf( TextList[TextNum], "%s", GlobalText[1460] ); break; 
        case 1: sprintf( TextList[TextNum], "%s", GlobalText[1461] ); break; 
        }
    }
    else if ( ip->Type==ITEM_HELPER+11 )
    {
        switch ( Level )
        {
        case 0: sprintf( TextList[TextNum], "%s", GlobalText[1416] ); break;
        case 1: sprintf( TextList[TextNum], "%s", GlobalText[1462] ); break;
        }
    }
	else if ( ip->Type==ITEM_POTION+9 )
	{
		...
		{
		case 0:sprintf(TextList[TextNum],"%s",p->Name);break;
		case 1:sprintf(TextList[TextNum],"%s", GlobalText[108]);break;
		}
	}
    ...
    if ( ip->Type==ITEM_HELPER+19)
    {
        ...
        TextListColor[TextNum] = TEXT_COLOR_WHITE;
		sprintf ( TextList[TextNum], "%s", GlobalText[730] ); TextBold[TextNum] = false; TextNum++;

        TextListColor[TextNum] = TEXT_COLOR_DARKRED;
        sprintf ( TextList[TextNum], "%s", GlobalText[815] ); TextBold[TextNum] = false; TextNum++;
        ...
        sprintf ( TextList[TextNum], "\n" ); TextBold[TextNum] = false; TextNum++; SkipNum++;
		sprintf ( TextList[TextNum], "%s", GlobalText[87] );     TextListColor[TextNum] = TEXT_COLOR_BLUE; TextBold[TextNum] = false; TextNum++;
        ...
        case 0:
            {
                ...
                sprintf ( TextList[TextNum], "%s", GlobalText[631] ); TextListColor[TextNum] = TEXT_COLOR_BLUE; TextBold[TextNum] = false; TextNum++;
        ...
        case 1:
            {
                ...
                sprintf ( TextList[TextNum], "%s", GlobalText[629] ); TextListColor[TextNum] = TEXT_COLOR_BLUE; TextBold[TextNum] = false; TextNum++;
        ...
        sprintf ( TextList[TextNum], "%s", GlobalText[628] ); TextListColor[TextNum] = TEXT_COLOR_BLUE; TextBold[TextNum] = false; TextNum++;
		sprintf ( TextList[TextNum], GlobalText[633], 7 ); TextListColor[TextNum] = TEXT_COLOR_BLUE; TextBold[TextNum] = false; TextNum++;
		sprintf ( TextList[TextNum], "%s", GlobalText[634] ); TextListColor[TextNum] = TEXT_COLOR_BLUE; TextBold[TextNum] = false; TextNum++;
		sprintf ( TextList[TextNum], "%s", GlobalText[635] ); TextListColor[TextNum] = TEXT_COLOR_BLUE; TextBold[TextNum] = false; TextNum++;

    }

    if (ip->Type>=ITEM_POTION+23 && ip->Type<=ITEM_POTION+26 )
    {
		sprintf(TextList[TextNum],"%s", GlobalText[730]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum],"%s", GlobalText[731]);
		TextListColor[TextNum] = TEXT_COLOR_RED;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum],"%s", GlobalText[732]);
		TextListColor[TextNum] = TEXT_COLOR_RED;
		TextBold[TextNum] = false;
		TextNum++;
    }
	else if(ip->Type == ITEM_POTION+12)
	{
		if(Level <= 1)
		{
			sprintf(TextList[TextNum],"%s", GlobalText[119]);
			TextListColor[TextNum] = TEXT_COLOR_WHITE;
			TextBold[TextNum] = false;
			TextNum++;
		}
	...
	else if(ip->Type >= ITEM_HELPER+46 && ip->Type <= ITEM_HELPER+48)
	{
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2270]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "\n");
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2261]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2269]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2250]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2256]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2297]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2567]);
		TextListColor[TextNum] = TEXT_COLOR_YELLOW;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2568]);
		TextListColor[TextNum] = TEXT_COLOR_YELLOW;
		TextBold[TextNum] = false;
		TextNum++;
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2257]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2297]);
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2258]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2297]);
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2566]);
		TextListColor[TextNum] = TEXT_COLOR_YELLOW;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if(ip->Type >= ITEM_POTION+70 && ip->Type <= ITEM_POTION+71)
	{
		...
		sprintf(TextList[TextNum], "%s", GlobalText[2500+index]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if(ip->Type >= ITEM_POTION+72 && ip->Type <= ITEM_POTION+77)
	{
		...
		sprintf(TextList[TextNum], "%s", GlobalText[2502]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2509]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if( ip->Type >= ITEM_HELPER+54 && ip->Type <= ITEM_HELPER+58)
	{
		...
		sprintf(TextList[TextNum], "%s", GlobalText[2510]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		...
		TextListColor[TextNum] = TEXT_COLOR_DARKRED;
		sprintf(TextList[TextNum], "%s", GlobalText[1908]);
		TextNum++;
	...
	else if(ip->Type >= ITEM_POTION+78 && ip->Type <= ITEM_POTION+82)
	{
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2517]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2518]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		...
		{
			sprintf(TextList[TextNum], "%s", GlobalText[3115]);
			TextListColor[TextNum] = TEXT_COLOR_YELLOW;
			TextBold[TextNum] = false;
			TextNum++;
		}
		...
		sprintf(TextList[TextNum], "%s", timetext.c_str() );
		TextListColor[TextNum] = TEXT_COLOR_PURPLE;
		TextBold[TextNum] = false;
		TextNum++;

		sprintf(TextList[TextNum], "%s", GlobalText[2302] );
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2519]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if( ip->Type == ITEM_HELPER+62 )
	{
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2570] );
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2566]);
	...
	else if( ip->Type == ITEM_HELPER+63 )
	{
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2572] );
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if( ip->Type == ITEM_POTION+97 )
	{
		...
		sprintf(TextList[TextNum], "%s", GlobalText[2580]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	...
	else if( ip->Type == ITEM_POTION+98 )
	{
		...
		sprintf(TextList[TextNum], "%s", GlobalText[2581]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2502]);
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2573]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2574]);
		TextListColor[TextNum] = TEXT_COLOR_PURPLE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2708]);
		TextListColor[TextNum] = TEXT_COLOR_PURPLE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2575]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2576]);
		TextListColor[TextNum] = TEXT_COLOR_PURPLE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2577]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2578]);
		TextListColor[TextNum] = TEXT_COLOR_PURPLE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2600]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if( ip->Type == ITEM_HELPER+123 )
	{
		...
		sprintf(TextList[TextNum], "%s", GlobalText[3068]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;

		sprintf(TextList[TextNum], "%s", GlobalText[3069]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;

		sprintf(TextList[TextNum], "%s", GlobalText[3070]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2746]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2747]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2748]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if( ip->Type == ITEM_HELPER+106 )
	{
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2744]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2748]);
	...
	{
		sprintf(TextList[TextNum],"%s", GlobalText[926]);
		TextListColor[TextNum] = TEXT_COLOR_RED;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2968]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2969]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2602]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		if (g_PortalMgr.IsRevivePositionSaved())
		{
			sprintf(TextList[TextNum], "%s", GlobalText[2603]);
			TextListColor[TextNum] = TEXT_COLOR_BLUE;
			TextBold[TextNum] = false;
			TextNum++;
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2604]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2714]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum++] = false;
		sprintf(TextList[TextNum], "%s", GlobalText[2729]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum++] = false;
		sprintf(TextList[TextNum], "%s", GlobalText[3084]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum++] = false;
	}
	else if (ip->Type == ITEM_HELPER+82)
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2715]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum++] = false;
		sprintf(TextList[TextNum], "%s", GlobalText[2730]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum++] = false;
		sprintf(TextList[TextNum], "%s", GlobalText[2716]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum++] = false;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2551]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if(ip->Type == ITEM_POTION+92)
	{
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2553]);
		TextListColor[TextNum] = TEXT_COLOR_YELLOW;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if(ip->Type == ITEM_POTION+93)
	{
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2556]);
		TextListColor[TextNum] = TEXT_COLOR_YELLOW;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if(ip->Type == ITEM_POTION+95)
	{
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2552]);
		TextListColor[TextNum] = TEXT_COLOR_YELLOW;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if( ip->Type == ITEM_POTION+94 )
	{
		...
		sprintf(TextList[TextNum], "%s", GlobalText[2559]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2011] );
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2549] );
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2550]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		std::string timetext;
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2532]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	...
	else if( ip->Type == ITEM_POTION+88 )
	{
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2534]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2535]);
	...
	else if( ip->Type == ITEM_POTION+89 )
	{
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2536]);
		TextListColor[TextNum] = TEXT_COLOR_DARKYELLOW;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if( ip->Type == ITEM_POTION+90 )
	{
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2537]);
		TextListColor[TextNum] = TEXT_COLOR_DARKYELLOW;
		TextBold[TextNum] = false;
		TextNum++;
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2397]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2398]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2399]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2420]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}	
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[1665]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2709]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
        ...
        case 1:
		    {
			    ...
			    TextBold[TextNum] = false;
			    TextNum++;
			    sprintf(TextList[TextNum],"%s", GlobalText[733]);
			    TextListColor[TextNum] = TEXT_COLOR_RED;
			    TextBold[TextNum] = false;
			    TextNum++;
		    }
	...
	{
		const ITEM_ADD_OPTION& Item_data = g_pItemAddOptioninfo->GetItemAddOtioninfo(ip->Type);
		sprintf(TextList[TextNum],"%s", GlobalText[2717]);
		TextBold[TextNum] = false;
		TextNum++;
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2773]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2774]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2775]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum],"\n"); TextNum++; SkipNum++;
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2777]);
		TextListColor[TextNum] = TEXT_COLOR_DARKBLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2778]);
		TextListColor[TextNum] = TEXT_COLOR_DARKBLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if( ITEM_POTION+101 <= ip->Type && ip->Type <= ITEM_POTION+109 )
	{
		...
		case ITEM_POTION+101:
			{
				...
				sprintf(TextList[TextNum], "%s", GlobalText[2788]);
				TextListColor[TextNum] = TEXT_COLOR_WHITE;
				TextBold[TextNum] = false;
				TextNum++;
			}break;
		case ITEM_POTION+102:
			{
				sprintf(TextList[TextNum], "%s", GlobalText[2784]);
				TextListColor[TextNum] = TEXT_COLOR_WHITE;
				TextBold[TextNum] = false;
				TextNum++;
				sprintf(TextList[TextNum], "%s", GlobalText[2785]);
				TextListColor[TextNum] = TEXT_COLOR_WHITE;
				TextBold[TextNum] = false;
				TextNum++;
				sprintf(TextList[TextNum], "%s", GlobalText[2786]);
				TextListColor[TextNum] = TEXT_COLOR_WHITE;
				TextBold[TextNum] = false;
				TextNum++;
			}
		...
		case ITEM_POTION+108: 
			{
				sprintf(TextList[TextNum], "%s", GlobalText[2790]);
				TextListColor[TextNum] = TEXT_COLOR_WHITE;
				TextBold[TextNum] = false;
				TextNum++;
			}break;
		case ITEM_POTION+109:
			{
				sprintf(TextList[TextNum], "%s", GlobalText[2792]);
				TextListColor[TextNum] = TEXT_COLOR_WHITE;
				TextBold[TextNum] = false;
				TextNum++;
				sprintf(TextList[TextNum], "%s", GlobalText[2793]);
				TextListColor[TextNum] = TEXT_COLOR_WHITE;
				TextBold[TextNum] = false;
				TextNum++;
			}break;
	...
	else if( ITEM_HELPER+115 == ip->Type )
	{
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[635]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[2876] ); 
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[2875] ); 
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[2971] ); 
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[2877] ); 
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[2878] );
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[2879] ); 
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[2880] ); 
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[2972] ); 
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[3018] ); 
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[3116]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[3127]);
		TextListColor[TextNum] = TEXT_COLOR_YELLOW;
		TextBold[TextNum] = true;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[571]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if(ip->Type==ITEM_POTION+133)
	{
		...
		sprintf(TextList[TextNum], "%s", GlobalText[3267]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum],"%s", GlobalText[638]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum],"%s", GlobalText[639]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	if(ip->Type >= ITEM_POTION+45 && ip->Type <= ITEM_POTION+50) //Halloween event
	{
		...
		case ITEM_POTION+46:
			...
			sprintf(TextList[TextNum], "%s",Text_data);
			TextListColor[TextNum] = TEXT_COLOR_DARKYELLOW;
			break;
		case ITEM_POTION+47:
	...
	if(ip->Type==ITEM_POTION+11)
	{
		if( Level==7 )
		{
            sprintf(TextList[TextNum],"%s", GlobalText[112]);
		    TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
            sprintf(TextList[TextNum],"%s", GlobalText[113]);  
		    TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
            sprintf(TextList[TextNum],"%s", GlobalText[114]);  
		    TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
		}
        else if ( Level==14 )
        {
            sprintf(TextList[TextNum],"%s", GlobalText[1652]);
		    TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
            sprintf(TextList[TextNum],"%s", GlobalText[1653]);
		    TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
        }
        else
        {
	...
	if(tCount > 0)
	{
		...
		if( nJewelIndex != COMGEM::NOGEM )
		{
			...
			TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
			sprintf(TextList[TextNum], "%s", GlobalText[1820]);
			TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
		}
	}
	if(ip->Type==ITEM_POTION+13)
	{
		sprintf(TextList[TextNum],"%s", GlobalText[572]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	if(ip->Type==ITEM_POTION+14)
	{
		sprintf(TextList[TextNum],"%s", GlobalText[573]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	if(ip->Type==ITEM_POTION+16)
	{
		sprintf(TextList[TextNum],"%s", GlobalText[621]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	if(ip->Type==ITEM_POTION+17 || ip->Type==ITEM_POTION+18)
	{
		sprintf(TextList[TextNum],"%s", GlobalText[637]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum],"%s", GlobalText[574]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	if(ip->Type==ITEM_POTION+22)
	{
		sprintf(TextList[TextNum],"%s", GlobalText[619]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	if(ip->Type==ITEM_POTION+31)
	{
		sprintf(TextList[TextNum],"%s", GlobalText[1289]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum],"%s", GlobalText[576]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	if((ip->Type>=ITEM_WING && ip->Type<=ITEM_WING+2) || ip->Type==ITEM_WING+41)
	{
		...
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
		sprintf(TextList[TextNum],"%s", GlobalText[579]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	else if( ip->Type == ITEM_HELPER + 38 )
	{
		sprintf(TextList[TextNum],"%s", GlobalText[2207]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	else if( ip->Type == ITEM_POTION + 41 )
	{
		sprintf(TextList[TextNum],"%s", GlobalText[2208]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	else if( ip->Type == ITEM_POTION + 42 )
	{
		sprintf(TextList[TextNum],"%s", GlobalText[2209]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	else if( ip->Type == ITEM_POTION + 43 )
	{
		sprintf(TextList[TextNum],"%s", GlobalText[2210]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	...
	{
		// ¿¬ÀåÀÇ º¸¼®
		sprintf(TextList[TextNum],"%s", GlobalText[3305]);
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
	}
	...
	else if ((ip->Type>=ITEM_WING+3 && ip->Type<=ITEM_WING+6) || ip->Type==ITEM_WING+42) //³¯°³
    {
		...
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
		sprintf(TextList[TextNum],"%s", GlobalText[579]);             //  ÀÌµ¿ ¼Óµµ Çâ»ó.
		TextListColor[TextNum] = TEXT_COLOR_WHITE;TextBold[TextNum] = false;TextNum++;
    }
    ...
    else if ( ip->Type==ITEM_HELPER+31 )
    {
        ...
        switch ( Level )
        {
        case 0: sprintf ( TextList[TextNum], "%s", GlobalText[1215] ); TextNum++; break;
        case 1: sprintf ( TextList[TextNum], "%s", GlobalText[1216] ); TextNum++; break;
        }
    }
    else if ( ip->Type==ITEM_HELPER+14 )
    {
    ...
    {
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
        sprintf ( TextList[TextNum], "%s", GlobalText[816] );
        TextNum++;
    }
    ...
    {
		TextListColor[TextNum] = TEXT_COLOR_WHITE;
        sprintf ( TextList[TextNum], "%s", GlobalText[814] );
        TextNum++;
        sprintf ( TextList[TextNum], "\n" );
        TextNum++; SkipNum++;
		sprintf(TextList[TextNum],"%s", GlobalText[638]);
    ...
    else if ( ip->Type==ITEM_POTION+7 )
    {
        ...
        case 0:
            {
                sprintf( TextList[TextNum], "%s", GlobalText[1417] ); TextListColor[TextNum] = TEXT_COLOR_DARKRED; TextBold[TextNum] = false; TextNum++;
                sprintf( TextList[TextNum], "%s", GlobalText[1418] ); TextListColor[TextNum] = TEXT_COLOR_DARKRED; TextBold[TextNum] = false; TextNum++;
                sprintf( TextList[TextNum], "%s", GlobalText[1419] ); TextListColor[TextNum] = TEXT_COLOR_DARKRED; TextBold[TextNum] = false; TextNum++;
            }
            break;
        case 1:
            {
                sprintf( TextList[TextNum], "%s", GlobalText[1638] ); TextListColor[TextNum] = TEXT_COLOR_DARKRED; TextBold[TextNum] = false; TextNum++;
                sprintf( TextList[TextNum], "%s", GlobalText[1639] ); TextListColor[TextNum] = TEXT_COLOR_DARKRED; TextBold[TextNum] = false; TextNum++;
                sprintf( TextList[TextNum], "%s", GlobalText[1472] ); TextListColor[TextNum] = TEXT_COLOR_DARKRED; TextBold[TextNum] = false; TextNum++;
            }
            break;
        }
    }
    ...
    else if ( ip->Type==ITEM_HELPER+29 )
    {
		...
		TextNum++; 
		SkipNum++;
    	sprintf ( TextList[TextNum], "%s", GlobalText[1157] );  
		TextListColor[TextNum] = TEXT_COLOR_DARKBLUE; 
		TextNum++;
    }
    else if ( ip->Type==ITEM_POTION+21 )
    {
        ...
        switch ( Level )
        {
        case 1: sprintf ( TextList[TextNum], "%s", GlobalText[813] ); break;
        case 2: sprintf ( TextList[TextNum], "%s", GlobalText[1099] ); break;
        case 3: sprintf ( TextList[TextNum], "%s", GlobalText[1291] ); break;
		default: break;
        }
        TextNum++;
    }
      ...
      )
	{
		...
		else if(ip->Type >= ITEM_HELPER+125 && ip->Type <= ITEM_HELPER+127)
		{
			...
			{
				TextNum++;
				sprintf(TextList[TextNum], "%s", GlobalText[3105]);
			
			}
			else if (ip->Type == ITEM_HELPER+127)
			{
				TextNum++;
				sprintf(TextList[TextNum], "%s", GlobalText[3106]);
			}
			Success = true;
		}
		else if( ip->Type == ITEM_POTION+53 )
		...
		else if(ip->Type == ITEM_HELPER+70 )
		{
			if (ip->Durability == 2)
			{
				sprintf(TextList[TextNum], "%s", GlobalText[2605]);
				TextListColor[TextNum] = TEXT_COLOR_BLUE;
				TextBold[TextNum] = false;
				TextNum++;
			}
			...
			{
				sprintf(TextList[TextNum], "%s", GlobalText[2606]);
				TextListColor[TextNum] = TEXT_COLOR_BLUE;
				TextBold[TextNum] = false;
				TextNum++;
		...
		{
			sprintf(TextList[TextNum], "%s", GlobalText[2732+(ip->Type-(MODEL_TYPE_CHARM_MIXWING+EWS_BEGIN))]);
	...
	if(ip->Type>=MODEL_BOOTS-MODEL_ITEM && ip->Type<MODEL_BOOTS+MAX_ITEM_INDEX-MODEL_ITEM)
	{
		...
		{
			sprintf(TextList[TextNum],"\n");TextNum++;SkipNum++;
			sprintf(TextList[TextNum],"%s", GlobalText[78]);
	...
	if(ip->Type>=MODEL_GLOVES-MODEL_ITEM && ip->Type<MODEL_GLOVES+MAX_ITEM_INDEX-MODEL_ITEM)
	{
		if(Level >= 5)
		{
			...
			sprintf(TextList[TextNum],"%s", GlobalText[93]);
			TextListColor[TextNum] = TEXT_COLOR_BLUE;TextBold[TextNum] = true;TextNum++;
		}
	}
	...
	if( ip->Jewel_Of_Harmony_Option != 0 )
	{
		...
		if( type < SI_None )
		{
			...
			else
			{
				...
				sprintf( TextList[TextNum], "%s", GlobalText[2205] );
	...
	for(int i=0;i<ip->SpecialNum;i++)
	{
        ...
        {
			sprintf(TextList[TextNum], "%s", GlobalText[179] );
        ...
        {
			sprintf(TextList[TextNum], "%s", GlobalText[1201] );
     		TextListColor[TextNum] = TEXT_COLOR_DARKRED;TextBold[TextNum] = false;TextNum++;
        }
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[1917]);
		TextListColor[TextNum] = TEXT_COLOR_YELLOW; 
		TextNum++;
	}
	else if(ip->Type == ITEM_HELPER+34 || ip->Type == ITEM_HELPER+35)
	{
		sprintf(TextList[TextNum], "%s", GlobalText[1918]);
		TextListColor[TextNum] = TEXT_COLOR_YELLOW; 
		TextNum++;
	}
	else if(ip->Type == ITEM_HELPER+36)
	{
		sprintf(TextList[TextNum], "%s", GlobalText[1919]);
		TextListColor[TextNum] = TEXT_COLOR_YELLOW; 
		TextNum++;
	}
	else if(ip->Type == ITEM_HELPER+37)
	{
		...
		sprintf(TextList[TextNum], "%s", GlobalText[1920]);
		TextListColor[TextNum] = TEXT_COLOR_YELLOW; 
		TextNum++;
		
		if(ip->Option1 == 0x00)
		{
			sprintf(TextList[TextNum], "%s", GlobalText[1929]);
			TextListColor[TextNum] = TEXT_COLOR_YELLOW; 
			TextNum++;
		}
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[3088]);
		TextListColor[TextNum] = TEXT_COLOR_RED;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	{
		TextListColor[TextNum] = TEXT_COLOR_WHITE; 
		sprintf(TextList[TextNum], "%s", GlobalText[2244]); TextNum++;
	}
	else if (ip->Type == ITEM_POTION+52)
	{
		TextListColor[TextNum] = TEXT_COLOR_WHITE; 
		sprintf(TextList[TextNum], "%s", GlobalText[2323]); TextNum++;
		TextListColor[TextNum] = TEXT_COLOR_WHITE; 
		sprintf(TextList[TextNum], "%s", GlobalText[2011]); TextNum++;
	}
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[2743]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2744]);
		...
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[2745]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum],"\n"); TextNum++; SkipNum++;
	...
	{
		sprintf(TextList[TextNum], "%s", GlobalText[3065]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[3066]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum],"\n"); TextNum++; SkipNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[3067]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum], "%s", GlobalText[3072]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
		sprintf(TextList[TextNum],"\n"); TextNum++; SkipNum++;
	...
	else if( ip->Type == ITEM_HELPER+123 )
	{
		...
		sprintf(TextList[TextNum], "%s", GlobalText[3071]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	...
	else if(g_pMyInventory->IsInvenItem(ip->Type))
	{
		...
		{
			sprintf(TextList[TextNum], "%s", GlobalText[3143]);
			TextListColor[TextNum] = TEXT_COLOR_BLUE;
			TextBold[TextNum] = false;
			TextNum++;
			sprintf(TextList[TextNum],"\n"); TextNum++; SkipNum++;
		...
		switch (ip->Type)
		{
			...
			case ITEM_HELPER+129:
				sprintf(TextList[TextNum], "%s", GlobalText[3121]);
				break;
			case ITEM_HELPER+134:
				sprintf(TextList[TextNum], "%s", GlobalText[3123]);
				break;
			...
			case ITEM_HELPER+133:
				sprintf(TextList[TextNum], "%s", GlobalText[3122]);
				break;
		...
		sprintf(TextList[TextNum], "%s", GlobalText[3124]);
		TextListColor[TextNum] = TEXT_COLOR_BLUE;
		TextBold[TextNum] = false;
		TextNum++;
	}
	...
	if( ip->bPeriodItem == true )
	{
		...
		{
			sprintf(TextList[TextNum], "%s", GlobalText[3266]);
			TextListColor[TextNum] = TEXT_COLOR_RED; 	
		}
		else
		{
			sprintf(TextList[TextNum], "%s", GlobalText[3265]);
...
void RenderSkillInfo(int sx,int sy,int Type,int SkillNum, int iRenderPoint /*= STRP_NONE*/)
{
		...
		)
    {
		...
		case AT_SKILL_ASHAKE_UP+4:
        case AT_SKILL_DARK_HORSE:
		    sprintf ( TextList[TextNum], "%s", GlobalText[1237] );
		    TextListColor[TextNum] = TEXT_COLOR_DARKRED;TextBold[TextNum] = false;TextNum++;
            break;
    ...
    if ( gCharacterManager.GetBaseClass(Hero->Class) == CLASS_KNIGHT )
    {
        ...
        {
            sprintf(TextList[TextNum],"%s", GlobalText[96] );
		    TextListColor[TextNum] = TEXT_COLOR_DARKRED;TextBold[TextNum] = false;TextNum++;
        }
		...
		if ( Hero->byExtensionSkill == 1 && CharacterAttribute->Level >= 220 )
		{
			...
			{
				sprintf(TextList[TextNum], "%s", GlobalText[99]);
				TextListColor[TextNum] = TEXT_COLOR_DARKRED;
				TextBold[TextNum] = false;
				TextNum++;
			}
			...
			{
				sprintf(TextList[TextNum], "%s", GlobalText[2115] );
				TextListColor[TextNum] = TEXT_COLOR_DARKRED;
				TextBold[TextNum] = false;
				TextNum++;
			}
    ...
    if ( MasteryType!=255 )
    {
        sprintf ( TextList[TextNum], "%s", GlobalText[1080+MasteryType] );
		TextListColor[TextNum] = TEXT_COLOR_BLUE;TextBold[TextNum] = false;TextNum++;
    }
    ...
    {
        sprintf ( TextList[TextNum], "%s", GlobalText[1482] );
		TextListColor[TextNum] = TEXT_COLOR_DARKRED;TextBold[TextNum] = false;TextNum++;
    ...
    if ( gCharacterManager.GetBaseClass(Hero->Class)==CLASS_DARK_LORD )
    {
        ...
        {
		    sprintf ( TextList[TextNum], "%s", GlobalText[1185] );
		    TextListColor[TextNum] = TEXT_COLOR_DARKRED;TextBold[TextNum] = false;TextNum++;
        }
	}
	...
	if(CharacterAttribute->Skill[Type] == AT_SKILL_PLASMA_STORM_FENRIR)
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[1926] );
		TextListColor[TextNum] = TEXT_COLOR_DARKRED;TextBold[TextNum] = false;TextNum++;
		sprintf ( TextList[TextNum], "%s", GlobalText[1927] );
		TextListColor[TextNum] = TEXT_COLOR_DARKRED;TextBold[TextNum] = false;TextNum++;	
	}

	if(CharacterAttribute->Skill[Type] == AT_SKILL_INFINITY_ARROW)
	{
		sprintf(TextList[1], "%s",lpszName);
		TextListColor[1] = TEXT_COLOR_BLUE;TextBold[1] = true;
		sprintf(TextList[2],"\n");
		sprintf(TextList[3],"%s", GlobalText[2040]);
	...
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[2047] );
		TextListColor[TextNum] = TEXT_COLOR_DARKRED;TextBold[TextNum] = false;TextNum++;
	}
	...
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[2048] );
		TextListColor[TextNum] = TEXT_COLOR_DARKRED;TextBold[TextNum] = false;TextNum++;
	}
	if(CharacterAttribute->Skill[Type] == AT_SKILL_SPEAR)
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[2049] );
		TextListColor[TextNum] = TEXT_COLOR_DARKRED;TextBold[TextNum] = false;TextNum++;
	}

	if( SkillType == AT_SKILL_SWELL_OF_MAGICPOWER )
	{
		sprintf ( TextList[TextNum], "%s", GlobalText[2054] );
		TextListColor[TextNum] = TEXT_COLOR_BLUE; TextBold[TextNum] = false; TextNum++;
	}
...
void RenderItemName(int i,OBJECT *o,int ItemLevel,int ItemOption,int ItemExtOption,bool Sort)
{
	...
	{
		SetLegacyColor3f(0.7f,0.7f,0.7f);
		sprintf( Name,"%s %s", SkillAttribute[ 30 + Level ].Name, GlobalText[102] );
	}
	else if(o->Type == MODEL_HELPER+46)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+46].Name);	
	}
	else if(o->Type == MODEL_HELPER+47)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+47].Name);
	}
	else if(o->Type == MODEL_HELPER+48)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+48].Name);
	}
	else if(o->Type == MODEL_POTION+54)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+54].Name);
	}
	...
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+125].Name);	
	}
	else if(o->Type == MODEL_HELPER+126)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+126].Name);
	}
	else if(o->Type == MODEL_HELPER+127)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+127].Name);
	}
	else if( o->Type == MODEL_POTION+53 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+53].Name);
	}
	else if( o->Type == MODEL_HELPER+43 || o->Type == MODEL_HELPER+44 || o->Type == MODEL_HELPER+45 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[o->Type - MODEL_HELPER].Name);
	}
	...
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+91].Name);
	}
	else if(o->Type == MODEL_POTION+92)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+92].Name);
	}
	else if(o->Type == MODEL_POTION+93)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+93].Name);
	}
	else if(o->Type == MODEL_POTION+95)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+95].Name);
	}
	else if( o->Type == MODEL_POTION + 94 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+94].Name);
	}
	else if( o->Type==MODEL_POTION+84 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+84].Name);
	}
	else if( o->Type==MODEL_POTION+85 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+85].Name);
	}
	else if( o->Type==MODEL_POTION+86 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+86].Name);
	}
	else if( o->Type==MODEL_POTION+87 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+87].Name);
	}
	else if( o->Type==MODEL_POTION+88 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+88].Name);
	}
	else if( o->Type==MODEL_POTION+89 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+89].Name);
	}
	else if( o->Type==MODEL_POTION+90 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+90].Name);
	}
	else if( o->Type == MODEL_HELPER+62 ) {
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+62].Name);
	}
	else if( o->Type == MODEL_HELPER+63 ) {
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+63].Name);
	}
	else if( o->Type == MODEL_POTION+97 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+97].Name);
	}
	else if( o->Type == MODEL_POTION+98 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+98].Name);
	}
	else if( o->Type == MODEL_POTION+96 ) 
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+96].Name);
	}
	else if( o->Type == MODEL_HELPER+64 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+64].Name);
	}
	else if( o->Type == MODEL_HELPER+65 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+65].Name);
	}
	else if( o->Type == MODEL_HELPER+67 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+67].Name);
	}
	else if( o->Type == MODEL_HELPER+80 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+80].Name);
	}
	else if( o->Type == MODEL_HELPER+106 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+106].Name);
	}
	else if( o->Type == MODEL_HELPER+123 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+123].Name);
	}
	else if( o->Type == MODEL_HELPER+68 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+68].Name);
	}
	else if( o->Type == MODEL_HELPER+76 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+76].Name);
	}
	else if( o->Type == MODEL_HELPER+122 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+122].Name);
	}
#ifdef LJH_ADD_ITEMS_EQUIPPED_FROM_INVENTORY_SYSTEM
	...
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+128].Name);
	}
	else if( o->Type == MODEL_HELPER+129 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+129].Name);
	}
	else if( o->Type == MODEL_HELPER+134 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+134].Name);
	}
#endif //LJH_ADD_ITEMS_EQUIPPED_FROM_INVENTORY_SYSTEM
#ifdef LJH_ADD_ITEMS_EQUIPPED_FROM_INVENTORY_SYSTEM_PART_2
	...
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+130].Name);
	}
	else if( o->Type == MODEL_HELPER+131 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+131].Name);
	}
	else if( o->Type == MODEL_HELPER+132 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+132].Name);
	}
	else if( o->Type == MODEL_HELPER+133 )
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+133].Name);
	}
#endif //LJH_ADD_ITEMS_EQUIPPED_FROM_INVENTORY_SYSTEM_PART_2
    ...
    {
		SetLegacyColor3f(1.f,0.8f,0.1f);
        sprintf (Name, "%s", GlobalText[111]);
    }
	else if(o->Type == MODEL_POTION+12)
	{
		switch(Level)
		{
		case 0:sprintf(Name,"%s", GlobalText[100]);break;
		case 1:sprintf(Name,"%s", GlobalText[101]);break;
		case 2:sprintf(Name,"%s", GlobalText[104]);break;
		}
	}
	...
	{
		SetLegacyColor3f(1.f,0.8f,0.1f);
		sprintf ( Name, "%s", GlobalText[1235] );
	}
	else if(o->Type == MODEL_EVENT+4)
	{
		sprintf(Name,"%s", GlobalText[105]);
	}
	else if(o->Type == MODEL_EVENT+5)
	{
        ...
        {
        case 14:
	    	sprintf(Name,"%s", GlobalText[1650]);
            break;

        case 15:
	    	sprintf(Name,"%s", GlobalText[1651]);
            break;

        default:
		    sprintf(Name,"%s", GlobalText[106]);
            break;
        }
	}
	else if(o->Type == MODEL_EVENT+6)
	{
		...
		{
			sprintf(Name,"%s", GlobalText[107]);
		}
	}
	else if(o->Type == MODEL_EVENT+7)
	{
		sprintf(Name,"%s", GlobalText[108]);
	}
	else if(o->Type == MODEL_EVENT+8)
	{
		sprintf(Name,"%s", GlobalText[109]);
	}
	else if(o->Type == MODEL_EVENT+9)
	{
		sprintf(Name,"%s", GlobalText[110]);
	}
	...
	{
		SetLegacyColor3f( 1.f, 0.3f, 0.3f );
		sprintf(Name, "%s", ItemAttribute[ITEM_WING+32].Name);	
	}
	else if(o->Type == MODEL_WING+33)
	{
		SetLegacyColor3f( 0.3f, 1.0f, 0.3f );
		sprintf(Name, "%s", ItemAttribute[ITEM_WING+33].Name);	
	}
	else if(o->Type == MODEL_WING+34)
	{
		SetLegacyColor3f( 0.3f, 0.3f, 1.f );
		sprintf(Name, "%s", ItemAttribute[ITEM_WING+34].Name);	
	}
	else if(o->Type == ITEM_HELPER+49)
	{
		SetLegacyColor3f( 1.f, 0.8f, 0.1f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+49].Name);
	}
	else if(o->Type == ITEM_HELPER+50)
	{
		SetLegacyColor3f( 1.f, 0.8f, 0.1f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+50].Name);
	}
	else if(o->Type == ITEM_HELPER+51)
	{
		SetLegacyColor3f( 1.f, 0.8f, 0.1f );
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+51].Name);
	}
	else if(o->Type == MODEL_POTION+64)
	{
		SetLegacyColor3f( 1.f, 0.8f, 0.1f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+64].Name);
	}
	else if(o->Type == MODEL_WING+48)//
	{
		SetLegacyColor3f( 1.f, 1.f, 1.f );
		sprintf(Name, "%s", ItemAttribute[ITEM_WING+48].Name);	
	}
	else if(o->Type == MODEL_WING+35)
	{
		SetLegacyColor3f( 1.f, 1.f, 1.f );
		sprintf(Name, "%s", ItemAttribute[ITEM_WING+35].Name);	
	}
	else if(o->Type == MODEL_POTION+45)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+45].Name);	
	}
	else if(o->Type == MODEL_POTION+46)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+46].Name);	
	}
	else if(o->Type == MODEL_POTION+47)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+47].Name);	
	}
	else if(o->Type == MODEL_POTION+48)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+48].Name);		
	}
	else if(o->Type == MODEL_POTION+49)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+49].Name);	
	}
	else if(o->Type == MODEL_POTION+50)
	{
		SetLegacyColor3f( 0.9f, 0.53f, 0.13f );
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+50].Name);		
	}
	else if(o->Type == MODEL_POTION+32)
	{
		...
		if(Level == 0)
		{
			SetLegacyColor3f( 1.f, 0.3f, 1.f );
			sprintf(Name, "%s", ItemAttribute[ITEM_POTION+32].Name);	
		}
		else
		if(Level == 1)
		{
			SetLegacyColor3f( 1.f, 0.3f, 1.f );
			sprintf(Name, "%s", GlobalText[2012]);	
		}
	}
	else if(o->Type == MODEL_POTION+33)
	{
		if(Level == 0)
		{
			SetLegacyColor3f( 1.0f, 0.3f, 0.3f );
			sprintf(Name, "%s", ItemAttribute[ITEM_POTION+33].Name);	
		}
		else
		if(Level == 1)
		{
			SetLegacyColor3f( 1.0f, 0.3f, 0.3f );
			sprintf(Name, "%s", GlobalText[2013]);	
		}
	}
	else if(o->Type == MODEL_POTION+34)
	{
		...
		{
			SetLegacyColor3f( 0.3f, 0.3f, 1.f );
			sprintf(Name, "%s", ItemAttribute[ITEM_POTION+34].Name);	
		}
		else
		if(Level == 1)
		{
			SetLegacyColor3f( 0.3f, 0.3f, 1.f );
			sprintf(Name, "%s", GlobalText[2014]);	
		}
	}
    ...
    else if ( o->Type==MODEL_EVENT+11 )
    {
        SetLegacyColor3f( 1.f, 0.8f, 0.1f );
        {
            sprintf ( Name, "%s", GlobalText[810] );
        }
    }
	else if(o->Type==MODEL_EVENT+12)
	{
		SetLegacyColor3f( 1.f, 0.8f, 0.1f );
		sprintf ( Name, "%s", GlobalText[906] );
	}
	else if(o->Type==MODEL_EVENT+13)
	{
		SetLegacyColor3f( 1.f, 0.8f, 0.1f );
		sprintf ( Name, "%s", GlobalText[907] );
	}
	else if(o->Type==MODEL_EVENT+14)
	{
        ...
        {
        case 2:
		    sprintf( Name, "%s", GlobalText[928] );
            break;
        case 3:
		    sprintf( Name, "%s", GlobalText[929] );
            break;
        default :
		    sprintf( Name, "%s", GlobalText[922] );
            break;
        }
	}
    ...
    {
	    SetLegacyColor3f( 1.f, 0.8f, 0.1f );
		sprintf( Name, "%s", GlobalText[1290] );
    }
    else if ( o->Type==MODEL_POTION+7 )
    {
        switch ( Level )
        {
        case 0: sprintf( Name, "%s", GlobalText[1413] ); break;
        case 1: sprintf( Name, "%s", GlobalText[1414] ); break;
        }
    }
    else if ( o->Type==MODEL_HELPER+7 )
    {
        switch ( Level )
        {
        case 0: sprintf( Name, "%s", GlobalText[1460] ); break;
        case 1: sprintf( Name, "%s", GlobalText[1461] ); break;
        }
    }
    else if ( o->Type==MODEL_HELPER+11 )
    {
        switch ( Level )
        {
        case 0: sprintf( Name, "%s", GlobalText[1416] ); break;
        case 1: sprintf( Name, "%s", GlobalText[1462] ); break;
        }
    }
    else if ( o->Type==MODEL_EVENT+18 )
    {
        sprintf( Name, "%s", GlobalText[1462] );
    }
	else
	{
		bFirstOK = false;
	...
	{
		SetLegacyColor3f(0.7f,0.4f,1.0f);	// TEXT_COLOR_VIOLET
		strcpy ( Name, ItemAttribute[o->Type-MODEL_ITEM].Name );
	}
	else if( o->Type == MODEL_HELPER+66 )
	{
		SetLegacyColor3f( 0.6f, 0.4f, 1.0f);
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+66].Name);
	}
	else if( o->Type == MODEL_POTION+100 )
	{
		SetLegacyColor3f(1.0f, 0.8f, 0.1f);
		sprintf(Name, "%s", ItemAttribute[ITEM_POTION+100].Name);
	}
	...
	{
		SetLegacyColor3f(1.0f, 0.8f, 0.1f);
		sprintf(Name, "%s", ItemAttribute[o->Type].Name);
	}
	else if(o->Type == MODEL_HELPER+99)
	{
		SetLegacyColor3f(1.0f, 0.8f, 0.1f);
		sprintf(Name, "%s", ItemAttribute[ITEM_HELPER+99].Name);
	}
	else if(o->Type == MODEL_HELPER+109)
	{
		SetLegacyColor3f(1.0f, 0.8f, 0.1f);
		sprintf(Name, "%s", ItemAttribute[MODEL_HELPER+109].Name);
	}
	else if(o->Type == MODEL_HELPER+110)
	{
		SetLegacyColor3f(1.0f, 0.8f, 0.1f);
		sprintf(Name, "%s", ItemAttribute[MODEL_HELPER+110].Name);
	}
	else if(o->Type == MODEL_HELPER+111)
	{
		SetLegacyColor3f(1.0f, 0.8f, 0.1f);
		sprintf(Name, "%s", ItemAttribute[MODEL_HELPER+111].Name);
	}
	else if(o->Type == MODEL_HELPER+112)
	{
		SetLegacyColor3f(1.0f, 0.8f, 0.1f);
		sprintf(Name, "%s", ItemAttribute[MODEL_HELPER+112].Name);
	}
	else if(o->Type == MODEL_HELPER+113)
	{
		SetLegacyColor3f(1.0f, 0.8f, 0.1f);
		sprintf(Name, "%s", ItemAttribute[MODEL_HELPER+113].Name);
	}
	else if(o->Type == MODEL_HELPER+114)
	{
		SetLegacyColor3f(1.0f, 0.8f, 0.1f);
		sprintf(Name, "%s", ItemAttribute[MODEL_HELPER+114].Name);
	}
	else if(o->Type == MODEL_HELPER+115)
	{
		SetLegacyColor3f(1.0f, 0.8f, 0.1f);
		sprintf(Name, "%s", ItemAttribute[MODEL_HELPER+115].Name);
	}
	else if(o->Type == MODEL_POTION+112)
	{
		SetLegacyColor3f(1.0f, 0.8f, 0.1f);
		sprintf(Name, "%s", ItemAttribute[MODEL_POTION+112].Name);
	}
	else if(o->Type == MODEL_POTION+113)
	{
		SetLegacyColor3f(1.0f, 0.8f, 0.1f);
		sprintf(Name, "%s", ItemAttribute[MODEL_POTION+113].Name);
	}
	else
	{
		...
		{
			SetLegacyColor3f(0.4f,0.7f,1.f);
		}
		else if(Level == 0)
		{
			SetLegacyColor3f(0.7f,0.7f,0.7f);
		...
		{
			SetLegacyColor3f(0.9f,0.9f,0.9f);
		}
		else if(Level < 5)
		{
			SetLegacyColor3f(1.f,0.5f,0.2f);
		}
		else if(Level < 7)
		{
			SetLegacyColor3f(0.4f,0.7f,1.f);
...
//		Position[0] += 0.02f;
	...
	// Screen-space item previews must not inherit the map terrain light.
...
void InventoryColor(ITEM *p)
{
	...
	case 0:
		...
		break;
	case 1:
		SetLegacyColor3f(0.8f,0.8f,0.8f);
		break;
	case 2:
		SetLegacyColor3f(0.6f,0.7f,1.f);
		break;
	case 3:
		SetLegacyColor3f(1.f,0.2f,0.1f);
		break;
	case 4:
		SetLegacyColor3f(0.5f,1.f,0.6f);
		break;
    case 5:
		SetLegacyColor4f(0.8f,0.7f,0.f,1.f);
        break;
    case 6:
		SetLegacyColor4f(0.8f,0.5f,0.f,1.f);
        break;
    case 7:
		SetLegacyColor4f(0.8f,0.3f,0.3f,1.f);
        break;
    case 8:
		SetLegacyColor4f(1.0f,0.f,0.f,1.f);
        break;
    case 99:
		SetLegacyColor3f(1.f,0.2f,0.1f);
        break;
...
void RenderServerDivision ()
{
    ...
    if ( g_bServerDivisionAccept )
        ...
        SetLegacyColor3f( 0.5f, 0.5f, 0.5f );
    RenderBitmap(BITMAP_INTERFACE+10,(float)x,(float)y,(float)Width,(float)Height,0.f,0.f,213.f/256.f);
