...
void CNewUIStorageInventory::SetPos(int x, int y)
{
	...
	if (m_pNewInventoryCtrl)
		m_pNewInventoryCtrl->SetPos(x + 15, y + 40);
	const int bottomX[MAX_BTN] = { 83, 113, 143 };
	for (int i = BTN_INSERT_ZEN; i < BTN_LEFT; ++i)
		m_abtn[i].SetPos(x + bottomX[i], y + 390);
	m_abtn[BTN_LEFT].SetPos(x + 20, y + 10);
	m_abtn[BTN_RIGHT].SetPos(x + 145, y + 10);
	m_abtn[BTN_CHANGE].SetPos(x + 23, y + 390);
	m_abtn[BTN_BUY].SetPos(x + 53, y + 390);
}
...
bool CNewUIStorageInventory::ProcessBtns()
{
	...
	else if (m_abtn[BTN_BUY].UpdateMouseEvent())
	{
		...
		//pMsgBox->AddMsg("Confirma compra de 1 ba\xFA adicional?", RGBA(255, 255, 255, 255), SEASON3B::MSGBOX_FONT_NORMAL);
		return true;
	}
