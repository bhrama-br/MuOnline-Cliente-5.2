...
void SEASON3B::CNewUIMyInventory::UnequipItem(int iIndex)
{
	if (gVisualInventory.InventoryVisualOpen == 1)
	{
		if (iIndex >= NEW_EQUIPMENT_WEAPON_RIGHT && iIndex <= NEW_EQUIPMENT_HELPER)
		{
			...
			if (iIndex == NEW_EQUIPMENT_HELPER)
			{
				g_ErrorReport.Write("[PetTrace] Unequip visual helper: type=%d key=%d.\r\n", pEquippedItem->Type, pEquippedItem->Key);
				if (gHelperSystem.CheckIsHelper(pEquippedItem->Type + MODEL_ITEM))
				{
					g_ErrorReport.Write("[PetTrace] Removing helper model.\r\n");
...
void SEASON3B::CNewUIMyInventory::Render3DItem()
{
	for (int i = NEW_EQUIPMENT_WEAPON_RIGHT; i < MAX_NEW_EQUIPMENT_INDEX; i++)
	{
		...
		if (pEquippedItem)
		{
			...
			{
				SetLegacyColor4f(1.0, 1.0, 1.0, 1.0);
				...
				if (pEquippedItem->Type >= 0)
				{
					...
					SetLegacyColor4f(1.f, 1.f, 1.f, 1.f);
					RenderItem3D(
...
void SEASON3B::CNewUIMyInventory::RenderEquippedItem()
{
	...
	if(CNewUIInventoryCtrl::GetPickedItem() && m_iPointedSlot != -1)
	{
		...
		{
			SetLegacyColor4f(0.9f, 0.1f, 0.1f, 0.4f);
			EnableAlphaTest();
...
bool SEASON3B::CNewUIMyInventory::EquipmentWindowProcess()
{
	...
	if (m_iPointedSlot != -1)
	{
		// The Web visual inventory needs a direct transfer for its helper slot.
		// The PC client keeps the original sourceOLD picked-item flow below.
#if defined(LEGACY_GLES_RENDERER)
		// A right-click on the visual helper must move the item directly.  The
		// generic drag path creates a temporary picked item while clearing the
		// same slot, which leaves custom pet state invalid.
		const int helperSlot = (gVisualInventory.InventoryVisualOpen == 1)
			? NEW_EQUIPMENT_HELPER
			: EQUIPMENT_HELPER;

		if (m_iPointedSlot == helperSlot
			&& IsRelease(VK_RBUTTON)
			&& GetRepairMode() != REPAIR_MODE_ON
			&& EquipmentItem == false
			&& CNewUIInventoryCtrl::GetPickedItem() == NULL)
		{
			...
			const int nDstIndex = FindEmptySlot(pEquippedItem);
			if (nDstIndex >= 0)
			{
				ITEM itemToMove = *pEquippedItem;
				g_ErrorReport.Write("[PetTrace] Direct helper move: slot=%d item=%d key=%d destination=%d\r\n", m_iPointedSlot, itemToMove.Type, itemToMove.Key, nDstIndex);
				ResetMouseRButton();
				UnequipItem(m_iPointedSlot);
				SendRequestEquipmentItem(REQUEST_EQUIPMENT_INVENTORY, m_iPointedSlot, &itemToMove, REQUEST_EQUIPMENT_INVENTORY, nDstIndex + 12);
				return true;
			}
		}
#endif
...
{
	g_ErrorReport.Write("[PetTrace] Unequip click: item=%d key=%d\r\n", pEquippedItem->Type, pEquippedItem->Key);
}
...
if (m_iPointedSlot == EQUIPMENT_HELPER)
{
	g_ErrorReport.Write("[PetTrace] Unequip local state removed\r\n");
}
										}
									}
								}
...
if (m_iPointedSlot == EQUIPMENT_HELPER)
{
											...
											CNewUIPickedItem* pActivePickedItem = CNewUIInventoryCtrl::GetPickedItem();
											if (pActivePickedItem != NULL)
												pActivePickedItem->HidePickedItem();
										}
										if (m_iPointedSlot == EQUIPMENT_HELPER)
{
	g_ErrorReport.Write("[PetTrace] Sending equipment change\r\n");
}
...
void SEASON3B::CNewUIMyInventory::LockMyShopButtonOpen()
{
	...
	m_BtnMyShop.Lock();
	// 1125 "\xB0\xB3\xC0\xCE\xBB\xF3\xC1\xA1\xBF\xAD\xB1\xE2(S)"
	m_BtnMyShop.ChangeToolTipText(GlobalText[1125], true);	
}
