...
bool WriteJpeg(char *filename,int Width,int Height,unsigned char *Buffer,int quality)
{
	...
	if ((outfile = Platform::LegacyFileOpen(filename, "wb")) == NULL)
	{
...
void SaveImage(int HeaderSize,char *Ext,char *filename,BYTE *PakBuffer,int Size)
{
	if(PakBuffer==NULL || Size==0)
	{
		...
		FILE *fp = Platform::LegacyFileOpen(OpenFileName,"rb");
		if(fp == NULL)
		{
			return;
		}
	...
	FILE *fp = Platform::LegacyFileOpen(SaveFileName,"wb");
	if(fp == NULL) return;
...
bool OpenJpegBuffer(char *filename,float *BufferFloat)
{
	...
	if((infile = Platform::LegacyFileOpen(FileName, "rb")) == NULL) 
	{
		char Text[256];
...
#endif // KJH_ADD_INGAMESHOP_UI_SYSTEM
{
	...
	if(bCheck)
	{
		...
		{
#if !defined(_WIN32)
			// Nome VAZIO nao derruba o cliente.
			//
			// `PopUpErrorCheckMsgBox(..., true)` encerra o processo (ExitProcess em
			// ZzzTexture.cpp:267) -- e a verificacao de integridade do cliente. Mas com
			// szFileName vazio o caminho fica so "Data\\", que nao pode carregar nunca:
			// nao ha integridade a verificar, e derrubar o jogo por isso e pior que
			// seguir. Foi o que matava o cliente 2 segundos depois de ENTRAR NO MAPA,
			// com o mapa e o HUD ja desenhados.
			//
			// O indice vai na mensagem porque e ele que identifica o subsistema dono da
			// chamada (as faixas BITMAP_*), e o nome vazio nao diz nada.
			if (szFileName == NULL || szFileName[0] == '\0')
			{
				...
				{
					avisado = true;
					fprintf(stderr, "[textura] LoadBitmap com nome VAZIO no indice %u; "
					                "ignorado em vez de encerrar o cliente\n",
			...
			sprintf(szErrorMsg, "LoadBitmap Failed: %s (indice %u)", szFullPath,
			        (unsigned int)uiTextureIndex);
#ifdef FOR_WORK
