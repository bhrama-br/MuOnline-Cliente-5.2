...
void CSBaseMatch::RenderTime ( void )
{
	...
	if ( m_iMatchCountDownType>TYPE_MATCH_NONE && m_iMatchCountDownType<TYPE_MATCH_END )
	{
		...
		else
		{
			...
			g_pRenderText->RenderText(GetCenterX(640) + 640 / 2, (int)y, lpszStr, 0, 0, RT3_WRITE_CENTER);
		}
	}
}
...
{
	// m_PosResult.x e LONG; a inicializacao por chaves proibe estreitamento.
	int xPos[6] = {(int)m_PosResult.x, };
	xPos[1] = xPos[0] + 15;
	xPos[2] = xPos[1] + 15;
	xPos[3] = xPos[2] + 60;
	xPos[4] = xPos[3] + 50;
