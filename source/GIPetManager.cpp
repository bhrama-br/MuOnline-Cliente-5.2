...
namespace giPetManager
{
	...
	bool RenderPetCmdInfo(int sx, int sy, int Type)
	{
		...
		if (gCharacterManager.GetBaseClass(Hero->Class) == CLASS_DARK_LORD || gDarkSpirit.checkIsDarkSpirit(elementFirst->Type))
		{
			...
			sprintf(TextList[TextNum], "%s", GlobalText[1219 + cmdType]); TextNum++; SkipNum++;
			...
			switch (cmdType)
			{
			case PET_CMD_DEFAULT: sprintf(TextList[TextNum], "%s", GlobalText[1223]); TextNum++; SkipNum++; break;
			case PET_CMD_RANDOM: sprintf(TextList[TextNum], "%s", GlobalText[1224]); TextNum++; SkipNum++; break;
			case PET_CMD_OWNER: sprintf(TextList[TextNum], "%s", GlobalText[1225]); TextNum++; SkipNum++; break;
			case PET_CMD_TARGET: sprintf(TextList[TextNum], "%s", GlobalText[1226]); TextNum++; SkipNum++; break;
			}
	...
	bool RenderPetItemInfo(int sx, int sy, ITEM* pItem, int iInvenType)
	{
		...
		if (pItem->Type == ITEM_HELPER + 4)
		{
			...
			sprintf(TextList[TextNum], "%s", GlobalText[1187]); TextNum++; SkipNum++;
		}
		else if (pItem->Type == ITEM_HELPER + 5)
		{
			RequireCharisma = (185 + (pPetInfo->m_wLevel * 15));

			sprintf(TextList[TextNum], "%s", GlobalText[1214]); TextNum++; SkipNum++;
		}
