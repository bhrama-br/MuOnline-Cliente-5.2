...
void CNewUIDoppelGangerWindow::SetPos(int x, int y)
{
	m_Pos.x = x;
	m_Pos.y = y;
	m_BtnEnter.ChangeButtonInfo(m_Pos.x + INVENTORY_WIDTH / 2 - 27, m_Pos.y + 190, 53, 23);
	m_BtnClose.ChangeButtonInfo(m_Pos.x + INVENTORY_WIDTH / 2 - 27, m_Pos.y + 360, 53, 23);
}
...
bool CNewUIDoppelGangerWindow::Render()
{
	...
	if (m_iRemainTime == 0)
	{
		sprintf(szText, "%s", GlobalText[2164]);
	}
	else
	{
