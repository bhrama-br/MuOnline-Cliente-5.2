...
void CSideHair::RenderLine( vec3_t v1, vec3_t v2, vec3_t c1, vec3_t c2)
{
	...
	renderer.BindTexture(Bitmaps[BITMAP_ROBE + 4].TextureNumber);
	renderer.Begin(Platform::LegacyPrimitiveQuads);
	//glColor3fv( c1);
	 renderer.TexCoord2f(0.f,0.f+fTextureMove+fTextureV); renderer.Vertex3f(p1[0]-vOrtho[0],p1[1]-vOrtho[1],p1[2]-vOrtho[2]);
	//glColor3fv( c2);
	 renderer.TexCoord2f(0.f,1.f-fTextureMove+fTextureV); renderer.Vertex3f(p2[0]-vOrtho[0],p2[1]-vOrtho[1],p2[2]-vOrtho[2]);
	 renderer.TexCoord2f(1.f,1.f-fTextureMove+fTextureV); renderer.Vertex3f(p2[0]+vOrtho[0],p2[1]+vOrtho[1],p2[2]+vOrtho[2]);
	//glColor3fv( c1);
	 renderer.TexCoord2f(1.f,0.f+fTextureMove+fTextureV); renderer.Vertex3f(p1[0]+vOrtho[0],p1[1]+vOrtho[1],p1[2]+vOrtho[2]);
	renderer.End();
