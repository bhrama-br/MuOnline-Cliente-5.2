	...
	if ( PCT_MASK_LIGHT&m_dwType )
    {
...
#ifdef RENDER_CLOTH
     {
		 ...
		 if (ret)
		 {
			 ...
			 Platform::GetLegacyRenderAdapter().Color4f(lights, lights, lights, 1.f);
...
void CPhysicsCloth::RenderVertex( vec3_t *pvRenderPos, int xVertex, int yVertex)
{
	...
	renderer.TexCoord2f( ( float)xVertex / ( float)( m_iNumHor - 1), min( 0.99f, ( float)yVertex / ( float)( m_iNumVer - 1)));
	renderer.Vertex3f( ( *pvPos)[0], ( *pvPos)[1], ( *pvPos)[2]);
}

void CPhysicsCloth::RenderCollisions( void)
{
#ifdef RENDER_COLLISION
	SetLegacyColor3f( 1.0f, 1.0f, 0.6f);
