...
namespace SEASON3B
{
	...
	public:
		...
		// Sobrecarga para temporario: os chamadores constroem TContainer<T>() na
		// propria chamada, e so o MSVC liga temporario a referencia nao-const.
		template <class T>
		T* NewMessageBox(TContainer<T>&& _container)
		{
	...
	public:
		...
		// Sobrecarga para temporario; encaminha para a versao de lvalue acima.
		template <class T>
		T* NewMessageBox(CNewUIMessageBoxFactory::TContainer<T>&& container)
		{
	...
	// MSGBOX_LAYOUT_CLASS(x) produz um temporario, e ligar temporario a
	// referencia nao-const so e aceito pelo MSVC. Estas sobrecargas recebem o
	// temporario e encaminham para as versoes de lvalue acima, sem alterar o
