...
// TERRAIN_INDEX, TERRAIN_INDEX_REPEAT e TERRAIN_ATTRIBUTE mudaram para
// ZzzLodTerrain.h -- ver o comentario lá. Definidas aqui, so este arquivo podia
// embuti-las, e chamar de outra unidade nao ligava fora do MSVC.
...
{
	FILE *fp = Platform::LegacyFileOpen(FileName,"rb");
	if(fp == NULL)
	{
		char Text[256];
...
{
	FILE *fp = Platform::LegacyFileOpen(FileName,"wb");
...
int OpenTerrainMapping(char *FileName)	//
{
	...
	// O fclose(fp) que existia aqui era o SEGUNDO sobre o mesmo descritor:
	// o arquivo ja e lido inteiro e fechado logo apos o fread, acima. No CRT
	// do Windows a segunda chamada so devolve EOF; no musl (Emscripten e
	// Android) o FILE ja foi liberado e fclose salta por um ponteiro de
	// funcao nulo. Mesmo defeito de OpenFilterFile e CGlobalBitmap::OpenJpeg.
...
*/
	...
	{
		fp = Platform::LegacyFileOpen(FileName,"rb");
		if(fp == NULL)
		{
			return ( false);
		}
		...
		fp = Platform::LegacyFileOpen(FileName,"wb");
...
void SaveTerrainHeight(char *name)
{
    ...
    FILE *fp = Platform::LegacyFileOpen(name,"wb");
...
bool OpenTerrainHeightNew(const char* strFilename)
{
	...
	FILE* fp = Platform::LegacyFileOpen(FileName, "rb");
	// Sem esta verificacao um asset ausente vira crash em vez de mensagem — o
	// carregador antigo (OpenTerrainHeight) ja tratava, este nao.
	if (fp == NULL)
	{
		char Text[256];
		sprintf(Text, "%s file not found.", FileName);
...
// Emissores de vertice do terreno migrados para o ILegacyRenderAdapter.
// glColor3f/glColor3fv definem alpha implicito 1.0, preservado em Color4f.
inline void TerrainVertexEmit(const float* textureCoord, const float* light, float alpha, const float* vertex)
{
	Platform::ILegacyRenderAdapter& renderer = Platform::GetLegacyRenderAdapter();
	renderer.TexCoord2f(textureCoord[0],textureCoord[1]);
	renderer.Color4f(light[0],light[1],light[2],alpha);
	renderer.Vertex3fv(vertex);
}

inline void Vertex0()
{
	TerrainVertexEmit(TerrainTextureCoord[0],PrimaryTerrainLight[TerrainIndex1],1.f,TerrainVertex[0]);
}

inline void Vertex1()
{
	TerrainVertexEmit(TerrainTextureCoord[1],PrimaryTerrainLight[TerrainIndex2],1.f,TerrainVertex[1]);
}

inline void Vertex2()
{
	TerrainVertexEmit(TerrainTextureCoord[2],PrimaryTerrainLight[TerrainIndex3],1.f,TerrainVertex[2]);
}

inline void Vertex3()
{
	TerrainVertexEmit(TerrainTextureCoord[3],PrimaryTerrainLight[TerrainIndex4],1.f,TerrainVertex[3]);
}

inline void Vertex01()
{
	TerrainVertexEmit(TerrainTextureCoord01,PrimaryTerrainLight[Index01],1.f,TerrainVertex01);
}

inline void Vertex12()
{
	TerrainVertexEmit(TerrainTextureCoord12,PrimaryTerrainLight[Index12],1.f,TerrainVertex12);
}

inline void Vertex23()
{
	TerrainVertexEmit(TerrainTextureCoord23,PrimaryTerrainLight[Index23],1.f,TerrainVertex23);
}

inline void Vertex30()
{
	TerrainVertexEmit(TerrainTextureCoord30,PrimaryTerrainLight[Index30],1.f,TerrainVertex30);
}

inline void Vertex02()
{
	TerrainVertexEmit(TerrainTextureCoord02,PrimaryTerrainLight[Index02],1.f,TerrainVertex02);
}

inline void VertexAlpha0()
{
	TerrainVertexEmit(TerrainTextureCoord[0],PrimaryTerrainLight[TerrainIndex1],TerrainMappingAlpha[TerrainIndex1],TerrainVertex[0]);
}

inline void VertexAlpha1()
{
	TerrainVertexEmit(TerrainTextureCoord[1],PrimaryTerrainLight[TerrainIndex2],TerrainMappingAlpha[TerrainIndex2],TerrainVertex[1]);
}

inline void VertexAlpha2()
{
	TerrainVertexEmit(TerrainTextureCoord[2],PrimaryTerrainLight[TerrainIndex3],TerrainMappingAlpha[TerrainIndex3],TerrainVertex[2]);
}

inline void VertexAlpha3()
{
	TerrainVertexEmit(TerrainTextureCoord[3],PrimaryTerrainLight[TerrainIndex4],TerrainMappingAlpha[TerrainIndex4],TerrainVertex[3]);
}

inline void VertexAlpha01()
{
	TerrainVertexEmit(TerrainTextureCoord01,PrimaryTerrainLight[Index01],TerrainMappingAlpha01,TerrainVertex01);
}

inline void VertexAlpha12()
{
	TerrainVertexEmit(TerrainTextureCoord12,PrimaryTerrainLight[Index12],TerrainMappingAlpha12,TerrainVertex12);
}

inline void VertexAlpha23()
{
	TerrainVertexEmit(TerrainTextureCoord23,PrimaryTerrainLight[Index23],TerrainMappingAlpha23,TerrainVertex23);
}

inline void VertexAlpha30()
{
	TerrainVertexEmit(TerrainTextureCoord30,PrimaryTerrainLight[Index30],TerrainMappingAlpha30,TerrainVertex30);
}

inline void VertexAlpha02()
{
	TerrainVertexEmit(TerrainTextureCoord02,PrimaryTerrainLight[Index02],TerrainMappingAlpha02,TerrainVertex02);
}

inline void VertexBlend0()
{
    float Light = TerrainMappingAlpha[TerrainIndex1];
	vec3_t BlendLight; Vector(Light,Light,Light,BlendLight);
	TerrainVertexEmit(TerrainTextureCoord[0],BlendLight,1.f,TerrainVertex[0]);
}
...
inline void VertexBlend1()
{
	...
	vec3_t BlendLight; Vector(Light,Light,Light,BlendLight);
	TerrainVertexEmit(TerrainTextureCoord[1],BlendLight,1.f,TerrainVertex[1]);
}
...
inline void VertexBlend2()
{
	...
	vec3_t BlendLight; Vector(Light,Light,Light,BlendLight);
	TerrainVertexEmit(TerrainTextureCoord[2],BlendLight,1.f,TerrainVertex[2]);
}
...
void RenderFace(int Texture,int mx,int my)
{
	...
	Vertex2();
	Vertex3();
	Platform::GetLegacyRenderAdapter().End();
}
...
bool RenderTerrainTile(float xf,float yf,int xi,int yi,float lodf,int lodi,bool Flag)
{
	...
	if(!Flag)
	{
...
#endif // CSK_DEBUG_MAP_PATHFINDING
		{
			if ( 2 <= path->GetClosedStatus( TerrainIndex1))
			{
				...
				{
					renderer.Color4f(0.3f,0.3f,1.0f,0.5f);
				}
				else
				{
					renderer.Color4f(1.0f,1.0f,1.0f,0.3f);
				}
				for(int i=0;i<4;i++)
				{
					renderer.Vertex3fv(TerrainVertex[i]);
				}
				renderer.End();
				DisableAlphaBlend();
			}
	...
	else
	{
        ...
        if(EditFlag!=EDIT_LIGHT)
		{
			DisableTexture();
			Platform::ILegacyRenderAdapter& renderer = Platform::GetLegacyRenderAdapter();
			renderer.Begin(Platform::LegacyPrimitiveLineStrip);
			renderer.Color4f(0.5f,0.5f,0.5f,1.f);
			for(int i=0;i<4;i++)
			{
				renderer.Vertex3fv(TerrainVertex[i]);
			}
           ...
           ) )
		{
			...
			Platform::ILegacyRenderAdapter& renderer = Platform::GetLegacyRenderAdapter();
			renderer.Begin(Platform::LegacyPrimitiveTriangleFan);
			renderer.Color4f(1.f,0.5f,0.5f,0.3f);
			for(int i=0;i<4;i++)
			{
				renderer.Vertex3fv(TerrainVertex[i]);
			}
...
void RenderTerrainBitmapTile(float xf,float yf,float lodf,int lodi,vec3_t c[4],bool LightEnable,float Alpha,float Height=0.f)
{
	...
	for(int i=0;i<4;i++)
	{
		if(LightEnable)
		{
			// glColor3fv define alpha implicito 1.0 no caminho Alpha==1.f.
			renderer.Color4f(Light[i][0],Light[i][1],Light[i][2],Alpha==1.f ? 1.f : Alpha);
		}
...
{
	// RenderTerrainBitmapTile e chamado com LightEnable=false, entao a cor corrente
	// definida aqui e a unica fonte de cor dos vertices. glColor3fv implica alpha 1.0.
	Platform::GetLegacyRenderAdapter().Color4f(Light[0],Light[1],Light[2],Alpha==1.f ? 1.f : Alpha);
