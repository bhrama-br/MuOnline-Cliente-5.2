...
{
	// The platform scene runner can reach this entry point after WebzenScene has
	// already built the legacy main UI.  These UI objects are singletons within
	// CNewUIManager; creating them twice leaves stale registrations behind.
	if (m_pNewChaosCastleTime != NULL)
	{
		g_ErrorReport.Write("[BootTrace] Main UI already loaded; duplicate request ignored.\r\n");
		return true;
	}

	g_ErrorReport.Write("[BootTrace] Main UI creation started.\r\n");
	gLuaLoadImage.Init();
	gInterface.Init();
...
void SEASON3B::CNewUISystem::Show(DWORD dwKey)
{
	...
	// Reflow side windows created before the browser canvas reached its final size.
	m_pNewMyInventory->SetPos(GWidescreen.WidescreenPosX1, 0);
	m_pChaosGenesis->SetPos(GWidescreen.WidescreenPosX2, 0);
	m_pNewNPCShop->SetPos(GWidescreen.WidescreenPosX2, CNewUINPCShop::NPCSHOP_POS_Y);
	m_pNewPetInfoWindow->SetPos(GWidescreen.WidescreenPosX2, 0);
	...
	// Event/status windows use the physical canvas edges or its centre rather
	// than the side-panel anchors above.  They are also created before the web
	// canvas has its final size, so refresh those positions before opening UI.
	m_pNewBloodCastle->SetPos(GetWindowsX - 127, 480 - 132);
	m_pNewKanturu2ndEnterNpc->SetPos(setPosCenterX(230), 20);
	m_pNewKanturuInfoWindow->SetPos(GetWindowsX - 99, 351);
	m_pNewChaosCastleTime->SetPos(GetWindowsX - 127, 480 - 132);
	m_pNewBattleSoccerScore->SetPos(GetWindowsX - 131, 359);
	m_pNewWindowMenu->SetPos(GetWindowsX - 112, 480 - 171);
	m_pNewDuelWindow->SetPos(GetWindowsX - 131, 359);
	m_pNewSiegeWarfare->SetPos(GetWindowsX - 154, 234);
	m_pNewCursedTempleEnterWindow->SetPos(setPosCenterX(230), 80);
	m_pNewCursedTempleResultWindow->SetPos(setPosCenterX(230), 120);
	m_pNewGoldBowman->SetPos(GWidescreen.WidescreenPosX1, 0);
	m_pNewGoldBowmanLena->SetPos(GWidescreen.WidescreenPosX1, 0);
	m_pNewLuckyCoinRegistration->SetPos(GWidescreen.WidescreenPosX2, 0);
	m_pNewExchangeLuckyCoinWindow->SetPos(GWidescreen.WidescreenPosX2, 0);
	m_pNewDuelWatchWindow->SetPos(GWidescreen.WidescreenPosX1, 0);
	m_pNewDuelWatchUserListWindow->SetPos(GetWindowsX - 57, 480 - 51);
	m_pNewDoppelGangerWindow->SetPos(GWidescreen.WidescreenPosX1, 0);
	m_pNewDoppelGangerFrame->SetPos(GetWindowsX - 227, 480 - 51 - 87);
	m_pNewQuestProgress->SetPos(GWidescreen.WidescreenPosX1, 0);
	m_pNewQuestProgressByEtc->SetPos(GWidescreen.WidescreenPosX1, 0);
	m_pNewEmpireGuardianNPC->SetPos(GWidescreen.WidescreenPosX1, 0);
	m_pNewEmpireGuardianTimer->SetPos(GetWindowsX - 127, 342);
	m_pNewUnitedMarketPlaceWindow->SetPos(GWidescreen.WidescreenPosX1, 0);
#endif
	...
	if(m_pNewUIMng) 
	{
		...
		else if(dwKey == SEASON3B::INTERFACE_CHARACTER)
		{
			...
			g_pCharacterInfoWindow->SetPos(GWidescreen.WidescreenPosX1, 0);
		...
		{
#if defined(__EMSCRIPTEN__)
			// SetWndPos was historically called only before the edit controls were
			// allocated.  When the Web canvas changes size, update the controls with
			// the same centered coordinates used by Create(), not its old x+72/y+32
			// construction-time fallback.
			const int chatY = setPosDown(415);
			m_pNewChatInputBox->SetWndPosForWebViewport(0, chatY);
#endif
