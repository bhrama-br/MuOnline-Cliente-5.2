...
// Substitui o caminho de vertex array cliente-side (glVertexPointer/glColorPointer/
// glTexCoordPointer + glDrawArrays), removido do GLES3/WebGL2, reproduzindo os
// mesmos vertices pelo ILegacyRenderAdapter. O adapter agrega tudo em um unico
// VBO e emite um glDrawArrays, entao o lote continua sendo um so por chamada.
static void DrawLegacyVertexArray(const vec3_t* vertices, const vec4_t* colors, const vec2_t* texCoords, int count)
{
...
void BMD::BeginRenderCoinHeap()
{
    ...
    DisableAlphaBlend();
    // O estado de client array nao existe mais: o lote e montado em
    // AddToCoinHeap e emitido em EndRenderCoinHeap via adapter.
}
...
void BMD::RenderMesh(int meshIndex, int renderFlags, float alpha, int blendMeshIndex, float blendMeshAlpha, float blendMeshTextureCoordU, float blendMeshTextureCoordV, int explicitTextureIndex)
{
    ...
    else if (blendMeshIndex <= -2 || m->Texture == blendMeshIndex)
    {
        ...
        SetLegacyColor3f(BodyLight[0] * blendMeshAlpha,
...
void BMD::RenderMeshAlternative(int iRndExtFlag, int iParam, int i, int RenderFlag, float Alpha, int BlendMesh, float BlendMeshLight, float BlendMeshTexCoordU, float BlendMeshTexCoordV, int MeshTexture)
{
    ...
    else if (BlendMesh <= -2 || m->Texture == BlendMesh)
    {
        ...
        // Com EnableLight=false o laco de vertices nao emite cor, entao esta cor
        // corrente e a unica fonte. glColor3f implica alpha 1.0.
        Platform::GetLegacyRenderAdapter().Color4f(BodyLight[0] * BlendMeshLight, BodyLight[1] * BlendMeshLight, BodyLight[2] * BlendMeshLight, 1.f);
        //glColor3f(BlendMeshLight,BlendMeshLight,BlendMeshLight);
        EnableLight = false;
    }
        ...
        if (m->m_csTScript != NULL)
        {
            if (m->m_csTScript->getHiddenMesh() == false && i != HiddenMesh)
            {
                ...
                if (shadowType == SHADOW_RENDER_COLOR)
                {
                    DisableAlphaBlend();
                    if (Alpha >= 0.99f)
                        SetLegacyColor3f(0.f, 0.f, 0.f);
                    else
                        SetLegacyColor4f(0.f, 0.f, 0.f, Alpha);
...
void BMD::AddClothesShadowTriangles(void* pClothes, const int clothesCount, const float sx, const float sy) const
{
    ...
    // Sombra de tecido: so posicoes, sem cor nem UV proprios (a cor corrente vem
    // do chamador, como no caminho legado com apenas GL_VERTEX_ARRAY ativo).
    DrawLegacyVertexArray(vertices, NULL, NULL, target_vertex_index + 1);
}
...
void BMD::RenderObjectBoundingBox()
{
    ...
    for (int i = 0; i < NumBones; i++)
    {
        ...
        if (b->BoundingBox)
        {
            ...
            renderer.TexCoord2f(1.0F, 1.0F); renderer.Vertex3fv(BoundingVertices[7]);
            renderer.TexCoord2f(1.0F, 0.0F); renderer.Vertex3fv(BoundingVertices[6]);
            renderer.TexCoord2f(0.0F, 0.0F); renderer.Vertex3fv(BoundingVertices[4]);
            renderer.TexCoord2f(0.0F, 1.0F); renderer.Vertex3fv(BoundingVertices[5]);

            renderer.Color4f(0.2f, 0.2f, 0.2f, 1.f);
            renderer.TexCoord2f(0.0F, 1.0F); renderer.Vertex3fv(BoundingVertices[0]);
            renderer.TexCoord2f(1.0F, 1.0F); renderer.Vertex3fv(BoundingVertices[2]);
            renderer.TexCoord2f(1.0F, 0.0F); renderer.Vertex3fv(BoundingVertices[3]);
            renderer.TexCoord2f(0.0F, 0.0F); renderer.Vertex3fv(BoundingVertices[1]);

            renderer.Color4f(0.6f, 0.6f, 0.6f, 1.f);
            renderer.TexCoord2f(1.0F, 1.0F); renderer.Vertex3fv(BoundingVertices[7]);
            renderer.TexCoord2f(1.0F, 0.0F); renderer.Vertex3fv(BoundingVertices[3]);
            renderer.TexCoord2f(0.0F, 0.0F); renderer.Vertex3fv(BoundingVertices[2]);
            renderer.TexCoord2f(0.0F, 1.0F); renderer.Vertex3fv(BoundingVertices[6]);

            renderer.Color4f(0.6f, 0.6f, 0.6f, 1.f);
            renderer.TexCoord2f(0.0F, 1.0F); renderer.Vertex3fv(BoundingVertices[0]);
            renderer.TexCoord2f(1.0F, 1.0F); renderer.Vertex3fv(BoundingVertices[1]);
            renderer.TexCoord2f(1.0F, 0.0F); renderer.Vertex3fv(BoundingVertices[5]);
            renderer.TexCoord2f(0.0F, 0.0F); renderer.Vertex3fv(BoundingVertices[4]);
            ...
            renderer.TexCoord2f(1.0F, 0.0F); renderer.Vertex3fv(BoundingVertices[5]);
            renderer.TexCoord2f(0.0F, 0.0F); renderer.Vertex3fv(BoundingVertices[1]);
            renderer.TexCoord2f(0.0F, 1.0F); renderer.Vertex3fv(BoundingVertices[3]);

            renderer.Color4f(0.4f, 0.4f, 0.4f, 1.f);
            renderer.TexCoord2f(0.0F, 1.0F); renderer.Vertex3fv(BoundingVertices[0]);
            renderer.TexCoord2f(1.0F, 1.0F); renderer.Vertex3fv(BoundingVertices[4]);
            renderer.TexCoord2f(1.0F, 0.0F); renderer.Vertex3fv(BoundingVertices[6]);
            renderer.TexCoord2f(0.0F, 0.0F); renderer.Vertex3fv(BoundingVertices[2]);
            renderer.End();
        }
    ...
    for (int i = 0; i < NumBones; i++)
    {
        ...
        if (!b->Dummy)
        {
            ...
            if (Parent > 0)
            {
                ...
                renderer.Begin(Platform::LegacyPrimitiveLines);
                renderer.Vertex3fv(BoneVertices[0]);
                renderer.Vertex3fv(BoneVertices[1]);
                renderer.Vertex3fv(BoneVertices[1]);
                renderer.Vertex3fv(BoneVertices[2]);
                renderer.Vertex3fv(BoneVertices[2]);
                renderer.Vertex3fv(BoneVertices[0]);
                renderer.End();
            }
...
bool BMD::Open(char* DirName, char* ModelFileName)
{
    ...
    FILE* fp = Platform::LegacyFileOpen(ModelName, "rb");
    if (fp == NULL)
    {
        return false;
    }
...
bool BMD::Save(char* DirName, char* ModelFileName)
{
    ...
    strcpy(ModelName, DirName);
    strcat(ModelName, ModelFileName);
    FILE* fp = Platform::LegacyFileOpen(ModelName, "wb");
...
bool BMD::Open2(const char* DirName, const char* ModelFileName, bool bReAlloc)
{
    ...
    if (Version == 12)
    {
        // `long` tem 4 bytes no Windows (LLP64) e no wasm32, mas 8 no Android
        // (LP64). O .bmd foi gravado por ferramenta Windows de 32 bits, entao o
        // campo tem 4 bytes FIXOS. Ler `long` no Android consumia 8: o tamanho
        // vinha com lixo nos bytes altos e o cursor avancava errado, e o `new`
        // logo abaixo estourava com std::bad_alloc.
        //
        // Passou despercebido porque o Web e wasm32, onde `long` tem 4 bytes --
        // este e o tipo de defeito que SO aparece no alvo de 64 bits.
        //
        // memcpy em vez de cast: o offset nao tem alinhamento garantido.
        int32_t lSize = 0;
        memcpy(&lSize, Data + DataPtr, sizeof(lSize)); DataPtr += (int)sizeof(lSize);
...
bool BMD::Save2(char* DirName, char* ModelFileName)
{
    ...
    // Mesma razao da leitura acima: o campo de tamanho no arquivo tem 4 bytes
    // fixos. Gravar `sizeof(long)` no Android escreveria 8 e geraria um arquivo
    // que nem o proprio cliente leria de volta.
    const int32_t lSize = (int32_t)(pbyCur - pbyBuffer);
    const int32_t lEncSize = (int32_t)MapFileEncrypt(NULL, pbyBuffer, lSize);
