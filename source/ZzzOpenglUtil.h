...
// Ver ZzzOpenglUtil.cpp: stream de textura legacy sem chamadores, aguardando
// suporte a lote persistente no ILegacyRenderAdapter.
#if 0
...
// Definida aqui, nao em ZzzOpenglUtil.cpp: mesmo motivo das funcoes de terreno em
// ZzzLodTerrain.h -- `inline` num .cpp com `extern inline` no cabecalho liga no
// MSVC e nao liga no Clang, e o wasm aborta ao chamar.
...
// Define a cor corrente no pipeline fixo e no ILegacyRenderAdapter ao mesmo tempo.
// Necessario porque helpers como RenderColor/RenderBitmap usam Alpha = 0.f por
// padrao e nao emitem cor propria nesse caminho: dependem da cor que o chamador
// definiu antes. Sem isso o backend GLSL desenharia a UI com a cor anterior.
// Alpha test e fog do pipeline fixo. No Windows continuam indo para o OpenGL;
// em GLES3/WebGL2 nao existem como estado fixo e viram uniformes do shader.
...
// Variantes de byte (0..255) usadas pela UI.
