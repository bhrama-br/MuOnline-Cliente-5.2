...
{
	// Optional legacy text controls can be positioned before allocation on a cold start.
	if ((UINT_PTR)this < 0x10000)
	{
		#if defined(_MSC_VER)
		...
		#endif
		g_ErrorReport.Write("CUIControl::SetPosition ignored invalid control (caller=%p)\r\n", caller);
		return;
	}
...
void CUIButton::Render()
{
	...
	if (GetState() == UISTATE_DISABLE)
	{
		SetLegacyColor4f(1.0f, 0.4f, 0.4f, 1.0f);
...
BOOL CUIChatPalListBox::RenderDataLine(int iLineNumber)
{
	...
	if (SLGetSelectLineNum() == m_iCurrentRenderEndLine + iLineNumber + 1)
	{
		if (g_dwKeyFocusUIID == GetUIID())
			SetLegacyColor4f(1.0f, 1.0f, 1.0f, 0.7f);
		else 
			SetLegacyColor4f(1.0f, 1.0f, 1.0f, 0.3f);
	...
	if (m_iLayoutType == 1)
	{
		...
		{
			sprintf(Text,"%s", GlobalText[1039]);
		}
		...
		{
			sprintf(Text,"%s", GlobalText[1041]);
		}
...
//		{
//			sprintf(Text,"%s", GlobalText[1040]);
//		}
		else
		{	
...
BOOL CUIWindowListBox::RenderDataLine(int iLineNumber)
{
	...
	if (SLGetSelectLineNum() == m_iCurrentRenderEndLine + iLineNumber + 1)
	{
		if (g_dwKeyFocusUIID == GetUIID()) SetLegacyColor4f(1.0f, 1.0f, 1.0f, 0.7f);
		else SetLegacyColor4f(1.0f, 1.0f, 1.0f, 0.3f);
		RenderColor(m_iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13);
		SetLegacyColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		SetLegacyTexture2D(true);
		g_pRenderText->SetTextColor(0, 0, 0, 255);
...
void CUIRenderTextOriginal::WriteText(int iOffset, int iWidth, int iHeight)
{
	...
	// Casts explicitos: SIZE tem campos LONG e a inicializacao por chaves proibe
	// estreitamento. O valor e o mesmo que Create() usa para dimensionar a DIB.
...
#ifdef _DEBUG
#ifdef _WIN32
				__asm { int 3 };   // ponto de parada; sem equivalente portavel
#endif
#endif // _DEBUG
				return;
			}
										...
										int iSort /* = RT3_SORT_LEFT */, OUT SIZE* lpTextSize /* = NULL */)
{
	...
	// SIZEF tem campos float; o cast era para (long), que estreita na inicializacao
	// por chaves. O valor final ja era float de qualquer forma.
	SIZEF RealRenderingSize = { (float)RealTextSize.cx, (float)RealTextSize.cy };	
	...
	{
		EnableAlphaTest();
		SetLegacyColor4ub(GetRed(m_dwBackColor), GetGreen(m_dwBackColor), 
...
void CUITextInputBox::WriteText(int iOffset, int iWidth, int iHeight)
{
	...
	SIZE RealBoxSize = { (LONG)((long)m_iWidth*g_fScreenRate_x), (LONG)((long)m_iHeight*g_fScreenRate_y) };
	const int LIMIT_WIDTH = 256, LIMIT_HEIGHT = 32;
	...
	RECT rcCaret = { (LONG)(pt.x-LIMIT_WIDTH*iSectionX), (LONG)(pt.y-LIMIT_HEIGHT*iSectionY), (LONG)(pt.x-LIMIT_WIDTH*iSectionX+m_fCaretWidth), (LONG)(pt.y-LIMIT_HEIGHT*iSectionY+m_fCaretHeight) };
...
void CUITextInputBox::Render()
{
	...
	POINT RealWndPos = { (LONG)(m_iPos_x*g_fScreenRate_x), (LONG)(m_iPos_y*g_fScreenRate_y) };
	SIZE RealWndSize = { (LONG)(m_iWidth*g_fScreenRate_x), (LONG)(m_iHeight*g_fScreenRate_y) };
...
void CUITextInputBox::SetFont(HFONT hFont)
{
	...
	// WPARAM tem largura de ponteiro; (UINT) truncava o HFONT em 64 bits.
...
void CUISlideHelp::Render(BOOL bForceFadeOut)
{
	...
	EnableAlphaTest();
	SetLegacyColor4ub(0, 0, 0, (m_iAlphaRate > 180 ? m_iAlphaRate : (m_iAlphaRate - 25 < 0 ? 0 : m_iAlphaRate - 25)));

	RenderColor(0, m_iPos_y - 3, WindowWidth, 1);
	RenderColor(0, m_iPos_y + m_iFontHeight + 2, WindowWidth, 1);
	SetLegacyColor4ub(0, 0, 0, (m_iAlphaRate - 25 < 0 ? 0 : m_iAlphaRate - 25));
...
void CSlideHelpMgr::OpenSlideTextFile(const char * szFileName)
{
	...
	FILE *fp = Platform::LegacyFileOpen(szFileName,"rb");
	if(fp == NULL)
	{
		char Text[256];
...
BOOL CUIGuildNoticeListBox::RenderDataLine(int iLineNumber)
{
	...
	if (SLGetSelectLineNum() == m_iCurrentRenderEndLine + iLineNumber + 1)
	{
		SetLegacyColor4f(1.0f, 1.0f, 1.0f, 0.5f);
		RenderColor(m_iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13);
		SetLegacyColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		g_pRenderText->SetTextColor(0, 0, 0, 255);
	}
...
BOOL CUINewGuildMemberListBox::RenderDataLine(int iLineNumber)
{
	...
	{
		SetLegacyColor4ub(255, 100, 50, 127);
		RenderColor(m_iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13);
		SetLegacyColor4f(255, 255, 255, 255);
	}
	else if (iCharacterLevel == 1)
	{
		SetLegacyColor4ub(255, 150, 80, 127);
		RenderColor(m_iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13);
		SetLegacyColor4f(255, 255, 255, 255);
	}
	else if (iCharacterLevel == 2)
	{
		SetLegacyColor4ub(255, 200, 100, 127);
		RenderColor(m_iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13);
		SetLegacyColor4f(255, 255, 255, 255);
	}
...
BOOL CUIBCGuildListBox::RenderDataLine(int iLineNumber)
{
	...
	{
		SetLegacyColor4f(1.0f, 1.0f, 1.0f, 0.2f);
		RenderColor(m_iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13);
		SetLegacyColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		g_pRenderText->SetTextColor(230, 220, 200, 255);
	}
...
void RenderGoldRect(float fPos_x, float fPos_y, float fWidth, float fHeight, int iFillType = 0)
{
	...
	{
	case 1:
		SetLegacyColor4ub(146, 144, 141, 200);
...
BOOL CUICurQuestListBox::RenderDataLine(int iLineNumber)
{
	...
	if (SLGetSelectLineNum() == m_iCurrentRenderEndLine + iLineNumber + 1)
	{
		::SetLegacyColor4f(0.5f, 0.7f, 0.3f, 0.5f);
		RenderColor(m_iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13);
		SetLegacyColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		SetLegacyTexture2D(true);
	}
...
BOOL CUIInGameShopListBox::RenderDataLine(int iLineNumber)
{
	...
	if(SLGetSelectLineNum() == m_iCurrentRenderEndLine + iLineNumber + 1)
	{
		::SetLegacyColor4f(0.15f, 0.3f, 0.4f, 0.5f);
...
BOOL CUIPackCheckBuyingListBox::RenderDataLine(int nLine)
{
	...
	{
		::SetLegacyColor4f(0.07f, 0.31f, 0.31f, 0.5f);
