...
{
	// LegacyFileOpen: normaliza a barra invertida e aciona a busca sob demanda.
	// Com ::fopen o Web nunca achava credit.bmd (o arquivo EXISTE na arvore), e a
	// falha aqui chama MessageBox + PostMessage(WM_DESTROY) -- ou seja, o caminho de
	// erro era uma caixa de dialogo sobre a cena de login. Repasse direto no Windows.
	FILE *fp = Platform::LegacyFileOpen(CRW_DATA_FILE, "rb");
	if (fp == NULL)
	{
		char szMessage[256];
