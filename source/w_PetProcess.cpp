...
bool PetProcess::LoadData()
{
	...
	else
	{
		...
		for(int i=0;i<_listSize;i++)
		{
			...
			_blendMesh = -1;
			_count = 0;
			// sizeof do PONTEIRO, nao do vetor: zerava 4 bytes de 200. Inofensivo
			// por acidente (o memcpy abaixo reescreve tudo), mas e o mesmo erro que
			// no _speed adiante e melhor nao deixar dois exemplares do padrao.
			...
			// sizeof(_speed) e o tamanho do PONTEIRO, nao do float.
			//
			// No Windows 32 bits ponteiro e float medem 4 bytes, entao a conta dava
			// certo por acaso e o defeito nunca apareceu. Em 64 bits sao 8: copiava
			// 400 bytes para um buffer de 200 (corrompendo o heap) e avancava pSeek
			// o dobro do devido, fazendo as iteracoes seguintes lerem lixo. O _count
			// que saia dai era um inteiro absurdo, e SetActions alocava sobre ele --
			// 2,6 GB em poucos segundos, com o LMK matando o processo sem tombstone
			// e a ~2 minutos de distancia da causa.
			//
			// Com sizeof(float) o resultado em 32 bits e byte a byte o mesmo: o
			// build de PC nao muda.
