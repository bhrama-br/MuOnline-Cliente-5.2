...
void RenderTipText(int sx, int sy, const char* Text)
{
	...
	SetLegacyColor4f(0.0f, 0.0f, 0.0f, 0.8f);
...
void SetBooleanPosition(CHAT *c)
{
	...
	if ( g_isCharacterBuff((&c->Owner->Object), eBuff_GMEffect) || // GM \xC0\xCF\xB0\xE6\xBF\xEC
...
extern float g_fScreenRate_x;	// \xA1\xD8
extern float g_fScreenRate_y;
...
int RenderDebugText(int y)
{
    ...
    for(int i=0;i<min(DebugTextCount,10);i++)
	{
		...
		{
			SetLegacyColor3f(0.6f,0.6f,0.6f);
			if(j==0)
			{
...
void Action(CHARACTER *c,OBJECT *o,bool Now)
{
	...
	case MOVEMENT_GET:
		...
		else if(g_pMyInventory->FindEmptySlot(&Items[ItemKey].Item) == -1)
		{
			...
			unicode::_sprintf(Text, "%s", GlobalText[375]);
...
void CheckChatText(char *Text)
{
	...
	else if(FindText(Text,"\xA4\xD0.\xA4\xD0") || FindText(Text,"\xA4\xCC.\xA4\xCC") || FindText(Text,"T_T") || FindText(Text,GlobalText[306]) || FindText(Text,GlobalText[307]) || FindText(Text,GlobalText[308]) || FindText(Text,GlobalText[309]))
	{
		SetActionClass(c,o,PLAYER_CRY1,AT_CRY1);
		SendRequestAction(AT_CRY1,((BYTE)((o->Angle[2]+22.5f)/360.f*8.f+1.f)%8));
	}
	else if(FindText(Text,"\xA4\xD1.\xA4\xD1") || FindText(Text,"\xA4\xD1.,\xA4\xD1") || FindText(Text,"\xA4\xD1,.\xA4\xD1") || FindText(Text,"-.-") || FindText(Text,"-_-") || FindText(Text,GlobalText[310]) || FindText(Text,GlobalText[311]))
	{
	...
	else if(FindText(Text,GlobalText[342]) || FindText(Text,GlobalText[343]) || FindText(Text,"/\xA4\xD1") || FindText(Text,"\xA4\xD1^"))
	{
...
void RenderBar(float x,float y,float Width,float Height,float Bar,bool Disabled,bool clipping)
{
	...
	EnableAlphaTest();
   	SetLegacyColor4f(0.f,0.f,0.f,0.5f);
	RenderColor(x+1,y+1,Width+4,Height+4);
	
	EnableAlphaBlend();
	if ( Disabled)
		SetLegacyColor3f(0.2f,0.0f,0.0f);
	else
		SetLegacyColor3f(0.f,0.2f,0.2f);
	RenderColor(x,y,Width+4,Height+4);
	if ( Disabled)
		SetLegacyColor3f(50.f/255.f,10/255.f,0.f);
	else
		SetLegacyColor3f(0.f/255.f,50/255.f,50.f/255.f);
	RenderColor(x+2,y+2,Width,Height);
	if ( Disabled)
		SetLegacyColor3f(200.f/255.f,50/255.f,0.f);
	else
		SetLegacyColor3f(0.f/255.f,200/255.f,50.f/255.f);
...
void RenderOutSides()
{
	...
	{
		EnableAlphaTest();
       	SetLegacyColor4f(1.f,1.f,1.f,0.5f);
		EnableAlphaBlend();
       	SetLegacyColor3f(0.3f,0.3f,0.25f);
...
void RenderTournamentInterface()
{
	...
	sprintf(t_Str, "%s", GlobalText[1393]);
	g_pRenderText->RenderText( WindowX+Width/2-50, WindowY+20, t_Str);
	g_pRenderText->SetTextColor(255, 255, 255, 255);

	sprintf(t_Str, "%s", GlobalText[1394]);
	g_pRenderText->SetTextColor(255, 255, 10, 255);
	...
	if(g_wtMatchResult.m_Score1 == g_wtMatchResult.m_Score2)
	{
		g_pRenderText->SetFont(g_hFontBig);
		g_pRenderText->SetTextColor(255, 255, 10, 255);
		sprintf(t_Str, "%s", GlobalText[1395]);
	...
	else if(g_wtMatchResult.m_Score1 > g_wtMatchResult.m_Score2)
	{
		...
		sprintf(t_Str, "%s", GlobalText[1396]);
		g_pRenderText->RenderText( WindowX+47, WindowY+115, t_Str);
		g_pRenderText->SetTextColor(255, 10, 10, 255);
		sprintf(t_Str, "%s", GlobalText[1397]);
...
void RenderPartyHP ()
{
    ...
    for ( int j=0; j<PartyNumber; ++j )
    {
        ...
        SetLegacyColor3f( 250.f/255.f, 10/255.f, 0.f );
