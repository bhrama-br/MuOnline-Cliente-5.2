...
void HELPER_VIEW::Render()
{
	...
	if (b) 
	{
		// TODO(Platform): a ponte Lua recebe ponteiros como "i" (inteiro). Em 64
		// bits isso trunca: o cast duplo abaixo apenas torna a perda explicita
		// para o compilador, nao a resolve. O mesmo vale para `b`, que ja passava
		// truncado sem aviso. Corrigir exige um tipo de handle na ponte Lua.
		gHelperSystem.m_Lua.Generic_Call("MoveEffects", "iii>", b, (DWORD)(DWORD_PTR)(m_Obj), m_Obj->Type);
		return;
	}
}
