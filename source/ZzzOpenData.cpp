...
//	gLoadData.AccessModel(MODEL_HELPER+132, "Data\\Item\\", "goldenmaple_cham");
	...
	//Novos Itens
	for (std::map<int, CUSTOM_ITEM_INFO>::iterator it = gItemManager.m_CustomItemInfo.begin(); it != gItemManager.m_CustomItemInfo.end(); it++) 
	{
		auto ItemInfo = &(*it).second;

		if (ItemInfo->Index != -1)
		{
			gLoadData.AccessModel(ItemInfo->ItemIndex, ((ItemInfo->ItemIndex >= gItemManager.GET_ITEM_MODEL(7, 0) && ItemInfo->ItemIndex < gItemManager.GET_ITEM_MODEL(12, 0)) ? "Data\\Player\\" : "Data\\Item\\"), ItemInfo->ModelName);
		}
	}
	...
	for (int n = 0; n < MAX_CUSTOM_JEWEL; n++)
	{
		...
		{
			gLoadData.AccessModel((gCustomJewel.m_CustomJewelInfo[n].ItemIndex + MODEL_ITEM), "Data\\Item\\", gCustomJewel.m_CustomJewelInfo[n].ModelName);
		}
	}
	...
	for (std::map<int, HELPER_INFO>::iterator it = gHelperSystem.m_HelperInfo.begin(); it != gHelperSystem.m_HelperInfo.end(); it++)
	{
		...
		gLoadData.AccessModel(it->second.ItemIndex, "Data\\Item\\", it->second.Model);

		if (lpInfo->Type > 0)
		{
			lpInfo->ModelID = StartModelHelper;

			gLoadData.AccessModel(StartModelHelper, "Data\\Skill\\", it->second.ObjectModel);
	...
	for (std::map<int, DARK_SPIRIT_INFO>::iterator it = gDarkSpirit.m_DarkSpiritInfo.begin(); it != gDarkSpirit.m_DarkSpiritInfo.end(); it++) 
	{
		...
		gLoadData.AccessModel(it->second.ItemIndex + MODEL_ITEM, "Data\\Item\\", it->second.ModelName);

		it->second.ItemModel = DarkSpirit++;

		gLoadData.AccessModel(it->second.ItemModel, "Data\\Item\\", it->second.ObjectName);

		gDarkSpirit.m_DarkSpiritModels[it->second.ItemModel] = it->second.ItemIndex;
	}

	int StartModelPatent = gItemManager.GET_ITEM_MODEL(18, 0);

	for (std::map<int, PATENT_MODEL_INFO>::iterator it = gPatente.m_PatentModelInfo.begin(); it != gPatente.m_PatentModelInfo.end(); it++) 
	{
		auto lpInfo = &(*it).second;

		char paths[MAX_PATH] = { 0 };

		wsprintf(paths, "Data\\%s", lpInfo->ModelFolder);		

		gLoadData.AccessModel(StartModelPatent, paths, lpInfo->ModelName);

		gPatente.PatentModelSetIndex(lpInfo->PatentIndex, StartModelPatent);
...
//---------------------------------------------------------------------------------
	...
	for (std::map<int, CUSTOM_ITEM_INFO>::iterator it = gItemManager.m_CustomItemInfo.begin(); it != gItemManager.m_CustomItemInfo.end(); it++)
	{
		...
		if (ItemInfo->Index != -1)
		{
			gLoadData.OpenTexture(ItemInfo->ItemIndex, ((ItemInfo->ItemIndex >= gItemManager.GET_ITEM_MODEL(7, 0) && ItemInfo->ItemIndex < gItemManager.GET_ITEM_MODEL(12, 0)) ? "Player\\" : "Item\\"));
		}
	}
	...
	for (int n = 0; n < MAX_CUSTOM_JEWEL; n++)
	{
		if (gCustomJewel.m_CustomJewelInfo[n].Index != -1)
		{
			gLoadData.OpenTexture((gCustomJewel.m_CustomJewelInfo[n].ItemIndex + MODEL_ITEM), "Item\\");
		}
	}
	...
	for (std::map<int, HELPER_INFO>::iterator it = gHelperSystem.m_HelperInfo.begin(); it != gHelperSystem.m_HelperInfo.end(); it++)
	{
		...
		if (lpInfo->Type > 0)
		{
			gLoadData.OpenTexture(lpInfo->ModelID, "Skill\\");
		}
	}
	...
	for (std::map<int, DARK_SPIRIT_INFO>::iterator it = gDarkSpirit.m_DarkSpiritInfo.begin(); it != gDarkSpirit.m_DarkSpiritInfo.end(); it++) 
	{
		...
		gLoadData.OpenTexture(it->second.ItemIndex + MODEL_ITEM, "Item\\");

		it->second.ItemModel = DarkSpirit++;

		gLoadData.OpenTexture(it->second.ItemModel, "Item\\");
	}
	...
	for (std::map<int, PATENT_MODEL_INFO>::iterator it = gPatente.m_PatentModelInfo.begin(); it != gPatente.m_PatentModelInfo.end(); it++) 
	{
		...
		gLoadData.OpenTexture(StartModelPatent, lpInfo->ModelFolder);
...
//    case 73:
	...
	case 147:
		...
		break;
	case 148:
		LoadWaveFile( SOUND_MONSTER+34,						"Data\\Sound\\m\xC7\xEF\xBD\xBA\xC6\xC4\xC0\xCC\xB4\xF5\xC1\xD7\xB1\xE2.wav" ,1);
...
void OpenSounds()
{
    ...
    //LoadWaveFile(SOUND_BOSS01		    ,"Data\\Sound\\a\xC4\xEF\xB5\xD0.wav",1);
    ...
    //LoadWaveFile(SOUND_SHOUT01    		,"Data\\Sound\\p\xB1\xE2\xC7\xD5.wav",1); 
    ...
    //LoadWaveFile(SOUND_FLASH            ,"Data\\Sound\\m\xC8\xF7\xB5\xE5\xB6\xF3\xB0\xF8\xB0\xDD""1.wav",2,Enable3DSound);
    ...
    //LoadWaveFile(SOUND_METEORITE02	    ,"Data\\Sound\\e\xC0\xAF\xBC\xBA.wav",2,Enable3DSound);
...
{
	FILE *fp = Platform::LegacyFileOpen(FileName,"rt");
	if(fp == NULL) return;
	for(int i=0;i<10;i++)
	{
...
void OpenLogoSceneData()
{
	...
	::LoadBitmap("Custom\\NewInterface\\login_back.jpg", BITMAP_LOG_IN+7);
	::LoadBitmap("Custom\\NewInterface\\btn_medium.jpg", BITMAP_LOG_IN+8);
