...
// Declaracoes icadas para escopo de ARQUIVO.
// Estavam dentro de funcoes membro de SEASON3B, e um `extern` ali
// nomeia SEASON3B::X em vez do global ::X -- o link ficava com um
// simbolo fantasma no namespace, que ninguem define.
...
void SEASON3B::CNewUIKeyPadButton::Render()
{
	...
	{
		SetLegacyColor3f(0.80f, 0.80f, 0.80f);
		RenderImage(BITMAP_INVENTORY + 17, GetPosX(), GetPosY(), GetWidth(), GetHeight());
...
{
	float x, y, width, height;
	SetLegacyColor3f(0.3f, 0.3f, 0.3f);
	...
	SetLegacyColor3f(1.0f, 1.0f, 1.0f);
...
void SEASON3B::CGemIntegrationUnityMsgBox::SetButtonInfo()
{
	...
	for (int k = 0; k < (int)COMGEM::eCOMTYPE_END; k++)
	{
		...
		// 1808 "%d\xB0\xB3 \xC1\xB6\xC7\xD5(%d\xC1\xA8 \xBC\xD2\xBF\xE4)"
...
void SEASON3B::CChaosMixMenuMsgBox::RenderTexts()
{
	...
	g_pRenderText->SetFont(g_hFontBold);
	sprintf(szText, "%s", GlobalText[734]);
	...
	sprintf(szText, "%s", GlobalText[872]);
...
bool SEASON3B::CTradeZenMsgBoxLayout::SetLayout()
{
	...
	if (NULL == pMsgBox->Create(MSGBOX_COMMON_TYPE_OKCANCEL, INPUTBOX_TYPE_NUMBER, INPUTBOX_WIDTH, INPUTBOX_HEIGHT, INPUTBOX_TEXTLIMIT))
		return false;
...
bool SEASON3B::CPersonalShopNameMsgBoxLayout::SetLayout()
{
	...
	if (NULL == pMsgBox->Create(MSGBOX_COMMON_TYPE_OKCANCEL, INPUTBOX_TYPE_TEXT, INPUT_WIDTH, INPUT_HEIGHT, INPUT_TEXTLIMIT))
		return false;
...
bool SEASON3B::CPasswordKeyPadMsgBoxLayout::SetLayout()
{
	...
	if (NULL == pMsgBox->Create(KEYPAD_TYPE_MOVE, 4))
		return false;
...
bool SEASON3B::CStorageLockKeyPadMsgBoxLayout::SetLayout()
{
	...
	if (NULL == pMsgBox->Create(KEYPAD_TYPE_LOCK_FIRST, 4))
		return false;
...
bool SEASON3B::CStorageLockCheckKeyPadMsgBoxLayout::SetLayout()
{
	...
	if (NULL == pMsgBox->Create(KEYPAD_TYPE_LOCK_SECOND, 4))
		return false;
...
bool SEASON3B::CStorageLockMsgBoxLayout::SetLayout()
{
	...
	if (NULL == pMsgBox->Create(MSGBOX_COMMON_TYPE_OKCANCEL, INPUTBOX_TYPE_NUMBER, _width, INPUTBOX_HEIGHT, g_iLengthAuthorityCode, true))
	{
		return false;
	}
...
bool SEASON3B::CStorageLockFinalKeyPadMsgBoxLayout::SetLayout()
{
	...
	if (NULL == pMsgBox->Create(KEYPAD_TYPE_LOCK_FINAL, g_iLengthAuthorityCode))
		return false;
...
bool SEASON3B::CStorageUnlockKeyPadMsgBoxLayout::SetLayout()
{
	...
	if (0 == pMsgBox)
		return false;
	if (NULL == pMsgBox->Create(KEYPAD_TYPE_UNLOCK, g_iLengthAuthorityCode))
		return false;
...
void SEASON3B::CLuckyTradeMenuMsgBox::SetButtonInfo()
{
	...
	m_BtnTrade.SetText("\xB7\xB0\xC5\xB0\xBE\xC6\xC0\xCC\xC5\xDB \xB1\xB3\xC8\xAF");	// "GlobalText"
	...
	m_BtnRefinery.SetText("\xB7\xB0\xC5\xB0\xBE\xC6\xC0\xCC\xC5\xDB \xC1\xA6\xB7\xC3");	// "GlobalText"
...
void SEASON3B::CLuckyTradeMenuMsgBox::RenderTexts()
{
	...
	g_pRenderText->SetTextColor(255, 255, 255, 255);
	g_pRenderText->SetFont(g_hFontBold);
	sprintf(szText, "\xB7\xB0\xC5\xB0\xBE\xC6\xC0\xCC\xC5\xDB \xB1\xB3\xC8\xAFNPC");	// "LuckyItem Trade NPC"
	g_pRenderText->RenderText(fPos_x, fPos_y, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);

	fPos_y += 15;
	g_pRenderText->SetFont(g_hFont);
	sprintf(szText, "\xB7\xB0\xC5\xB0\xBE\xC6\xC0\xCC\xC5\xDB\xC0\xB8\xB7\xCE \xB1\xB3\xC8\xAF\xC7\xCF\xB0\xC5\xB3\xAA \xC1\xA6\xB7\xC3\xC7\xD2 \xBC\xF6 \xC0\xD6\xBD\xC0\xB4\xCF\xB4.");
...
void SEASON3B::CTrainerMenuMsgBox::RenderTexts()
{
	...
	g_pRenderText->SetTextColor(255, 255, 255, 255);
	g_pRenderText->SetFont(g_hFontBold);
	sprintf(szText, "%s", GlobalText[1227]);
	g_pRenderText->RenderText(fPos_x, fPos_y, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);

	fPos_y += 15;
	g_pRenderText->SetFont(g_hFont);
	sprintf(szText, "%s", GlobalText[1202]);
	g_pRenderText->RenderText(fPos_x, fPos_y + 1 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);
...
void SEASON3B::CTrainerRecoverMsgBox::RenderTexts()
{
	...
	fPos_y += 15;
	g_pRenderText->SetFont(g_hFont);
	sprintf(szText, "%s", GlobalText[1228]);
	g_pRenderText->RenderText(fPos_x, fPos_y + 1 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);
...
void SEASON3B::CElpisMsgBox::RenderTexts()
{
	...
	g_pRenderText->SetTextColor(255, 255, 255, 255);
	g_pRenderText->SetFont(g_hFontBold);
	sprintf(szText, "%s", GlobalText[2206]);
	...
	{
	case 0:
		sprintf(szText, "%s", GlobalText[2074]);
		g_pRenderText->RenderText(fPos_x, fPos_y + 1 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);
		break;
...
void SEASON3B::CSeedMasterMenuMsgBox::RenderTexts()
{
	...
	g_pRenderText->SetTextColor(255, 255, 255, 255);
	g_pRenderText->SetFont(g_hFontBold);
	sprintf(szText, "%s", GlobalText[2666]);
	g_pRenderText->RenderText(fPos_x, fPos_y, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);

	fPos_y += 15;
	g_pRenderText->SetFont(g_hFont);
	sprintf(szText, "%s", GlobalText[2667]);
	g_pRenderText->RenderText(fPos_x, fPos_y + 1 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);
	sprintf(szText, "%s", GlobalText[2668]);
	g_pRenderText->RenderText(fPos_x, fPos_y + 2 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);
}
...
void SEASON3B::CSeedInvestigatorMenuMsgBox::RenderTexts()
{
	...
	g_pRenderText->SetTextColor(255, 255, 255, 255);
	g_pRenderText->SetFont(g_hFontBold);
	sprintf(szText, "%s", GlobalText[2671]);
	g_pRenderText->RenderText(fPos_x, fPos_y, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);

	fPos_y += 15;
	g_pRenderText->SetFont(g_hFont);
	sprintf(szText, "%s", GlobalText[2672]);
	g_pRenderText->RenderText(fPos_x, fPos_y + 1 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);
	sprintf(szText, "%s", GlobalText[2673]);
	g_pRenderText->RenderText(fPos_x, fPos_y + 2 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);
}
...
void SEASON3B::CResetCharacterPointMsgBox::SetButtonInfo()
{
	...
	m_ResetCharacterPointBtn.SetText(GlobalText[1884]); // "\xBD\xBA\xC5\xC8 \xC3\xCA\xB1\xE2\xC8\xAD"
...
void SEASON3B::CResetCharacterPointMsgBox::RenderTexts()
{
	...
	g_pRenderText->SetTextColor(255, 255, 255, 255);
	g_pRenderText->SetFont(g_hFontBold);
	sprintf(szText, "%s", GlobalText[1885]);
	g_pRenderText->RenderText(fPos_x, fPos_y, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);

	fPos_y += 25;
	g_pRenderText->SetFont(g_hFont);
	sprintf(szText, "%s", GlobalText[1886]);
	g_pRenderText->RenderText(fPos_x, fPos_y + 1 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER);
...
void SEASON3B::CGuild_ToPerson_Position::RenderButtons()
{
	...
	if (COMGEM::m_cGemType == COMGEM::CELE)
	{
		...
		SetLegacyColor4f(1.0f, 1.0f, 0.2f, 1.0f);
