	...
	for (y = 0; y < m_nRow; y++)
	{
		for (x = 0; x < m_nColumn; x++)
		{
			...
			if (m_pdwItemCheckBox[iCurSquareIndex] > 1)
			{
				...
				{
					BeginBitmap();
					SetLegacyColor4f(1.0, 1.0, 1.0, 0.3); //Bota isso antes da imagem
					...
					SetLegacyColor4f(1.0, 1.0, 1.0, 1.0); //Bota isso depois da imagem
					EndOpengl();
					...
					case 0:
						break;
					case 1:
						SetLegacyColor4f(1.0, 1.0, 0.0, 0.30000001);//SetLegacyColor4f(1.0, 1.0, 0.0, 0.60000001);
						break;
					case 2:
						SetLegacyColor4f(1.0, 0.66000003, 0.0, 0.30000001);//SetLegacyColor4f(1.0, 0.66000003, 0.0, 0.60000001);
						break;
					case 3:
						SetLegacyColor4f(0.89000001, 0.10900001, 0.10900001, 0.30000001);//SetLegacyColor4f(1.0, 0.33000001, 0.0, 0.60000001);
						break;
					case 4:
						SetLegacyColor4f(1.0, 0.0, 0.0, 0.40000001);
						break;
					case 5:
						SetLegacyColor4f(1.0, 0.2, 0.1, 0.40000001);
						break;
					default:
	...
	if( ms_pPickedItem )
	{
		...
		if( pickitemvisible )
		{
			...
			if(IntersectRect(&rcIntersect, &rcPickedItem, &rcInventory))
			{
				...
				{
					EnableAlphaTest();
					SetLegacyColor4f(1.f, 0.2f, 0.2f, 0.4f);
						...
						for(int y=0; y<nItemRow; y++)
						{
							for(int x=0; x<nItemColumn; x++)
							{
								...
								if(m_pdwItemCheckBox[iCurSquareIndex] > 1)
								{
									...
									SetLegacyColor4f(m_afColorStateWarning[0], m_afColorStateWarning[1], m_afColorStateWarning[2], 0.4f);
								}
								else
								{
									SetLegacyColor4f(m_afColorStateNormal[0], m_afColorStateNormal[1], m_afColorStateNormal[2], 0.4f);
								}
...
void SEASON3B::CNewUIInventoryCtrl::RenderNumberOfItem()
{
	...
	for(; li != m_vecItem.end(); li++)
	{
		...
		{
			SetLegacyColor3f(1.f,0.9f,0.7f);
...
void SEASON3B::CNewUIInventoryCtrl::BackupPickedItem()
{
	if(ms_pPickedItem && EquipmentItem == false)
	{
		...
		else if(pItemObj->ex_src_type == ITEM_EX_SRC_EQUIPMENT)
		{
			...
			// Only the helper slot owns a pet visual.  Running this handler for
			// weapons/armor clears or recreates a pet with unrelated item data.
