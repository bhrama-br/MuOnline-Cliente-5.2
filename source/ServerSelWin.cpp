...
void CServerSelWin::UpdateWhileActive(double dDeltaTick)
{
	...
	for( i=0 ; i<SSW_SERVER_G_MAX ; i++ )
	{
		if (m_aServerGroupBtn[i].IsClick())
		{
			...
			g_ErrorReport.Write("[ServerTrace] Server group selected: button=%d.\r\n", i);
	...
	for( i=0 ; i<m_icntServer ; i++ )
	{
		if (m_aServerBtn[i].IsClick())
		{
			...
			{
				g_ErrorReport.Write("[ServerTrace] Server selected: row=%d connect=%d load=%d.\r\n", i, pServerInfo->m_iConnectIndex, pServerInfo->m_iPercent);
				CUIMng::Instance().HideWin(this);

				g_ErrorReport.Write("[ServerTrace] Requesting game server address.\r\n");
				...
				g_ErrorReport.Write("[ServerTrace] Selected server state stored.\r\n");
