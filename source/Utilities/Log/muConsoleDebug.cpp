...
{
	// Debug output must never influence gameplay.  The old implementation used
	// an unbounded variadic vsprintf and faults when legacy call sites pass a
	// mismatched format/argument list.
	(void)iType;
	(void)pStr;
}
...
void CmuSimpleLog::log(char *str, ...)
{
	...
	if(m_bLogfirst) 
	{
		m_pFile = Platform::LegacyFileOpen(m_strFilename.c_str(), "a");
		fclose(m_pFile);
		m_bLogfirst = false;
	}
