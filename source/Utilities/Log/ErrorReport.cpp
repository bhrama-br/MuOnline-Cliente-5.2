...
#include "stdafx.h"
#ifdef _WIN32
// DirectDraw/DirectInput/DirectMusic: so existem no Windows e este arquivo
// nao usa nenhum tipo deles fora dos trechos ja guardados.
#include <ddraw.h>
#include <dinput.h>
#include <dmusicc.h>
#endif
...
#endif //_WIN32
#ifdef _WIN32
// eh.h (tratamento de excecao estruturada) e imagehlp.h (simbolos de depuracao)
// nao tem equivalente fora do Windows. O relatorio de crash com pilha continua
// sendo caminho Windows-only; nas outras plataformas a classe so registra texto.
#include <eh.h>
#include <imagehlp.h>
#endif
...
{
	// PetTrace was temporary diagnostic output added while investigating the
	// helper crash.  It must not pass through the legacy variadic formatter.
	if (lpszFormat != NULL && strncmp(lpszFormat, "[PetTrace]", 10) == 0)
	{
		return;
	}
	...
	_vsnprintf_s(lpszBuffer, sizeof(lpszBuffer), _TRUNCATE, lpszFormat, va);
	va_end( va);
