...
void SetLineColor(int iType, float fAlphaRate = 1.0f)
{
	...
	{
	case 0:
		SetLegacyColor4ub(146, 134, 121, ubWindowAlpha);	break;
	case 1:
		SetLegacyColor4ub(37, 37, 37, ubWindowAlpha);		break;
	case 2:
		SetLegacyColor4ub(106, 97, 88, ubWindowAlpha);		break;
	case 3:
		SetLegacyColor4ub(0, 0, 0, 179 * fAlphaRate);		break;
	case 4:
		SetLegacyColor4ub(173, 167, 150, ubWindowAlpha);	break;
	case 5:
		SetLegacyColor4ub(53, 49, 48, ubWindowAlpha);		break;
	case 6:
		SetLegacyColor4ub(26, 22, 21, ubWindowAlpha);		break;
	case 7:
		SetLegacyColor4ub(0, 0, 0, 255 * fAlphaRate);		break;
	case 8:
		SetLegacyColor4ub(153, 156, 166, ubWindowAlpha);	break;
	case 9:
		SetLegacyColor4ub(136, 138, 147, ubWindowAlpha);	break;
	case 10:
		SetLegacyColor4ub(83, 85, 93, ubWindowAlpha);		break;
	case 11:
		SetLegacyColor4ub(102, 104, 112, ubWindowAlpha);	break;
	case 12:
		SetLegacyColor4ub(0, 0, 8, ubWindowAlpha);			break;
	case 13:
		SetLegacyColor4ub(0, 0, 0, ubWindowAlpha);			break;
	case 14:
		SetLegacyColor4ub(185, 185, 185, ubWindowAlpha);	break;
	case 15:
		SetLegacyColor4ub(194, 194, 194, ubWindowAlpha);	break;
	case 16:
		SetLegacyColor4ub(194, 194, 194, ubWindowAlpha);	break;
	case 17:
		SetLegacyColor4ub(209, 188, 134, ubWindowAlpha);	break;
	case 18:
		SetLegacyColor4ub(205, 209, 133, ubWindowAlpha);	break;
	default:	break;
	}
}
...
void CUIBaseWindow::SetControlButtonColor(int iSelect)
{
	if (m_iControlButtonClick == iSelect && CheckMouseIn(m_iPos_x + m_iWidth - 38,m_iPos_y + 8, 38, 9) == TRUE)
	{
		if (MouseLButtonPush == true)
			SetLegacyColor4f(0.6f, 0.6f, 0.6f, 1.0f);
		else
			SetLegacyColor4f(1.0f, 1.0f, 1.0f, 1.0f);
	}
	else
	{
		SetLegacyColor4f(0.8f, 0.8f, 0.8f, 1.0f);
	}
}
...
void CUIBaseWindow::Render()
{
	...
	if (bBackWindow == TRUE)
		SetLegacyColor4f(0.5f, 0.5f, 0.5f, 1.0f);
...
//	m_PalListBox.AddText("\xC0\xCC\xB8\xA7\xB3\xD7\xC0\xDA", 1, 1);
//	m_PalListBox.AddText("\xC0\xCC\xB8\xA7\xB3\xCB\xC0\xDA", 1, 1);
//	m_PalListBox.AddText("\xC0\xCC\xB8\xA7\xBC\xF6\xB3\xDD", 1, 1);
//	m_PalListBox.AddText("\xC0\xCC\xB8\xA7""1\xC0\xDA", 1, 1);
//	m_PalListBox.AddText("\xC0\xCC\xB8\xA7""2\xC0\xDA", 1, 1);
//	m_PalListBox.AddText("\xC0\xCC\xB8\xA7""3\xB3\xDD", 1, 1);
//	m_PalListBox.AddText("\xC0\xCC\xB8\xA7""4\xC0\xDA", 1, 1);
//	m_PalListBox.AddText("\xC0\xCC\xB8\xA7""5\xC0\xDA", 1, 1);
//	m_PalListBox.AddText("\xC0\xCC\xB8\xA7""6\xB3\xDD", 1, 1);
//	m_PalListBox.AddText("\xC0\xCC\xB8\xA7""7\xB3\xDD", 1, 1);
//	m_PalListBox.AddText("\xC0\xCC\xB8\xA7""8\xB3\xDD", 1, 1);
//	m_PalListBox.AddText("\xC0\xCC\xB8\xA7""9\xB3\xDD", 1, 1);
...
CWsctlc * CUIChatWindow::GetCurrentSocket()
{
	...
	{
		assert(!"\xB9\xE6 \xBC\xD2\xC4\xCF \xB0\xCB\xBB\xF6 \xBD\xC7\xC6\xD0!!!");
		return NULL;
	}
}
...
void CUIPhotoViewer::SetID(const char * pszID)
{
	...
	sprintf(m_PhotoChar.ID, "%s", pszID);
}
...
void CUIPhotoViewer::Render()
{
	if (m_bIsWebzenMail == TRUE)
	{
        SetLegacyColor4f( 0.f, 0.f, 0.f, 1.0f );
	...
	if (CheckOption(UIPHOTOVIEWER_CANCONTROL))
	{
		...
		else
		{
			...
			TextNum = 0;
			sprintf(TextList[TextNum],"%s", GlobalText[997]);TextListColor[TextNum]=0;TextBold[TextNum]=false;TextNum++;
			sprintf(TextList[TextNum],"%s", GlobalText[998]);TextListColor[TextNum]=0;TextBold[TextNum]=false;TextNum++;
			sprintf(TextList[TextNum],"%s", GlobalText[999]);TextListColor[TextNum]=0;TextBold[TextNum]=false;TextNum++;
			SIZE TextSize;
			...
			case 4:		// ÆíÁö¾²±â
				{
					char temp[MAX_TEXT_LENGTH + 1];
					sprintf(temp, GlobalText[1071], g_cdwLetterCost);
					dwUIID = g_pWindowMgr->AddWindow(UIWNDTYPE_WRITELETTER, 100,100,temp);	// "\xC6\xED\xC1\xF6\xBE\xB2\xB1\xE2"
					if (dwUIID == 0) break;
