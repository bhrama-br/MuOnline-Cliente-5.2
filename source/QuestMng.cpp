...
void CQuestMng::LoadNPCDialogueScript()
{
	...
	// Com ::fopen o arquivo nunca abria no Web. Repasse direto no Windows.
	FILE* fp = Platform::LegacyFileOpen(QM_NPCDIALOGUE_FILE, "rb");
	if (fp == NULL)
	{
		char szMessage[256];
...
{
	FILE* fp = Platform::LegacyFileOpen(QM_QUESTPROGRESS_FILE, "rb");
	if (fp == NULL)
	{
		char szMessage[256];
...
{
	FILE* fp = Platform::LegacyFileOpen(QM_QUESTWORDS_FILE, "rb");
	if (fp == NULL)
	{
		char szMessage[256];
		::sprintf(szMessage, "%s file not found.\r\n", QM_QUESTWORDS_FILE);
