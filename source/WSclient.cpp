...
void ReceiveServerList(BYTE* ReceiveBuffer)
{
	...
	g_ErrorReport.Write("[ServerTrace] Server list received: count=%d.\r\n", MAKEWORD(Value2, Data->Value));
	...
	{
		g_ErrorReport.Write("[ServerTrace] Showing server selection.\r\n");
		rUIMng.ShowWin(&rUIMng.m_ServerSelWin);
		rUIMng.m_ServerSelWin.UpdateDisplay();
		g_ErrorReport.Write("[ServerTrace] Server selection display ready.\r\n");
...
void ReceiveServerConnect(BYTE* ReceiveBuffer) //Recebe informação do ConnectServer sobre a sala e envia a conexão para a sala escolhida
{
	...
	g_ErrorReport.Write("[ServerTrace] Server address received: %s:%d.\r\n", IP, Data->Port);
	SocketClient.Close();
	...
	if (CreateSocket(IP, Data->Port))
	{
		...
		g_ErrorReport.Write("[ServerTrace] Game server socket connected.\r\n");
	}
	else
	{
		g_ErrorReport.Write("[ServerTrace] Game server socket connection failed.\r\n");
	}
#else
	gProtocolSend.DisconnectServer();
...
BOOL ReceiveInventory(BYTE* ReceiveBuffer, BOOL bEncrypted)
{
	...
	int Offset = sizeof(PHEADER_DEFAULT_SUBCODE_WORD);
	g_ErrorReport.Write("[PetTrace] Inventory received: entries=%d.\r\n", Data->Value);
	DeleteBug(&Hero->Object);
	giPetManager::DeletePet(Hero);

	ThePetProcess().DeletePet(Hero);
	// The select-character scene can leave a custom helper view associated with
	// Hero.  It must not survive into an inventory packet that has no helper.
	...
	for (int i = 0; i < Data->Value; i++)
	{
		...
		{
			g_ErrorReport.Write("[PetTrace] Inventory visual slot=%d.\r\n", Data2->Index);
	...
	g_ErrorReport.Write("[PetTrace] Inventory complete: normal=%d visual=%d.\r\n", CharacterMachine->Equipment[EQUIPMENT_HELPER].Type, CharacterMachine->Equipment[NEW_EQUIPMENT_HELPER].Type);
	// Debug-console output uses an unsafe variadic formatter in this legacy
	// client.  This packet is received while its UI state is being rebuilt.
	// Do not format output here; the persistent error log above is sufficient.
...
void ReceiveChatKey(BYTE* ReceiveBuffer)
{
	...
	if (Hero->GuildStatus == G_MASTER && !strcmp(CharactersClient[Index].ID, "\xB1\xE6\xB5\xE5 \xB8\xB6\xBD\xBA\xC5\xCD"))
	{
...
void ReceiveChangePlayer(BYTE* ReceiveBuffer)
{
	...
	{
		// The server can acknowledge an equipment change after the viewport entry
		// has been removed. Never write past CharactersClient in that case.
		return;
	}
...
if ((Data->Item[1] >> 4) == 8)
{
	g_ErrorReport.Write("[PetTrace] Receive equipment change: key=%d index=%d type=%d element=%d\r\n", Key, characterIndex, Type, Data->ElementSlot);
}
...
void ReceiveGuildEndWar(BYTE* ReceiveBuffer)
{
	...
	switch (Data->Value)
	{
	case 0:sprintf(Text, "%s", GlobalText[527]); break;
	case 1:sprintf(Text, "%s", GlobalText[528]); Win = 2; break;
	case 2:sprintf(Text, "%s", GlobalText[529]); Win = 2; break;
	case 3:sprintf(Text, "%s", GlobalText[530]); break;
	case 4:sprintf(Text, "%s", GlobalText[531]); Win = 2; break;
	case 5:sprintf(Text, "%s", GlobalText[532]); break;
	case 6:sprintf(Text, "%s", GlobalText[480]); Win = 1; break;
...
void ReceiveMix(BYTE* ReceiveBuffer)
{
	...
	case 0:
	{
		...
		case SEASON3A::MIXTYPE_SEED_SPHERE:
			unicode::_sprintf(szText, "%s", GlobalText[594]);
			g_pChatListBox->AddText("", szText, SEASON3B::TYPE_ERROR_MESSAGE);
			break;
			// 			case SEASON3A::MIXTYPE_TRAINER:
			// 				unicode::_sprintf(szText, "%s", GlobalText[1208]);	// ºÎÈ° ½ÇÆÐ
			// 				g_pChatListBox->AddText("", szText, SEASON3B::TYPE_ERROR_MESSAGE);
			// 				break;
	...
	case 1:
	{
		...
		case SEASON3A::MIXTYPE_EXTRACT_SEED:
		case SEASON3A::MIXTYPE_SEED_SPHERE:
			unicode::_sprintf(szText, "%s", GlobalText[595]);
			g_pChatListBox->AddText("", szText, SEASON3B::TYPE_SYSTEM_MESSAGE);
			break;
			// 			case SEASON3A::MIXTYPE_TRAINER:
			// 				unicode::_sprintf(szText, "%s", GlobalText[1209]);
			// 				g_pChatListBox->AddText("", szText, SEASON3B::TYPE_SYSTEM_MESSAGE);
			// 				break;
		case SEASON3A::MIXTYPE_OSBOURNE:
...
void ReceiveDuelStart(BYTE* ReceiveBuffer)
{
	...
	{
		g_DuelMgr.SetDuelPlayer(DUEL_ENEMY, MAKEWORD(Data->bIndexL, Data->bIndexH), Data->szID);
		sprintf(szMessage, "%s", GlobalText[1811]);
		g_pChatListBox->AddText("", szMessage, SEASON3B::TYPE_ERROR_MESSAGE);
	}
}
...
void ReceiveCrownSwitchState(BYTE* ReceiveBuffer)
{
	...
	case 2:
	{
		...
		if (pMsgBox)
		{
			...
			{
				unicode::_sprintf(strText, "%s", GlobalText[1487]);
			}
...
bool ReceiveEnterDoppelGangerEvent(BYTE* ReceiveBuffer)
{
	...
	case 1:
		...
		break;
	case 2:
		unicode::_sprintf(szText, "%s", GlobalText[2864]);
		g_pChatListBox->AddText("", szText, SEASON3B::TYPE_ERROR_MESSAGE);
		g_pDoppelGangerWindow->LockEnterButton(TRUE);
		break;
	case 3:
		unicode::_sprintf(szText, "%s", GlobalText[2865]);
		g_pChatListBox->AddText("", szText, SEASON3B::TYPE_ERROR_MESSAGE);
		g_pDoppelGangerWindow->LockEnterButton(TRUE);
		break;
	case 4:
