...
{
	if(m_pButton == NULL || iIndex < 0 || iIndex >= m_iBtnNum)
	{
		return;
	}
...
{
	// This event window is not active during normal gameplay.  Processing its
	// Escape shortcut from the global UI update path can run after its event
