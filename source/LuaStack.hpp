...
#include <cstddef>   // std::size_t, usado pelos invocadores por posicao
...
#else
// Lua 5.2+ ja fornece luaL_setfuncs com a mesma semantica. luaL_mysetfuncs era
// chamada nos dois caminhos, mas so estava definida no de 5.1 — o que quebrava a
// compilacao ao trocar LuaJIT (API 5.1) por Lua padrao.
inline void luaL_mysetfuncs(lua_State* L, const luaL_Reg* l, int nup) {
...
namespace LUAAA_NS
{
	...
	struct LuaStack<unsigned long>
	{
		inline static unsigned long get(lua_State* L, unsigned long idx)
		{
			if (lua_isnumber(L, idx) || lua_isstring(L, idx))
			{
				// "unsigned long(x)" nao e cast funcional valido: o nome do tipo tem
				// mais de um token. So o MSVC aceita.
	...
	// DWORD e `unsigned long` no Win32 e `unsigned int` em LP64 (Android arm64).
	// Sem esta especializacao, funcoes que devolvem DWORD — CreateMonsterStruct,
	// por exemplo — deixavam de casar com qualquer LuaStack fora do Windows.
	template<>
	struct LuaStack<unsigned int>
	{
		inline static unsigned int get(lua_State* L, unsigned int idx)
		{
			if (lua_isnumber(L, idx) || lua_isstring(L, idx))
			{
...
#define IMPLEMENT_CALLBACK_INVOKER(CALLCONV) \
	...
	//========================================================
	// Leitura de argumentos independente da ordem de avaliacao
	//========================================================
	// Os invocadores abaixo liam a pilha com `LuaStack<ARGS>::get(state, idx--)...`
	// dentro da LISTA DE ARGUMENTOS de uma chamada. A ordem de avaliacao de
	// argumentos de funcao NAO e especificada em C++: o MSVC avalia da direita para a
	// esquerda, e nessa ordem o `idx--` produz o mapeamento pretendido (parametro p
	// recebe o indice p); o Clang avalia da esquerda para a direita e INVERTE todos
	// os argumentos.
	//
	// O sintoma ficava longe da causa. O script chama
	//
	//   LoadImage("Interface\\GB_Chest.tga", 40000)
	//
	// e o C++ recebia os dois trocados, entao o cliente reclamava de
	// "LoadBitmap Failed: Data\40000" -- um indice virando nome de arquivo. Vale para
	// TODA funcao Lua com dois ou mais parametros, nao apenas essa.
	//
	// Aqui o indice de cada parametro vem da POSICAO dele, via index sequence, e nao
	// de um contador mutavel: o resultado nao depende mais de ordem de avaliacao. Em
	// C++11 puro (sem std::index_sequence nem std::apply) porque nem toda
	// configuracao deste projeto compila em C++17.
	//
	// Os auxiliares ficam em escopo de namespace de proposito: os invocadores usam
	// uma `struct HelperClass` LOCAL, e classe local nao pode ter membro template.
	template<std::size_t ...I> struct LuaaaArgIndices {};

	template<std::size_t N, std::size_t ...I>
	struct LuaaaMakeArgIndices : LuaaaMakeArgIndices<N - 1, N - 1, I...> {};

	template<std::size_t ...I>
	struct LuaaaMakeArgIndices<0, I...> { typedef LuaaaArgIndices<I...> type; };

	// `base` e quantos lugares da pilha vem ANTES do primeiro argumento: 0 para
	// funcao livre, 1 para metodo (o objeto ocupa o indice 1).
	template<typename TRET, typename ...ARGS>
	struct LuaaaFreeInvoker
	{
		template<typename FTYPE, std::size_t ...I>
		static TRET Invoke(FTYPE func, lua_State* state, int base, LuaaaArgIndices<I...>)
		{
			return func(LuaStack<ARGS>::get(state, base + 1 + (int)I)...);
		}
	};

	template<typename TRET, typename ...ARGS>
	struct LuaaaMemberInvoker
	{
		template<typename TOBJ, typename FTYPE, std::size_t ...I>
		static TRET Invoke(TOBJ&& obj, FTYPE func, lua_State* state, int base, LuaaaArgIndices<I...>)
		{
			return (obj.*func)(LuaStack<ARGS>::get(state, base + 1 + (int)I)...);
		}
	};

	template<typename TCLASS, typename ...ARGS>
	struct LuaaaCtorInvoker
	{
		template<std::size_t ...I>
		static TCLASS* Invoke(lua_State* state, LuaaaArgIndices<I...>)
		{
			return new TCLASS(LuaStack<ARGS>::get(state, 1 + (int)I)...);
		}
	};
	...
	{ \
		...
		{ \
			...
			{ \
				...
				if (calleePtr) \
				{ \
					LuaStackReturn<TRET>(state, LuaaaFreeInvoker<TRET, ARGS...>::Invoke( \
						*(FTYPE*)(calleePtr), state, SKIPPARAM, \
						typename LuaaaMakeArgIndices<sizeof...(ARGS)>::type())); \
					return 1; \
				} \
				return 0; \
			} \
	...
	{ \
		...
		{ \
			...
			{ \
				...
				if (calleePtr) \
				{ \
					LuaaaFreeInvoker<void, ARGS...>::Invoke( \
						*(FTYPE*)(calleePtr), state, SKIPPARAM, \
						typename LuaaaMakeArgIndices<sizeof...(ARGS)>::type()); \
				} \
				return 0; \
...
#	undef _NOTHING		
	...
	lua_CFunction MemberFunctionCaller(TRET(TCLASS::* func)(ARGS...))
	{
		...
		struct HelperClass
		{
			static int Invoke(lua_State* state)
			{
				...
				if (calleePtr)
				{
					LuaStackReturn<TRET>(state, LuaaaMemberInvoker<TRET, ARGS...>::Invoke(
						LuaStack<TCLASS>::get(state, 1), *(FTYPE*)(calleePtr), state, 1,
						typename LuaaaMakeArgIndices<sizeof...(ARGS)>::type()));
					return 1;
				}
				return 0;
	...
	lua_CFunction MemberFunctionCaller(void(TCLASS::* func)(ARGS...))
	{
		...
		struct HelperClass
		{
			static int Invoke(lua_State* state)
			{
				...
				if (calleePtr)
				{
					LuaaaMemberInvoker<void, ARGS...>::Invoke(
						LuaStack<TCLASS>::get(state, 1), *(FTYPE*)(calleePtr), state, 1,
						typename LuaaaMakeArgIndices<sizeof...(ARGS)>::type());
				}
				return 0;
	...
	struct ConstructorCaller
	{
		...
		{
			(void)(state);
			return LuaaaCtorInvoker<TCLASS, ARGS...>::Invoke(
				state, typename LuaaaMakeArgIndices<sizeof...(ARGS)>::type());
		}
	};
}
