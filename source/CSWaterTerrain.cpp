...
void    CSWaterTerrain::Render ( void )
{
    ...
    renderer.Color4f ( 0.2f, 0.5f, 0.65f, 1.f );
	for ( j=0; j<m_iTriangleListNum; j++ )
	{
        offset = m_iTriangleList[j];
		renderer.TexCoord2f ( g_chrome[offset][1], g_chrome[offset][0] );
        renderer.Vertex3fv ( m_Vertices[offset] );
	}
	renderer.End();
    EnableAlphaBlend ();
	...
	for ( j=0; j<m_iTriangleListNum; j++ )
	{
        ...
        renderer.Color4f( alpha, alpha*2.5f, alpha*3.f, 1.f );//, alpha );
		renderer.TexCoord2f ( g_chrome[offset][1], g_chrome[offset][0] );
        renderer.Vertex3fv ( m_Vertices[offset] );
	}
	renderer.End();
...
{
	if(Alpha==1.f)
     	SetLegacyColor3fv(Light);
	else
     	SetLegacyColor4f(Light[0],Light[1],Light[2],Alpha);
...
void    CSWaterTerrain::RenderWaterBitmapTile(float xf,float yf,float lodf,int lodi,vec3_t c[4],bool LightEnable,float Alpha,float Height)
{
	...
	for(int i=0;i<4;i++)
	{
		if(LightEnable)
		{
			// glColor3fv implica alpha 1.0 no caminho Alpha==1.f.
