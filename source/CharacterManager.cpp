...
{
	// Subtracao de PONTEIROS: com (int) o endereco truncava em 64 bits e o
	// indice saia errado. ptrdiff_t e o tipo natural da diferenca.
	return (int)((ptrdiff_t)((char*)pChar - (char*)&CharactersClient[0]) / (ptrdiff_t)sizeof(CHARACTER));
}
...
const char* CCharacterManager::GetCharacterClassText(const BYTE byClass)
{
	...
	else if(byCharacterClass == CLASS_LORDEMPEROR)
	{
		...
		return GlobalText[3150];	// 3150 "\xB7\xB9\xC0\xCC\xC1\xF6\xC6\xC4\xC0\xCC\xC5\xCD"
	else if(byCharacterClass == CLASS_TEMPLENIGHT)
		return GlobalText[3151];	// 3151 "\xC5\xDB\xC7\xC3\xB3\xAA\xC0\xCC\xC6\xAE"
#endif //PBG_ADD_NEWCHAR_MONK
