...
public:
	...
	bool Load(const std::string& strFilePath, DWORD dwLoadDisposition)
	{
		...
		FILE* fp = Platform::LegacyFileOpen(strFilePath.c_str(), "rb");
		if(NULL == fp)
			return false;
		...
		{
			fclose(fp);   // faltava: este retorno vazava o descritor
			return false;
		}
	...
	{
		FILE* fp = Platform::LegacyFileOpen(strFilePath.c_str(), "wb");
		if(NULL == fp)
			return false;
