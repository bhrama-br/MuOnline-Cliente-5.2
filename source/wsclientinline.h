...
__forceinline bool SendRequestEquipmentItem(int iSrcType,int iSrcIndex, ITEM* pItem, int iDstType,int iDstIndex)
{
	...
	// The legacy console formatter is variadic and unsafe for the mixed item
	// fields above.  Avoid it on equipment transfers; packet data is unchanged.
...
{\
	...
	if( RelationType == 0x01 && RequestType == 0x01 )\
		...
		sprintf(szTmp,"%s", GlobalText[1387]);\
