...
#include "Widescreen.h"   // g_fScreenRate_x: antes era declarado com um
...
extern int MouseY;
                          // `extern` dentro das funcoes-membro, o que no
                          // padrao nomeia SEASON3B::g_fScreenRate_x e nao a
                          // variavel global. O MSVC tolerava.
...
bool SEASON3B::CNewUIChatLogWindow::RenderBackground()
{
	if (m_bShowFrame)
	{
		...
		EnableAlphaTest();
		SetLegacyColor4f(0.0f, 0.0f, 0.0f, (GetBackAlpha() > 0.8f) ? 0.8f : GetBackAlpha());
