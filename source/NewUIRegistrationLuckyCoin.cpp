...
namespace SEASON3B
{
	...
	void CNewUIRegistrationLuckyCoin::SetPos( int x, int y )
	{
		m_Pos.x = x; 
		m_Pos.y = y;
		const float btnX = m_Pos.x + LUCKYCOIN_REG_WIDTH / 2.0f - MSGBOX_BTN_EMPTY_SMALL_WIDTH / 2.0f;
		m_RegistButton.ChangeButtonInfo(btnX, m_Pos.y + LUCKYCOIN_REG_HEIGHT - 220, m_width, m_height);
		m_CloseButton.ChangeButtonInfo(btnX, m_Pos.y + 360, m_width, m_height);
	}
