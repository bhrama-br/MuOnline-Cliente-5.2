    ...
    if((EditFlag!=EDIT_NONE) || (EditFlag==EDIT_NONE&&o->HiddenMesh!=-2))
	{
		if (ExtraMon != 10)
		{
			if (o->Type >= gItemManager.GET_ITEM_MODEL(18, 0) && o->Type <= gItemManager.GET_ITEM_MODEL(18, 254) || o->Type == 349) 
			{
				...
				if (b) 
				{
					if (gPatente.PatentModelRender((DWORD_PTR)(b), (DWORD_PTR)(o), o->Type) != 0)
					{
						return;
					}
				}
			...
			else if (o->Type >= gItemManager.GET_ITEM_MODEL(16, 255) && o->Type < gItemManager.GET_ITEM_MODEL(17, 0))
			{
				...
				if (b)
				{
					gDarkSpirit.RenderModel((DWORD_PTR)b, (DWORD_PTR)o, o->Type);
					return;
				}
			}
		}
        ...
        else if ( o->Type==MODEL_DARK_HORSE )
        {
			...
			if ( gMapManager.WorldActive != WD_10HEAVEN && gMapManager.InHellas()==false )
			{
				if(!g_Direction.m_CKanturu.IsMayaScene())
				{
					...
					{
						SetLegacyColor4f(0.f,0.f,0.f,0.2f);
					}
					else
					{
   						SetLegacyColor4f(0.f,0.f,0.f,0.7f);
					}
        ...
        else if ( o->Type==MODEL_DARK_SPIRIT )
        {
            ...
            if ( gMapManager.WorldActive!=WD_10HEAVEN && gMapManager.InHellas()==false )
            {
				if(!g_Direction.m_CKanturu.IsMayaScene())
				{
					...
					{
						SetLegacyColor4f(0.f,0.f,0.f,1.f);
					}
					VectorCopy(o->Position,Position);
...
{
	FILE *fp = Platform::LegacyFileOpen(FileName,"wt");
    fprintf(fp,"0\n");
	for(int i=0;i<16;i++)
	{
		for(int j=0;j<16;j++)
...
//     }
	...
	else if ( Type==MODEL_SWORD+31 && !(RenderType & RENDER_DOPPELGANGER))
	{
		...
		VectorCopy(b->BodyLight, Light);
		b->BeginRender(1.f);
		SetLegacyColor3f(b->BodyLight[0],b->BodyLight[1],b->BodyLight[2]);
	...
	else if(Type == MODEL_SWORD+20)
	{
		...
		b->RenderMesh(1,RENDER_TEXTURE,Alpha,1,o->BlendMeshLight,o->BlendMeshTexCoordU,o->BlendMeshTexCoordV);
		//. ³¯
		SetLegacyColor3f(0.43f,0.14f,0.6f);
	...
	{
		b->BeginRender(1.f);
		SetLegacyColor4f(b->BodyLight[0],b->BodyLight[1],b->BodyLight[2], 0.8f);
		b->RenderMesh(1,RENDER_TEXTURE,0.8f,-1,o->BlendMeshLight,o->BlendMeshTexCoordU,o->BlendMeshTexCoordV);
		SetLegacyColor4f(b->BodyLight[0],b->BodyLight[1],b->BodyLight[2], 0.5f);
...
void RenderPartObjectEdge(BMD *b,OBJECT *o,int Flag,bool Translate,float Scale)
{
    ...
    if(o->Type == MODEL_WARCRAFT)
    {
		...
		{
			SetLegacyColor4f(b->BodyLight[0],b->BodyLight[1],b->BodyLight[2],o->Alpha);
		}
...
void RenderBoundingBox(OBJECT* pObj)
{
	...
	// O adapter usa matrizes proprias: sincroniza a modelview transformada acima.
	...
	renderer.TexCoord2f( 1.0F, 1.0F); renderer.Vertex3fv(TransformVertices[7]);
	renderer.TexCoord2f( 1.0F, 0.0F); renderer.Vertex3fv(TransformVertices[6]);
	renderer.TexCoord2f( 0.0F, 0.0F); renderer.Vertex3fv(TransformVertices[4]);
	renderer.TexCoord2f( 0.0F, 1.0F); renderer.Vertex3fv(TransformVertices[5]);

	renderer.Color4f(0.2f,0.2f,0.2f,1.f);
	renderer.TexCoord2f( 0.0F, 1.0F); renderer.Vertex3fv(TransformVertices[0]);
	renderer.TexCoord2f( 1.0F, 1.0F); renderer.Vertex3fv(TransformVertices[2]);
	renderer.TexCoord2f( 1.0F, 0.0F); renderer.Vertex3fv(TransformVertices[3]);
	renderer.TexCoord2f( 0.0F, 0.0F); renderer.Vertex3fv(TransformVertices[1]);

	renderer.Color4f(0.6f,0.6f,0.6f,1.f);
	renderer.TexCoord2f( 1.0F, 1.0F); renderer.Vertex3fv(TransformVertices[7]);
	renderer.TexCoord2f( 1.0F, 0.0F); renderer.Vertex3fv(TransformVertices[3]);
	renderer.TexCoord2f( 0.0F, 0.0F); renderer.Vertex3fv(TransformVertices[2]);
	renderer.TexCoord2f( 0.0F, 1.0F); renderer.Vertex3fv(TransformVertices[6]);

	renderer.Color4f(0.6f,0.6f,0.6f,1.f);
	renderer.TexCoord2f( 0.0F, 1.0F); renderer.Vertex3fv(TransformVertices[0]);
	renderer.TexCoord2f( 1.0F, 1.0F); renderer.Vertex3fv(TransformVertices[1]);
	renderer.TexCoord2f( 1.0F, 0.0F); renderer.Vertex3fv(TransformVertices[5]);
	renderer.TexCoord2f( 0.0F, 0.0F); renderer.Vertex3fv(TransformVertices[4]);
	...
	renderer.TexCoord2f( 1.0F, 0.0F); renderer.Vertex3fv(TransformVertices[5]);
	renderer.TexCoord2f( 0.0F, 0.0F); renderer.Vertex3fv(TransformVertices[1]);
	renderer.TexCoord2f( 0.0F, 1.0F); renderer.Vertex3fv(TransformVertices[3]);

	renderer.Color4f(0.4f,0.4f,0.4f,1.f);
	renderer.TexCoord2f( 0.0F, 1.0F); renderer.Vertex3fv(TransformVertices[0]);
	renderer.TexCoord2f( 1.0F, 1.0F); renderer.Vertex3fv(TransformVertices[4]);
	renderer.TexCoord2f( 1.0F, 0.0F); renderer.Vertex3fv(TransformVertices[6]);
	renderer.TexCoord2f( 0.0F, 0.0F); renderer.Vertex3fv(TransformVertices[2]);
	renderer.End();
