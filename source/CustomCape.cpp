...
void CCustomCape::RenderModel(BMD* b, OBJECT* o, DWORD ItemIndex)
{
	...
	if (b->BodyLight[0] == 1.0 && b->BodyLight[1] == 1.0 && b->BodyLight[2] == 1.0)
	{
		...
		// "j" no lugar de "i" nos dois primeiros: sao PONTEIROS. Lua.cpp le 'i'
	// como va_arg(int) e 'j' como va_arg(unsigned long); com "i" o endereco
	// truncava em 64 bits. Os dois empilham lua_pushnumber, entao o script Lua
	// nao percebe diferenca, e em 32 bits o valor e identico ao de antes.
	gCustomCape.m_Lua.Generic_Call("RenderCapeModel", "jjii>", (DWORD_PTR)b, (DWORD_PTR)o, ItemIndex, renderCharacter);
}
...
{
	this->m_Lua.Generic_Call("CreateCapePlayer", "jjii>", (DWORD_PTR)pCloth, (DWORD_PTR)&character->Object, ItemIndex, Class);
}
