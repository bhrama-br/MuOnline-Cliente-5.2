...
// Declaracoes icadas para escopo de ARQUIVO: dentro de funcoes membro de
// SEASON3B, um `extern` nomeia SEASON3B::X em vez do global ::X.
extern int MouseY;
...
void SEASON3B::CNewUIGuildInfoWindow::Render_Text()
{
	...
	SetLegacyColor4f(0.6f, 0.6f, 0.6f, 1.f);
	...
	if (m_nCurrentTab == 0)
	{
		...
		sprintf(Text, "%s", GlobalText[1323]);
...
void SEASON3B::CNewUIGuildInfoWindow::Render_Guild_History()
{
	...
	SetLegacyColor4f(0.0f, 0.0f, 0.0f, 0.4f);
...
void SEASON3B::CNewUIGuildInfoWindow::Render_Guild_Info()
{
	...
	{
		SetLegacyColor4ub(21, 21, 21, 150);
		RenderColor(m_Pos.x + 12, m_Pos.y + 12 + 113, 165, 85);

		SetLegacyColor4ub(59, 59, 59, 150);
	...
	else
	{
		...
		if (m_MessageInfo.s_byRelationShipType == 0x01)			// Union
		{
			if (m_MessageInfo.s_byRelationShipRequestType == 0x01)	// Join
			{
				...
				sprintf(szText[1], "%s", GlobalText[1281]);
				sprintf(szText[2], "%s", GlobalText[1283]);
			}
			else										// Break Off
			{
				sprintf(szText[0], GlobalText[1280], pPlayer->ID);
				sprintf(szText[1], "%s", GlobalText[1282]);
				sprintf(szText[2], "%s", GlobalText[1283]);
			}
		}
		else if (m_MessageInfo.s_byRelationShipType == 0x02)		// Rival
		{
			if (m_MessageInfo.s_byRelationShipRequestType == 0x01)	// Join
			{
				...
				sprintf(szText[1], "%s", GlobalText[1286]);
				sprintf(szText[2], "%s", GlobalText[1283]);
			}
			else
			{
				sprintf(szText[0], GlobalText[1284], pPlayer->ID);
				sprintf(szText[1], "%s", GlobalText[1285]);
				sprintf(szText[2], "%s", GlobalText[1283]);
			}
		}
