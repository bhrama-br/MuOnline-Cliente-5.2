...
void SEASON3B::CNewUICursedTempleResult::RenderResultPanel()
{
	...
	::SetLegacyColor4f(1.0f, 1.0f, 1.0f, m_ResultEffectAlph);
...
void SEASON3B::CNewUICursedTempleResult::RenderTextLine( const CursedTempleGameResult& resultinfo, int x, int y, DWORD color, DWORD backcolor )
{
	...
	{
		unicode::_sprintf( Text, "%s", GlobalText[2387] );
	}
	else
	{
		unicode::_sprintf( Text, "%s", GlobalText[2388] );
	}
	...
	memset(&Text, 0, sizeof(unicode::t_char)*200);
	sprintf( Text, "%s", gCharacterManager.GetCharacterClassText(resultinfo.s_class));
...
void SEASON3B::CNewUICursedTempleResult::RenderText()
{
	...
	memset(&Text, 0, sizeof(unicode::t_char)*200);
	unicode::_sprintf( Text, "%s", GlobalText[2414]);
	...
	memset(&Text, 0, sizeof(unicode::t_char)*200);
	sprintf( Text, "%s", GlobalText[2416]);
