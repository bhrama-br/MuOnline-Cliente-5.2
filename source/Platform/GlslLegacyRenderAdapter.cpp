...
#define CHECK_GL(name) if (name == NULL) return "gl" #name " nao resolvida";
            CHECK_GL(AttachShader); CHECK_GL(ActiveTexture); CHECK_GL(BindBuffer); CHECK_GL(BindVertexArray); CHECK_GL(BufferData);
            CHECK_GL(CompileShader); CHECK_GL(CreateProgram); CHECK_GL(CreateShader); CHECK_GL(DeleteBuffers);
            CHECK_GL(DeleteProgram); CHECK_GL(DeleteShader); CHECK_GL(DeleteVertexArrays); CHECK_GL(DrawArrays);
...
#endif
            LOAD_GL(AttachShader); LOAD_GL(ActiveTexture); LOAD_GL(BindBuffer); LOAD_GL(BindVertexArray); LOAD_GL(BufferData);
            LOAD_GL(CompileShader); LOAD_GL(CreateProgram); LOAD_GL(CreateShader); LOAD_GL(DeleteBuffers);
            LOAD_GL(DeleteProgram); LOAD_GL(DeleteShader); LOAD_GL(DeleteVertexArrays); LOAD_GL(DrawArrays);
...
#undef LOAD_GL
            return AttachShader != NULL && ActiveTexture != NULL && BindBuffer != NULL && BindVertexArray != NULL && BufferData != NULL &&
                CompileShader != NULL && CreateProgram != NULL && CreateShader != NULL && DeleteBuffers != NULL &&
                DeleteProgram != NULL && DeleteShader != NULL && DeleteVertexArrays != NULL && DrawArrays != NULL &&
...
#define glVertexAttribPointer g_ModernGl.VertexAttribPointer
    ...
    public:
        GlslLegacyRenderAdapter()
            : m_primitive(Platform::LegacyPrimitiveQuads), m_program(0), m_vertexBuffer(0), m_vertexArray(0),
              ...
              m_alphaTestEnabled(false), m_texture2DEnabled(false), m_texture(0), m_failed(false), m_logged(false)
        {
        ...
        virtual void End()
        {
            ...
            const int usarTextura = (m_texture != 0) ? 1 : 0;
            ...
            glBufferData(GL_ARRAY_BUFFER, static_cast<GLsizeiptr>(drawVertices.size() * sizeof(LegacyVertex)), &drawVertices[0], GL_STREAM_DRAW);
        ...
        virtual void BindTexture(unsigned int texture) { m_texture = static_cast<GLuint>(texture); }
