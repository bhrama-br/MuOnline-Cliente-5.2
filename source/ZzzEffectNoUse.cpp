...
void RenderPlane(int Texture, vec3_t Position, float Scale, float Angle)
{
    ...
    // O adapter usa matrizes proprias: sincroniza a modelview ja transformada
    // pelo glTranslatef/glRotatef acima antes de emitir a geometria.
    ...
    renderer.TexCoord2f(0.f, 1.f); renderer.Vertex3fv(BoundingVertices[0]);
    renderer.TexCoord2f(1.f, 1.f); renderer.Vertex3fv(BoundingVertices[2]);
    renderer.TexCoord2f(1.f, 0.f); renderer.Vertex3fv(BoundingVertices[3]);
    renderer.TexCoord2f(0.f, 0.f); renderer.Vertex3fv(BoundingVertices[1]);
...
void RenderPlanes()
{
    for (int i = 0; i < MAX_PLANES; i++)
    {
        ...
        if (o->Live)
        {
            SetLegacyColor3f(o->Light[0], o->Light[1], o->Light[2]);
...
void RenderShpere(int Type, vec3_t ShperePosition, float Scale, vec3_t ShpereLight, float Rotation, float TextureV)
{
    ...
    for (float y = 0.f; y < Height; y += 1.f)
    {
        for (float x = 0.f; x < Width; x += 1.f)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                renderer.TexCoord2f(UV[i][0], UV[i][1] + TextureV);
...
void MagicBitmap(int Texture)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, (int)b->Width, (int)b->Height, 0, GL_RGB, GL_UNSIGNED_BYTE, b->Buffer);
}
...
void RenderMagicBox(int Texture, vec3_t Position, float Angle)
{
    ...
    Platform::ILegacyRenderAdapter& renderer = Platform::GetLegacyRenderAdapter();
    renderer.Color4f(Color, Color, Color, 1.f);
    renderer.Begin(Platform::LegacyPrimitiveQuads);
...
void RenderSky3()
{
    ...
    for (float x = 0.f; x < Num; x += 1.f)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            renderer.Color4f(Light[i][0], Light[i][1], Light[i][2], 1.f);
            renderer.TexCoord2f(UV[i][0], UV[i][1]);
            renderer.Vertex3f(Position[i][0], Position[i][1], Position[i][2] + 100.f);
        }
        renderer.End();
    }
}
...
void RenderBackLight(OBJECT* o, vec3_t Position, vec3_t Light1, vec3_t Light2)
{
    ...
    renderer.Color4f(Light1[0] * light, Light1[1] * light, Light1[2] * light, 1.f);
    for (int i = 0; i < iVertex; i++)
    {
        offset = g_backLightIndex[i];

        renderer.TexCoord2f(g_U[offset], g_V[offset]);
        renderer.Vertex3f(position[0] + g_vertX[offset], position[1] + g_vertY[offset], position[2] + g_vertZ[offset]);
    }
    renderer.End();

    renderer.Begin(Platform::LegacyPrimitiveTriangles);
    renderer.Color4f(Light2[0], Light2[1], Light2[2], 1.f);
    ...
    for (int i = 0; i < iVertex; i++)
    {
        ...
        renderer.TexCoord2f(g_U[offset], g_V[offset]);
        renderer.Vertex3f(position[0] + g_vertX2[offset], position[1] + g_vertY[offset], position[2] + g_vertZ[offset]);
    }
    renderer.End();
