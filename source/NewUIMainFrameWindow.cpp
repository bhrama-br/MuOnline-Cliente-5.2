...
{
	// The browser can finish resizing its canvas after the HUD is created.
	// Keep these controls anchored to the same dynamic HUD coordinates as PC.
	const int frame_cx = GetCenterX(640);
	const int y_Next = setPosDown(452);
	const int button_size = 22;
	m_BtnParty.ChangeButtonInfo(frame_cx + 349, y_Next, button_size, button_size);
	m_BtnChaInfo.ChangeButtonInfo(frame_cx + 380, y_Next, button_size, button_size);
	m_BtnMyInven.ChangeButtonInfo(frame_cx + 412, y_Next, button_size, button_size);
	m_BtnGuild.ChangeButtonInfo(frame_cx + 92, y_Next, button_size, button_size);
#ifndef PBG_ADD_INGAMESHOP_UI_MAINFRAME
	m_BtnCShop.ChangeButtonInfo(frame_cx + 489, y_Next, button_size, button_size);
#endif
	EnableAlphaTest();
...
void SEASON3B::CNewUIMainFrameWindow::RenderFrame()
{
	...
	EnableAlphaTest();
	SetLegacyColor3f(0.60000002, 0.60000002, 0.60000002);
...
void SEASON3B::CNewUIMainFrameWindow::RenderLifeMana()
{
	...
	SetLegacyColor3f(0.91000003, 0.81, 0.60000002);
...
void SEASON3B::CNewUIMainFrameWindow::RenderExperience()
{
	...
	if (m_bExpEffect == true)
	{
		...
		if (m_dwPreExp < dwPriorExperience)
		{
			...
			SetLegacyColor4f(1.f, 1.f, 1.f, 0.4f);
...
/*¹ÚÁ¾ÈÆ*/
	...
	if(iHotKey != -1)
	{
		SetLegacyColor3f(1.f, 0.9f, 0.8f);
...
void SEASON3B::CNewUISkillList::RenderSkillDelay(int iIndex, float x, float y, float width, float height)
{
    ...
    if ( iSkillDelay > 0 )
    {
		...
		EnableAlphaTest();
        SetLegacyColor4f(1.f, 0.5f, 0.5f, 0.5f);
