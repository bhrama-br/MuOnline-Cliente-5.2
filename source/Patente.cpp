...
{
	// Declarado antes do `goto CALLFUNC`: saltar por cima de uma inicializacao
	// e erro em C++, e o clang o aplica.
	decltype(gPatente.GetPlayer(character->Key)) lpInfo = nullptr;
	...
	lpInfo = gPatente.GetPlayer(character->Key);
...
int CPatente::PatentModelRender(DWORD_PTR BMDStruct, DWORD_PTR ObjectStruct, DWORD ItemIndex)
{
	int result = 0;
