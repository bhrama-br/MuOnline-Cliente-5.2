...
bool SEASON3B::CNewUI3DCamera::Render()
{
	...
	// Web canvas sizing finishes after the UI is constructed.  The 3D item
	// camera used to retain that provisional viewport, while item positions
	// were calculated from the final window size.  Keep both spaces aligned.
	if (m_uiWidth != (UINT)WindowWidth || m_uiHeight != (UINT)WindowHeight)
	{
		m_uiWidth = WindowWidth;
		m_uiHeight = WindowHeight;
	}
