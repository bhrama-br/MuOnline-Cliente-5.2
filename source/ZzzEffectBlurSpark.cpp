            ...
            if (b->Number >= 2)
            {
                ...
                for (int j = 0; j < b->Number - 1; j++)
                {
                    ...
                    if (b->Owner->Level == 0)
                        ...
                        Light = 1.f;
                    renderer.Color4f(b->Light[0] * Light, b->Light[1] * Light, b->Light[2] * Light, 1.f);
                    TexU = (j) / (float)b->Number;
                    renderer.TexCoord2f(TexU, 1.f); renderer.Vertex3fv(b->p1[j]);
                    renderer.TexCoord2f(TexU, 0.f); renderer.Vertex3fv(b->p2[j]);
                    if (b->Owner->Level == 0)
                    ...
                    renderer.TexCoord2f(TexU, 0.f); renderer.Vertex3fv(b->p2[j + 1]);
                    renderer.TexCoord2f(TexU, 1.f); renderer.Vertex3fv(b->p1[j + 1]);
                    renderer.End();
                }
            }
        }
...
void RenderFlagFace(OBJECT* o, int x, int y, vec3_t Light, int Tex1, int Tex2)
{
    ...
    for (int i = 0; i < n; i++)
    {
        int vlist = f->vlist[i];
        physics_vertex* v = &flag_vertex[vlist];
        renderer.TexCoord2f(TexCoord[i][0], TexCoord[i][1]);
        renderer.Color4f(Light[0] * v->light, Light[1] * v->light, Light[2] * v->light, 1.f);
        vec3_t p, Position;
        Vector(v->p[0] + 9.f, v->p[1] - 12.f, v->p[2] - 35.f, p);
        Models[o->Type].TransformPosition(o->BoneTransform[19], p, Position, true);
        renderer.Vertex3f(Position[0], Position[1], Position[2]);
    }
    renderer.End();
