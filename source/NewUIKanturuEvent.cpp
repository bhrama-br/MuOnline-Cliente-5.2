...
void SEASON3B::CNewUIKanturu2ndEnterNpc::SetPos(int x, int y)
{
	...
	// The buttons keep absolute hit rectangles.  Keep them bound to the
	// window whenever the web canvas is re-anchored.
...
void SEASON3B::CNewUIKanturu2ndEnterNpc::ReceiveKanturu3rdInfo(BYTE btState, BYTE btDetailState, BYTE btEnter, BYTE btUserCount, int iRemainTime)
{
	...
	else if(btState == KANTURU_STATE_STANDBY)
	{
		...
		{
			unicode::_sprintf(m_strStateText[0], "%s", GlobalText[2162]);
		}
		unicode::_sprintf(m_strStateText[1], "%s", GlobalText[2160]);
		unicode::_sprintf(m_strStateText[2], "%s", GlobalText[2161]);
		m_iStateTextNum = 3;
	}
	else
	{
...
void SEASON3B::CNewUIKanturuInfoWindow::RenderInfo()
{
	...
	SetLegacyColor3f( 134.f/255.f, 134.f/255.f, 199.f/255.f );
