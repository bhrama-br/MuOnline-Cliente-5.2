	...
	if(c->Object.Type==MODEL_PLAYER)
	{
		...
		else
		{
			...
			else
			{
				...
				{
                //  ą«±â°¢§ ľř°Ĺ©©Ş. şí·ŻŠŰÄ©©¢©¢©ŔĚ ľĆ´Ń ľČŔüÁö´ë ŔĎ¶§
			    if ( ( c->Weapon[0].Type==-1 && c->Weapon[1].Type==-1 ) || ( c->SafeZone && ( gMapManager.InBloodCastle() == false ) ) )
				{
					...
					else if (gCharacterManager.GetBaseClass(c->Class) == CLASS_SUMMONER && !gMapManager.InChaosCastle())	// ĽŇČŻĽú»ç´Â Ä«żŔ¢©şÄ©©¢©¢©ż¢§Ľ± ©©¢­ŔÚ ľÖ´Ď¸ŢŔĚĽÇ.
...
void SetPlayerWalk(CHARACTER *c)
{
	...
	if(c->Object.Type==MODEL_PLAYER)
	{
		...
		else if ((c->Helper.Type == MODEL_HELPER + 3 || gHelperSystem.CheckHelperType(c->Helper.Type, 4)) && !c->SafeZone)   //  Ćä°¢§¢©ĂľĆ¸¦ Ĺ¸°íŔÖŔ¢©.
		{
            if ( gMapManager.WorldActive==WD_8TARKAN || gMapManager.WorldActive==WD_10HEAVEN || g_Direction.m_CKanturu.IsMayaScene() )
            {
				// ľÖ´Ď¸ŢŔĚĽÇ Ć¢¨´Â°Ĺ¶§ą®ż¢§ ľĆżą ¸·ľĆąö¸¢­
//                if(c->Weapon[0].Type==-1 && c->Weapon[1].Type==-1)
...
void AttackEffect(CHARACTER* c)
{
	...
	if (c->TargetCharacter >= 0 && c->TargetCharacter < MAX_CHARACTERS_CLIENT)
	{
		...
		if ((c->Skill) == AT_SKILL_ENERGYBALL)
		{
			...
			case 87:	//. ŔÚŔĚľ©¢Ć®żŔ°Ĺ1
			case 93:	//. ŔÚŔĚľ©¢Ć®żŔ°Ĺ2
			case 99:	//. ŔÚŔĚľ©¢Ć®żŔ°Ĺ3
			case 116:	//. ŔÚŔĚľ©¢Ć®żŔ°Ĺ4
			case 122:	//. ŔÚŔĚľ©¢Ć®żŔ°Ĺ5
			case 128:	//. ŔÚŔĚľ©¢Ć®żŔ°Ĺ6
			case 141:
				if (c->CheckAttackTime(13))
				{
...
void PlayerNpcStopAnimationSetting ( CHARACTER* c, OBJECT* o )
{
    ...
    else
    {
		...
		char szText[512];
		wsprintf(szText, "%s", GlobalText[TextIndex]);
...
// 				CreateEffect(MODEL_LIGHTNING_SHOCK, to->Position, to->Angle, to->Light, 2, to);
		...
		else
		{
			...
			if(o->Type==MODEL_PLAYER && 
				...
				o->CurrentAction==PLAYER_ATTACK_FLY_BOW || o->CurrentAction==PLAYER_ATTACK_FLY_CROSSBOW ||
				o->CurrentAction==PLAYER_ATTACK_RIDE_BOW || o->CurrentAction==PLAYER_ATTACK_RIDE_CROSSBOW
				|| o->CurrentAction==PLAYER_FENRIR_ATTACK_BOW || o->CurrentAction==PLAYER_FENRIR_ATTACK_CROSSBOW	//^ Ćć¸± ¢©şĹ©© °ü·Ă(żäÁ¤ Č­»ě ©©Ş°¢§°Ô ÇĎ´Â °Í)
				))
            {
				if(AT_SKILL_MULTI_SHOT != ( c->Skill))
...
*/
	...
	if (BMDStruct) 
	{
		DWORD_PTR ObjectStruct = (DWORD_PTR)(&c->Object);
...
// 							Select,RENDER_DOPPELGANGER|RENDER_BRIGHT|RENDER_CHROME);
	...
	else
	{
        ...
        if(nCastle > 0 && nCastle <= BLOODCASTLE_NUM )		//. şí·Żµĺ Ä©©¢©¢©ŔĎ°ćżě
		{
...
CHARACTER *CreateMonster(int Type,int PositionX,int PositionY,int Key)
{
	...
	case 433:
		...
		c->Weapon[0].Level = 11;
		c->Object.Scale = 1.2f;
		strcpy(c->ID,"\xB8\xB6\x61""\xFD\xC7\x52""\xB0\x6E""");
        break;
    case 131 :
    	OpenMonsterModel(61);
    ...
    case 132 :
		...
		c->Object.Scale = 0.8f;
        c->Object.EnableShadow = false;
		strcpy(c->ID,"\x4C""\x73""\x52""\xDA\x52""\xC7\x4C""\xAE\xB0\xFC");
		break;
	case 133:
		OpenMonsterModel(60);
		c = CreateCharacter(Key,MODEL_MONSTER01+60,PositionX,PositionY);
	...
	case 430:
		...
		c->Weapon[1].Level = 0;
		c->Object.Scale = 1.0f;
		strcpy(c->ID,"\x43""\x63""\xC7\x52""\xB0\x6E""\x52""\xFC\xBB\xE7");
        break;
    case 87 :
	case 93 :
	case 99 :
	...
	case 431:
		...
		c->Object.Scale = 0.8f;
		strcpy(c->ID,"\x52""\xDA\x52""\x45""\x6C""\xA9\xA2\x43""\xAE\x7A""\x52""\x7A""\x65""\xB0\x4C""");
        break;
    case 88 :
	case 94 :
	case 111 :
	...
	case 432:
		...
		strcpy(c->ID,"\x73""\xD3\x52""\x73""\xC7\x52""\xB0\x6E""\xB1\xE2\xBB\xE7");
        break;
	case 78:
    	OpenMonsterModel(19);
		...
		c->Weapon[0].Level = 9;
		c->Object.Scale = 0.8f;
		strcpy(c->ID,"\xB0\xED\x73""\xED\xB8\xB0");
		break;
	case 79:
    	OpenMonsterModel(31);
		c = CreateCharacter(Key,MODEL_MONSTER01+31,PositionX,PositionY);
		strcpy(c->ID,"\xB5\x6C""\xB7\xA2\xA7\xB0\x64""");
		c->Object.Scale = 0.9f;
		break;
	case 80:
    	OpenMonsterModel(36);
	...
	case 466:
		...
		c->Object.Scale = 0.9f;
		strcpy(c->ID,"\x52""\xFA\xC1\xD6\x61""\x54""\x52""\x73"" \xB0\xED\x73""\xED\xB8\xB0");
		o = &c->Object;
		break;
	case 476:
		...
		c->Object.Scale = 1.7f;
		strcpy(c->ID,"\x52""\xFA\xC1\xD6\x61""\x54""\x52""\x73"" \xBB\x65""\x4C""\xB8");
		o = &c->Object;
		break;
	case 300:
	case 62:
	...
	case 43:
		...
		strcpy(c->ID,"\x43""\xA2\xAD\xB1\xDD\x61""\xF6\xC1\xF6\xB5\x6C""\xB7\xA2\xA7\xB0\x64""");
		c->Object.Scale = 0.7f;
		break;
	case 42:
    	OpenMonsterModel(31);
		c = CreateCharacter(Key,MODEL_MONSTER01+31,PositionX,PositionY);
		strcpy(c->ID,"\xC4\x64""\xB5\xD0""");
		c->Object.Scale = 1.3f;
	...
	case 41:
		...
		strcpy(c->ID,"\xB5\x41""\x6C""\xA2\xAD \xC4\xAB\x7A""\x65""");
	...
	case 40:
		...
		strcpy(c->ID,"\xB5\x41""\x6C""\xA2\xAD \xA9\xA9\x53""\x52""\x45""\x43""\xAE");
	...
	case 39:
		...
		strcpy(c->ID,"\x43""\xF7\x52""\x45""\xC1\xA9\xA2 \xA2\xA9\xA6\xB5\xB5\x7A""\x65""");
		c->Object.Scale = 1.2f;
		c->Level = 1;
		break;
	case 38:
	...
	case 37:
		...
		strcpy(c->ID,"\xB5\x41""\x73""\xF4");
		c->Object.Scale = 1.1f;
		break;
	case 36:
    	OpenMonsterModel(28);
		c = CreateCharacter(Key,MODEL_MONSTER01+28,PositionX,PositionY);
		strcpy(c->ID,"\xA2\xA9\xA6\xB5\xB5\x7A""\x65""");
		c->Object.Scale = 1.2f;
		break;
    	/*OpenMonsterModel(7);
		c = CreateCharacter(Key,MODEL_MONSTER01+7,PositionX,PositionY);
		strcpy(c->ID,"\x73""\xED\xB7\x5A""\xB5\x6C"" \xB0\xED\xA2\xA9\x73""\x43""\xAE");
	...
	case 35:
		...
		strcpy(c->ID,"\xB5\x41""\x6C""\xA2\xAD \xB0\xED\xB8\xA8\xA9\xB0\x64""");
		c->Object.Scale = 1.3f;
		c->Weapon[0].Type = MODEL_AXE+8;
		c->Weapon[1].Type = MODEL_AXE+8;
		c->Object.BlendMesh = 1;
	...
	case 34:
		...
		strcpy(c->ID,"\x52""\xFA\xC1\xD6\x61""\x54""\x52""\x73"" \x61""\xFD\xBB\xE7");
		...
		strcpy(c->ID,"\x52""\xDA\x52""\x45""\x6C""\xA9\xA2\x43""\xAE");
	...
	case 28:
		...
		c->Object.Scale = 0.8f;
		strcpy(c->ID,"\xC7\xA9\xA9\xB5\xAD\x52""\x45""\xB1\xAB\x61""\xB0");
		c->Object.BlendMesh = 1;
		break;
	case 27:
    	OpenMonsterModel(20);
		...
		c->Object.Scale = 1.1f;
		strcpy(c->ID,"\xB0\xED\xB8\xAE\x52""\xFC\xB0\x41""");
		break;
	case 26:
    	OpenMonsterModel(19);
		c = CreateCharacter(Key,MODEL_MONSTER01+19,PositionX,PositionY);
	...
	case 25:
		...
		c->Level = 3;
		strcpy(c->ID,"\x6C""\x43""\x52""\x45""\xA2\xA9\x73""\xC4\xFD");
		break;
	case 24:
    	OpenMonsterModel(17);
	...
	case 23:
		...
		strcpy(c->ID,"\x43""\xA8\xA9\xB8\xD3\xB5\x6C""");
		break;
	case 22:
    	OpenMonsterModel(15);
		...
		c->Object.BlendMesh = 0;
		c->Object.BlendMeshLight = 1.f;
		strcpy(c->ID,"\x6C""\xF3\x52""\xA2\xA9\xB1\xAB\x61""\xB0");
		break;
	case 21:
    	OpenMonsterModel(14);
		c = CreateCharacter(Key,MODEL_MONSTER01+14,PositionX,PositionY);
		c->Object.Scale = 0.95f;
		strcpy(c->ID,"\x6C""\x44""\xBB\x65""\x52""\xDA");
		break;
	case 20:
    	OpenMonsterModel(13);
		c = CreateCharacter(Key,MODEL_MONSTER01+13,PositionX,PositionY);
		strcpy(c->ID,"\x4C""\xA9\xA9\x52""\xCE \xB4\xEB\x52""\x6C""");
		c->Object.Scale = 1.4f;
		break;
	case 19:
    	OpenMonsterModel(12);
		c = CreateCharacter(Key,MODEL_MONSTER01+12,PositionX,PositionY);
		strcpy(c->ID,"\x4C""\xA9\xA9\x52""\xCE");
		c->Object.Scale = 1.1f;
		break;
	case 18:
    	OpenMonsterModel(11);
		c = CreateCharacter(Key,MODEL_MONSTER01+11,PositionX,PositionY);
		strcpy(c->ID,"\xB0\xED\xB8\xA8\xA9\xB0\x64""");
		c->Object.Scale = 1.5f;
	...
	case 3:
		...
		strcpy(c->ID,"\xB0\x4C""\x61""\x45""");
		c->Object.Scale = 0.4f;
		break;
	case 17:
    	OpenMonsterModel(10);
		c = CreateCharacter(Key,MODEL_MONSTER01+10,PositionX,PositionY);
		strcpy(c->ID,"\xA2\xA9\xCE\x52""\x45""\x4C""\xA9\xB7\xD3\xA2\xA9\x73""");
		c->Weapon[0].Type = MODEL_AXE+8;
	...
	default:
		...
		if(Type==0)
		{
			...
			strcpy(c->ID,"\x4C""\x4E""\xBB\xD4\x52""\xFC\xBB\xE7");
			c->Object.Scale = 0.8f;
		...
		else if(Type==4)
		{
			...
			strcpy(c->ID,"\x4C""\x4E""\xBB\xD4\x52""\xFC\xBB\xE7 \xB4\xEB\x52""\x6C""");
			c->Object.Scale = 1.15f;
			c->Level = 1;
		}
		else if(Type==8)
		{
			...
			strcpy(c->ID,"\x43""\xF7\x52""\x45""\xC1\xA9\xA2 \x4C""\x4E""\xBB\xD4\x52""\xFC\xBB\xE7");
			c->Object.Scale = 1.f;
			c->Level = 2;
	...
	case 11:
    	OpenMonsterModel(7);
		c = CreateCharacter(Key,MODEL_MONSTER01+7,PositionX,PositionY);
		strcpy(c->ID,"\xB0\xED\xA2\xA9\x73""\x43""\xAE");
		c->Object.AlphaTarget = 0.4f;
		c->MoveSpeed = 15;
		c->Blood = true;
		break;
	case 12:
		...
		strcpy(c->ID,"\x52""\x5A""\x41""\x63""");
		c->Object.Scale = 0.6f;
		break;
	case 13:
    	OpenMonsterModel(8);
		c = CreateCharacter(Key,MODEL_MONSTER01+8,PositionX,PositionY);
		strcpy(c->ID,"\xC7\x64""\xA2\xA9\x73""\x43""\xC4\x52""\x45""\xB4\x6F""");
	...
	case 5:
		...
		{
			c->Object.HiddenMesh = 0;
			strcpy(c->ID,"\xC7\x44""\x7A""\xEE\xB5\x6C""");
		...
		if(Type==5)
		{
			...
			strcpy(c->ID,"\xC7\x64""\xC7\x44""\x7A""\xEE\xB5\x6C""");
			c->Object.Scale = 1.1f;
			c->Level = 1;
		}
		break;
	...
	case 9:
		...
		if(Type==6)
		{
			strcpy(c->ID,"\xB8\xAE\xC4\xA2\xA7");
			c->Weapon[0].Type = MODEL_STAFF+2;
			c->Object.Scale = 0.85f;
		}
		else
		{
			strcpy(c->ID,"\xA2\xA9\x61""\xB4\x6F"" \xB8\xAE\xC4\xA2\xA7");
	...
	case 56:
		c = CreateCharacter(Key,MODEL_PLAYER,PositionX,PositionY);
		strcpy(c->ID,"\xC7\x52""\xB0\x6E""\x52""\xFC\xBB\xE7");
	...
	case 15:
		c = CreateCharacter(Key,MODEL_PLAYER,PositionX,PositionY);
		strcpy(c->ID,"\xC7\x52""\xB0\x6E""\xB1\x41""\x4C""\xF6");
		c->Object.Scale = 1.1f;
	...
	case 16:
		c = CreateCharacter(Key,MODEL_PLAYER,PositionX,PositionY);
		strcpy(c->ID,"\xC7\x52""\xB0\x6E""\x52""\xFC\xBB\xE7 \xB4\xEB\x52""\x6C""");
		c->Object.Scale = 1.2f;
	...
	case 372:
		c = CreateCharacter(Key, MODEL_PLAYER, PositionX, PositionY);
		::strcpy(c->ID, "\x7A""\xA4\xB8\xAE\x43""\xAE \xC7\x52""\xB0\x6E""\x52""\xFC\xBB\xE7");
		c->Object.Scale = 0.95f;
		c->Object.SubType = MODEL_SKELETON_PCBANG;
		break;
	case 373:
		c = CreateCharacter(Key, MODEL_PLAYER, PositionX, PositionY);
		::strcpy(c->ID, "\x52""\x63"" \x7A""\x52""\xB7\xA8\xA9\x4C""\x44""");
		c->Object.Scale = 0.95f;
		c->Object.SubType = MODEL_HALLOWEEN;
		break;
	case 374:
		c = CreateCharacter(Key, MODEL_PLAYER, PositionX, PositionY);
		::strcpy(c->ID, "\x4C""\xA9\xB8\xAE\xA2\xA9\x73""\xB8\xB6\xA2\xA9\x73"" \xB0\xC9");
		c->Object.Scale = 0.85f;
	...
	case 53:
		...
		strcpy(c->ID,"\x4C""\xB8\x52""\x45""\x4C""\x73""");
    	c->Object.Scale = 1.8f;
		c->Object.BlendMesh = 2;
		c->Object.BlendMeshLight = 1.f;
   		o = &c->Object;
    ...
    case 226:
		...
		strcpy ( c->ID, "\xC1\xB6\xB7\x41""\xBB\xE7 NPC" );
        break;
    ...
    case 227:
		...
		strcpy(c->ID,"\xBB\x65""\x52""\xCE\xB8\xB6\xBB\xF3\xC1\xA2\xA7");
        break;

    case 228:
		OpenNpc(MODEL_HERO_SHOP);
		c = CreateCharacter(Key,MODEL_HERO_SHOP,PositionX,PositionY);
		strcpy(c->ID,"\x7A""\xB5\x7A""\x6F""\xBB\xF3\xC1\xA2\xA7");
        break;
#endif	// _PVP_MURDERER_HERO_ITEM

	case 229 :
		c = CreateCharacter ( Key, MODEL_PLAYER, PositionX, PositionY );
		strcpy ( c->ID, "\xB8\xBB\xB7\xD0""" );
	...
	case 230:
		...
		strcpy(c->ID,"\xB7\xCE\xB7\xA8\xA9\x41""\xDF\xB0\xA2\xA7\xBB\xF3\x52""\xCE");
	...
	case 231:
		...
		strcpy(c->ID,"\xB5\x41""\x73""\x6E""\x41""\xDF\xB0\xA2\xA7\xBB\xF3\x52""\xCE");
		break;
	...
	case 241:
    	OpenNpc(MODEL_MASTER);
		c = CreateCharacter(Key,MODEL_MASTER,PositionX,PositionY);
		strcpy(c->ID,"\xB8\xB6\xA2\xA9\x73""\x4C""\xCD");
		break;
	case 256:
		OpenNpc(MODEL_NPC_SERBIS);
		c = CreateCharacter(Key, MODEL_NPC_SERBIS, PositionX, PositionY);
		strcpy(c->ID, "\x4C""\x4C""\xB8\xA8\xA9\x73""\x6E""\xA2\xA9\x73""");
		break;
	case 257:
		c = CreateCharacter(Key, MODEL_PLAYER, PositionX, PositionY);
		MakeElfHelper(c);
		strcpy(c->ID, "\x43""\xE4\x52""\x45""\x6C""\x43""");
		o = &c->Object;
	...
	case 242:
		...
		strcpy(c->ID,"\xB6\xF3\xB6\xF3 \x7A""\xE4\xC1\xA4");
		o = &c->Object;
		o->BlendMesh = 1;
	...
	case 248:
   		OpenNpc(MODEL_MERCHANT_MAN);
		c = CreateCharacter(Key,MODEL_MERCHANT_MAN,PositionX,PositionY);
		strcpy(c->ID,"\xB6\xB0\xB5\x61""\x52""\x45"" \xBB\xF3\x52""\xCE");
	...
	case 251:
		...
		strcpy(c->ID,"\xB4\xEB\x52""\x6C""\x52""\x6C""\x52""\x45"" \xC7\x4E""\xA2\xA9\x73""");
		c->Object.Scale = 0.95f;
		break;
	case 253:
    	OpenNpc(MODEL_MERCHANT_GIRL);
		c = CreateCharacter(Key,MODEL_MERCHANT_GIRL,PositionX,PositionY);
		strcpy(c->ID,"\x61""\xB0\x6C""\x72""\x43""\xC4\xB4\xC2 \x4C""\x4E""\xA9\xA9\x72""");
	...
	case 254:
		...
		strcpy(c->ID,"\xB8\xB6\x61""\xFD\xBB\xE7 \x43""\xC4\xA2\xA9\x41""");
		break;
	case 255:
    	OpenNpc(MODEL_MERCHANT_FEMALE);
		c = CreateCharacter(Key,MODEL_MERCHANT_FEMALE,PositionX,PositionY);
		strcpy(c->ID,"\x4C""\xFA\xC1\xFD\xB8\xB6\xB4\x61"" \xB8\xAE\x6C""\x43""\xB8\x4F""");
	...
	case 368:
		...
		strcpy(c->ID,"\xC1\xA6\xB7\x41""\x52""\xC7\x4C""\x6C""NPC");
		c->Object.Scale = 2.5f;
		c->Object.EnableShadow = false;
		c->Object.m_bRenderShadow = false;
		break;		
	...
	case 376:
		{
			...
			strcpy(c->ID,"\xB0\x72""\x4C""\x73"" NPC");
			c->Object.Scale = 1.0f;
	...
	case 406:
		...
		strcpy(c->ID,"\xBB\xE7\xC1\xA6\xB5\x41""\x73""\xF3");
		break;
	case 407:
		OpenNpc(MODEL_NPC_QUARREL);
		c = CreateCharacter(Key,MODEL_NPC_QUARREL,PositionX,PositionY);
		strcpy(c->ID,"\x7A""\x74""\x6C""\xEE\x7A""\x64""\xC7\xC1\xC4\x6F""\xB7\x4C""");
		c->Object.Scale = 1.9f;
		break;
	case 408:
	...
	case 413:
		{
			...
			strcpy(c->ID,"\xB4\x54""\x4C""\xE4\xA9\xA9\xA2\xA8");
			c->Object.Scale = 0.8f;
			c->Weapon[0].Type = -1;
			c->Weapon[1].Type = -1;
	...
	case 579:
		...
		strcpy(c->ID,"\xB4""a\x73""\x6E""\xB5\x6C""");
		c->Object.Scale = 0.95f;
	...
	case 452:
		...
		strcpy(c->ID,"\xA2\xA9\x41""\xB5\x6C""\xB8\xB6\xA2\xA9\x73""\x4C""\xCD");
		c->Object.Scale = 1.1f;
		c->Object.m_fEdgeScale = 1.2f;
		break;
	case 453:
		...
		strcpy(c->ID,"\xA2\xA9\x41""\xB5\x6C""\x7A""\xAC\xB1\xB8\xB0\xA2\xA7");
		c->Object.Scale = 0.9f;
	...
	case 464:
		{
			c = CreateCharacter(Key,MODEL_PLAYER,PositionX,PositionY);
			//c->Class = 2;
			strcpy(c->ID,"\x41""\x45""\xB1\xE2\x43""\xAD \xB5\xB5\x7A""\x65""\x61""\x45""");
	...
	case 566:
		...
		strcpy(c->ID, "\xB1\x63""\xB5\x6C""\xB0\xFC\xB8\xAE\x52""\xCE \x4C""\xD7\xB8\xA8\xA9\xA2\xA9\x41""\x6C""\x43""");
	...
	case 567:
		...
		strcpy(c->ID, "\xA2\xA9\x4C""\xA9\xA9\x72"" \x73""\xA8\xA9\x52""\x45""\xA9\xA9\x53""");
	...
	case 568:
		{
			...
			strcpy(c->ID, "\xB6\xB0\xB5\x61""\x52""\x45""\xBB\xF3\x52""\xCE \x52""\xDA\x52""\x45""\xB7\xCE");
			c->Object.LifeTime = 100;
			c->Object.Scale = 0.8f;
			c->Object.m_fEdgeScale = 1.1f;
			int i;
	...
	case 577:
		...
		strcpy(c->ID, "\x52""\xE2\x43""\xAD\xBB\xF3\x52""\xCE \xB7\x61""\x52""\x45""\xA9\xA9\x53""");
		c->Object.Scale = 1.1f;
		c->Object.m_fEdgeScale = 1.2f;
		break;
	case 578:
		...
		strcpy(c->ID, "\x61""\xAB\xB1\xE2\xBB\xF3\x52""\xCE \x73""\x4C""\xB7\xCE");
		c->Object.Scale = 0.9f;
		break;
#endif	// ASG_ADD_KARUTAN_NPC
	}
