...
void SEASON3B::CNewUIItemEnduranceInfo::RenderHPUI( int iX, int iY, unicode::t_char* pszName, int iLife, int iMaxLife/*=255*/, bool bWarning/*=false*/ )
{
	...
	SetLegacyColor4f(0.f, 0.f, 0.f, 0.9f);
	RenderColor(iX, iY, PETHP_FRAME_WIDTH, PETHP_FRAME_HEIGHT - 20);
	SetLegacyColor4f(0.f, 0.f, 0.f, 0.6f);
...
bool SEASON3B::CNewUIItemEnduranceInfo::RenderEquipedHelperLife( int iX, int iY )
{
	...
	// The helper HUD is valid only while its equipment slot contains an item.
	// Hero->Helper can briefly retain the previous model during an unequip, and
	// custom helper metadata uses -1 as a sentinel.  Neither represents a pet.
	const ITEM* pHelperItem = &CharacterMachine->Equipment[EQUIPMENT_HELPER];
	if (pHelperItem->Type < 0 || Hero->Helper.Type < MODEL_HELPER)
	{
		return false;
	}
		...
		|| gHelperSystem.CheckIsHelper(Hero->Helper.Type))
 	{
		...
		case MODEL_HELPER:
			{
				unicode::_sprintf( szText, "%s", GlobalText[353] );
			}
			break;
		case MODEL_HELPER+1:
			{
				ITEM_ATTRIBUTE *p = &ItemAttribute[Hero->Helper.Type-MODEL_SWORD];	
				unicode::_sprintf( szText, "%s", p->Name );
			}
			break;
		case MODEL_HELPER+2:
			{
				unicode::_sprintf( szText,"%s", GlobalText[355] );
			}
			break;
		case MODEL_HELPER+3:
			{
				unicode::_sprintf( szText,"%s", GlobalText[354] );
			}
			break;
        case MODEL_HELPER+4:
			{
				unicode::_sprintf( szText,"%s", GlobalText[1187] );
            }
			break;
		case MODEL_HELPER+37:
			{
				unicode::_sprintf( szText, "%s", GlobalText[1916] );
			}
			break;
		case MODEL_HELPER+64:
			{
				unicode::_sprintf( szText, "%s", ItemAttribute[ITEM_HELPER+64].Name );
			}
			break;
		case MODEL_HELPER+65:
			{
				unicode::_sprintf( szText, "%s", ItemAttribute[ITEM_HELPER+65].Name );
			}
			break;
		case MODEL_HELPER+67:
			{
				unicode::_sprintf( szText, "%s", ItemAttribute[ITEM_HELPER+67].Name );
			}
			break;
		case MODEL_HELPER+80:
			{
				unicode::_sprintf( szText, "%s", ItemAttribute[ITEM_HELPER+80].Name );
			}
			break;
		case MODEL_HELPER+106:
			{
				unicode::_sprintf( szText, "%s", ItemAttribute[ITEM_HELPER+106].Name );
			}
			break;
		case MODEL_HELPER+123:
			{
				unicode::_sprintf( szText, "%s", ItemAttribute[ITEM_HELPER+123].Name );
			}
			break;
		}
		...
		{
			unicode::_sprintf(szText, "%s", ItemAttribute[pHelperItem->Type].Name);
		}
...
bool SEASON3B::CNewUIItemEnduranceInfo::RenderEquipedPetLife( int iX, int iY )
{
	...
	unicode::t_char szText[256] = {NULL, };
	unicode::_sprintf( szText, "%s", GlobalText[1214] );
...
bool SEASON3B::CNewUIItemEnduranceInfo::RenderSummonMonsterLife( int iX, int iY )
{
	...
	unicode::t_char szText[256] = {NULL, };
	unicode::_sprintf( szText, "%s", GlobalText[356] );
...
bool SEASON3B::CNewUIItemEnduranceInfo::RenderItemEndurance( int ix, int iY )
{
    ...
    for ( int i=EQUIPMENT_WEAPON_RIGHT; i<MAX_EQUIPMENT; ++i )
    {
		...
		{
			SetLegacyColor4f( 1.f, 0.0f, 0.f, 0.5f );
		}
		else if( pItem->Durability <= iMaxDurability*0.2f )
		{
			SetLegacyColor4f( 1.f, 0.2f, 0.0f, 0.5f );
		}
		else if( pItem->Durability <= iMaxDurability*0.3f )
		{
			SetLegacyColor4f( 1.0f, 0.5f, 0.f, 0.5f );
		}
		else if( pItem->Durability <= iMaxDurability*0.5f )
		{
			SetLegacyColor4f( 1.f, 1.f, 0.f, 0.5f );
		}
