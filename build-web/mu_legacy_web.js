...
var ASM_CONSTS = {
 ...
 212350: () => { if (Module.muAudio && Module.muAudio.context) { try { Module.muAudio.context.close(); } catch (e) {} Module.muAudio = null; } },  
 212480: ($0, $1, $2, $3, $4) => { var audio = Module.muAudio; if (!audio) return 0; var slot = $0; var ptr = $1; var frames = $2; var channels = $3; var rate = $4; var buffer = audio.context.createBuffer(channels, frames, rate); for (var c = 0; c < channels; ++c) { var out = buffer.getChannelData(c); for (var i = 0; i < frames; ++i) { var sample = HEAP16[(ptr >> 1) + i * channels + c]; out[i] = sample / 32768.0; } } audio.buffers[slot] = buffer; if (!audio.gains[slot]) { var gain = audio.context.createGain(); gain.connect(audio.context.destination); audio.gains[slot] = gain; } audio.active[slot] = []; return 1; },  
 213069: ($0) => { var audio = Module.muAudio; if (!audio) return; delete audio.buffers[$0]; if (audio.gains[$0]) { audio.gains[$0].disconnect(); delete audio.gains[$0]; } delete audio.active[$0]; },  
 213251: ($0, $1) => { var audio = Module.muAudio; if (!audio || !audio.buffers[$0]) return 0; if (audio.context.state === 'suspended') audio.context.resume(); var source = audio.context.createBufferSource(); source.buffer = audio.buffers[$0]; source.loop = ($1 != 0); source.connect(audio.gains[$0]); var list = audio.active[$0] || (audio.active[$0] = []); source.onended = function() { var index = list.indexOf(source); if (index >= 0) list.splice(index, 1); }; list.push(source); source.start(0); return 1; },  
 213742: ($0) => { var audio = Module.muAudio; if (!audio || !audio.active[$0]) return; var list = audio.active[$0]; for (var i = 0; i < list.length; ++i) { try { list[i].stop(0); } catch (e) {} } audio.active[$0] = []; },  
 213947: () => { var audio = Module.muAudio; if (!audio) return; for (var slot in audio.active) { var list = audio.active[slot]; for (var i = 0; i < list.length; ++i) { try { list[i].stop(0); } catch (e) {} } audio.active[slot] = []; } },  
 214170: ($0, $1) => { var audio = Module.muAudio; if (audio && audio.gains[$0]) audio.gains[$0].gain.value = $1; },  
 214265: () => { try { if (!Module.muAudio) { var Context = window.AudioContext || window.webkitAudioContext; if (!Context) return 0; var audio = {}; audio.context = new Context(); audio.buffers = {}; audio.gains = {}; audio.active = {}; Module.muAudio = audio; } return 1; } catch (e) { return 0; } },  
 214552: ($0) => { var path = UTF8ToString($0); var url = path.charAt(0) === '/' ? path.substring(1) : path; var xhr = new XMLHttpRequest(); xhr.open('GET', url, false); xhr.overrideMimeType('text/plain; charset=x-user-defined'); try { xhr.send(null); } catch (e) { return -1; } if (xhr.status !== 200 && xhr.status !== 0) return -1; var text = xhr.responseText; var data = new Uint8Array(text.length); for (var i = 0; i < text.length; ++i) data[i] = text.charCodeAt(i) & 0xFF; var barra = path.lastIndexOf('/'); if (barra > 0) { try { FS.mkdirTree(path.substring(0, barra)); } catch (e) {} } try { FS.writeFile(path, data); } catch (e) { err('[assets] FS.writeFile falhou em ' + path + ': ' + e); return -2; } return data.length; },  
 215268: ($0) => { var path = UTF8ToString($0); try { var size = FS.stat(path).size; FS.unlink(path); return size; } catch (e) { return 0; } },  
 215394: ($0) => { var stream = FS.streams[$0]; var sock = stream && stream.node && stream.node.sock; if (!sock) return -2; var queue = sock.recv_queue ? sock.recv_queue.length : 0; if (queue > 0) return queue; var key = sock.daddr + ":" + sock.dport; var peer = sock.peers ? sock.peers[key] : undefined; if (!peer || !peer.socket) return -1; var state = peer.socket.readyState; if (state === 2 || state === 3) return -1; return 0; },  
 215811: ($0) => { try { var handle = window.open(UTF8ToString($0), '_blank'); if (!handle) return 0; try { handle.opener = null; } catch (e) {} return 1; } catch (e) { return 0; } }
};
