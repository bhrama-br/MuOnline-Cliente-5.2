...
| --- | --- | --- |
| `-statictransformcache=on` | **on** | Adia o laço por vértice de `BMD::Transform` até alguém ler os arrays de transformação, e cacheia a pose em `BMD::Animation`. **Ligada por padrão em 2026-08-10:** medida sob multidão em −15% de frame com 100 monstros e −5% com 50 players (`transforms_skipped` de 0 para 394, `cpu_skinning_vertices` −74%). A corretude depende de todo leitor dos arrays globais materializar antes de ler; auditado em 57 leituras / 4 arquivos, todas guardadas, e protegido por `python diagnostic\check_transform_cache_readers.py`. **A etapa `compare` não foi rodada** — diferente de `-cpumatrices`, esta subiu com ganho medido e corretude argumentada, não com divergência medida. `-statictransformcache=off` reverte. |
